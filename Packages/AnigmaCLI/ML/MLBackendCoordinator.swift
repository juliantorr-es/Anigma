...
struct ChatStreamWithBackendConfiguration: Sendable {
...
func chatStreamWithBackend(config: ChatStreamWithBackendConfiguration) async throws {
    switch config.backend {
    case .mlx:
        // Use ml-worker via MLWorkerService for streaming
        // Note: MLWorkerService currently only supports non-streaming chat in this implementation
        // Fallback to non-streaming but yield tokens if possible?
        // Actually, let's just use chat and yield once for now, or update MLWorkerService.
        // ... existing implementation using config.prompt, config.systemPrompt, etc.
    }
}
            let modelId = config.mlxModelsDir?.path ?? "mlx-community/Llama-3.2-3B-Instruct-4bit"
        ...
        case .llamaCpp:
            // llama.cpp streaming not yet implemented - fall back to non-streaming
            guard let provider = llamaChatProvider else {
                throw MLError.backendNotAvailable("llama.cpp")
            }
        ...
        case .deepseek, .openai, .anthropic, .google, .ollama:
            // Use real streaming for cloud providers
            // Better mapping logic would use the cloudProviders dictionary keys
