...
public actor CLIReceiptManager {
    ...
    public func recordIndexing(
        ...
        let receiptID = UUID().uuidString
        let now = Date().timeIntervalSince1970
        let runID = runID ?? "standalone-index"
    ...
    public func recordWorktreeOperation(
        ...
        let receiptID = UUID().uuidString
        let now = Date().timeIntervalSince1970
        let runID = runID ?? "standalone-worktree"
    ...
    public func verifyChain(runID: String) async throws -> ChainVerification {
        ...
        return ChainVerification(
            ...
            errors: errors
struct StoreReceiptConfiguration: Sendable {
...
func storeReceipt(config: StoreReceiptConfiguration) async throws {
    let metadataJSON = try JSONSerialization.data(withJSONObject: config.metadata)
    let metadataString = String(data: metadataJSON, encoding: .utf8) ?? "{}"
    
    _ = try await db.execute("""
        INSERT INTO receipts (
            receipt_id, run_id, step_id, receipt_type,
            request_hash, response_hash, metadata, timestamp
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        config.receiptID,
        config.runID,
    ...
    private func decodeReceipt(_ row: CLIRow) throws -> Receipt {
        ...
        if let metadataStr = row["tool_metadata"]?.asString,
           ...
           let dict = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            metadata = dict.compactMapValues { $0 as? String }
        }
