...
# Script to package the daemon-backed MCP bridge as a Claude Desktop .mcpb extension
...
echo "═══════════════════════════════════════════════════════════════"
echo "Anigma Daemon-backed MCP Extension Packaging"
echo "═══════════════════════════════════════════════════════════════"
echo ""

# Step 1: Build anigmad
echo "Step 1/5: Building anigmad (release mode)..."
cd "$REPO_ROOT"
...
echo ""
echo "Step 5/5: Creating anigmad.mcpb package..."

# Create temporary staging directory
STAGING_DIR=$(mktemp -d)
trap "rm -rf $STAGING_DIR" EXIT

BUNDLE_DIR="$STAGING_DIR/anigmad"
mkdir -p "$BUNDLE_DIR"

# Copy files to staging
cp "$EXTENSION_DIR/manifest.json" "$BUNDLE_DIR/"
cp "$EXTENSION_DIR/README.md" "$BUNDLE_DIR/"
cp "$BUILD_DIR/anigmad" "$BUNDLE_DIR/"
chmod +x "$BUNDLE_DIR/anigmad"
...
OUTPUT_FILE="$OUTPUT_DIR/anigmad.mcpb"
cd "$STAGING_DIR"
zip -r -q "$OUTPUT_FILE" anigmad/
...
echo ""
echo "Installation:"
echo "  1. Double-click anigmad.mcpb"
