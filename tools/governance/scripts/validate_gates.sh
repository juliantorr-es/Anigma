...
# 4. GATE_LOGGING_COVERAGE
if run_gate "GATE_LOGGING_COVERAGE"; then
    echo "🔍 [GATE_LOGGING_COVERAGE] Checking critical lifecycle, error, and fallback logging..."

    check_logging_surface() {
        local file_path="$1"
        local control_pattern="$2"
        local log_pattern="$3"
        local label="$4"

        if [ ! -f "$file_path" ]; then
            echo "❌ Error: Missing logging surface file: $file_path"
            exit 1
        fi

        if ! grep -qE "$control_pattern" "$file_path"; then
            echo "❌ Error: Missing control-flow coverage for $label in $file_path"
            echo "   Expected pattern: $control_pattern"
            exit 1
        fi

        if ! grep -qE "$log_pattern" "$file_path"; then
            echo "❌ Error: Missing useful logging for $label in $file_path"
            echo "   Expected pattern: $log_pattern"
            exit 1
        fi
    }

    check_logging_surface \
        "$PROJECT_ROOT/Sources/anigmad/main.swift" \
        'guard|catch' \
        'diagnostics\.(beginSpan|event)' \
        "daemon lifecycle"

    check_logging_surface \
        "$PROJECT_ROOT/Packages/AnigmaDaemonCore/Sources/AnigmaDaemonCore/DaemonServer.swift" \
        'guard|catch' \
        'telemetry\.(emit|emitError)' \
        "daemon server lifecycle"

    check_logging_surface \
        "$PROJECT_ROOT/Sources/ContextumModule/Systems/ChunkingSystem.swift" \
        'catch|fallbackChunkText|return fallbackChunkText' \
        'telemetry\.emitError' \
        "chunking fallback"

    check_logging_surface \
        "$PROJECT_ROOT/Sources/ContextumModule/Systems/RedactionSystem.swift" \
        'catch' \
        'telemetry\.emitError' \
        "redaction error handling"

    check_logging_surface \
        "$PROJECT_ROOT/Sources/AnigmaAppMac/ErrorHandling.swift" \
        'catch' \
        'errorLogger\.warning' \
        "UI retry logging"

    check_logging_surface \
        "$PROJECT_ROOT/Sources/AnigmaCLI/main.swift" \
        'catch|guard' \
        'cliLogger\.warning' \
        "CLI fallback logging"

    echo "✅ Logging coverage passed."
fi

# 5. GATE_BUILD_HYGIENE
