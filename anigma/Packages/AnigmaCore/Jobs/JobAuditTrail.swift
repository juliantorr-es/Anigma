...
public actor JobAuditTrail {
    ...
    public func getAuditStatistics(timeRange: TimeRange? = nil) async -> AuditStatistics {
        ...
        let userCounts = Dictionary(grouping: relevantEntries.compactMap { $0.userId })
            .mapValues { $0.count }
    ...
    private func loadExistingEntries() async {
        ...
        do {
            ...
            print("Loaded \(entries.count) audit entries from persistence")
        } catch {
...
public enum ComplianceRegulation: String, Codable, CaseIterable {
