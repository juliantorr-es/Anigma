    ...
    private func executeJob(_ record: JobRecord) async {
        ...
        let task = Task {
            do {
                ...
                await failJob(jobId: jobId, error: error, timedOut: error is JobError.executionTimeout)
            }
        }
...
extension JobError {
    public static func executionTimeout(_ jobId: JobId, _ timeout: TimeInterval) -> JobError {
        return .executionFailed("Job \(jobId) timed out after \(timeout) seconds")
    }
}
