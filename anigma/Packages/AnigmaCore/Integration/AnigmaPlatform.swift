...
public struct ComplianceReport: Sendable {
    ...
    public init(isCompliant: Bool = true, checkedAt: Date = Date(), violations: [String] = []) {
...
public actor AnigmaPlatform {
    ...
    public var governance: GovernanceController {
    ...
    public static func bootstrap(
        ...
        await securedWorld.governance.setMode(configuration.initialMode, by: "bootstrap")
    ...
    private func checkSecurityHealth() async -> SecurityHealthStatus {
        let registeredModels = await security.modelIntegrity.listModels().count
        let allVulns = await security.vulnerabilityManager.listVulnerabilities()
        var pendingCount = 0
        for vuln in allVulns {
            if vuln.status != .fixed && vuln.status != .fixReady {
                pendingCount += 1
            }
        }
        ...
        return SecurityHealthStatus(
            ...
            pendingVulnerabilities: pendingCount
        )
    }
    ...
    public func generateComplianceReport(
        ...
        return ComplianceReport(isCompliant: true, checkedAt: Date())
    }
    ...
    private static func configureStrictAccessControl(
        ...
        await securedWorld.governance.accessController.addPolicy(
