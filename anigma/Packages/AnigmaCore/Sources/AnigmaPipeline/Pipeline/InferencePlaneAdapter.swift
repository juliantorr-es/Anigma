...
import MLWorkerInterfaces  // For MLWorkerInterface
...
public struct MLWorkerInferencePlane: InferencePlane {
    ...
    public func perform(_ request: InferenceRequest) async throws -> InferenceResponse {
        ...
        switch request.task {
        case .textGeneration, .chat, .summarize, .codeGeneration, .codeExplanation, .extraction, .translation:
        ...
        case .toolCall:
            throw InferenceError.unavailable("Tool call task not supported by MLWorker adapter")
        }
    }
