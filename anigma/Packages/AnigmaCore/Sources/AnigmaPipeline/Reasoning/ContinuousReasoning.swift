...
public actor ContinuousReasoningEngine {
    ...
    private func processEvent(_ event: ReasoningEvent) async {
        ...
        case ReasoningEvent.scheduledRun:
    ...
    private func updateRiskFromEvent(_ event: ReasoningEvent) async {
        ...
        case .complianceDeadline(_, let daysUntil):
            ...
            await orchestrator.updateDomainRisk(ReasoningDomain.compliance, factor: ReasoningRiskFactor.complianceDeadline, magnitude: magnitude)
        default:
            break
        }
    }
    ...
    private func handleResult(_ result: ReasoningResult, domain: ReasoningDomain, puzzle: ReasoningPuzzle) async {
        if result.outcome == ReasoningOutcome.foundViolation {
            stats.issuesFound += 1
            ...
            if let scenario = await scenarioLibrary.createFromResult(
                result,
                puzzle: puzzle,
                kernelType: SpecializedKernelType.generalPurpose,
    ...
    private func handleEnsembleResult(_ result: EnsembleAnalysisResult, domain: ReasoningDomain, puzzle: ReasoningPuzzle) async {
        ...
        for (kernelType, kernelResult) in result.kernelResults {
            if kernelResult.outcome == ReasoningOutcome.foundViolation {
                stats.issuesFound += 1
...
public struct CICDReasoningGate: Sendable {
    ...
    public init(
        ...
        requiredDomains: [ReasoningDomain] = [ReasoningDomain.accessControl, ReasoningDomain.tenantIsolation]
    ) {
    ...
    public static let standard = CICDReasoningGate(
        ...
        maxCriticalIssues: 0,
        requiredDomains: [ReasoningDomain.accessControl, ReasoningDomain.tenantIsolation, ReasoningDomain.auditIntegrity]
    )
}
    ...
    public func generateSuggestions(
            ...
            for constraintId in result.violatedConstraints {
                if let constraint = puzzle.constraints.first(where: { $0.constraintId == constraintId }) {
                    suggestions.append(ProbeSuggestion(
                        ...
                        frequency: ReasoningProbeFrequency.hourly,
                        severity: constraint.severity == .critical ? ReasoningIssueSeverity.high : ReasoningIssueSeverity.medium,
        ...
        if result.stepsExplored > 50 {
            suggestions.append(ProbeSuggestion(
                ...
                frequency: ReasoningProbeFrequency.daily,
                severity: ReasoningIssueSeverity.low,
