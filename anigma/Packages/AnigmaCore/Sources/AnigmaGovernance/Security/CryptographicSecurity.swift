...
public actor KeyManager {
    ...
    public func generateSymmetricKey(
        ...
        if let log = auditLog {
            try? await log.recordEvent(
                id: UUID(),
                type: AuditEventType.dataCreated,
                principal: createdBy,
    ...
    public func destroyKey(_ keyId: UUID, destroyedBy: String, reason: String) async throws {
        ...
        if let log = auditLog {
            try? await log.recordEvent(
                id: UUID(),
                type: AuditEventType.dataDeleted,
...
public enum HashUtilities {
    /// Computes BLAKE3 hash (Saturated Architecture default).
    public static func blake3(_ data: Data) -> Data {
        Data(BLAKE3Digest.digest(data))
    }

    /// Computes BLAKE3 hash as hex string.
