...
public struct ThreatLevelPolicy: EnforcementPolicy, Sendable {
    ...
    public func evaluate(_ threat: DetectedThreat) -> EnforcementAction? {
        ...
        let action = levelActions[threat.level]
        
        // Log to SecurityEventsManager if action is triggered
        if let action = action, action != .logOnly, let manager = SecurityEventingService.shared {
            manager.logThreatEnforcement(
                threatType: threat.threatType,
                threatLevel: threat.level.label,
                source: threat.source,
                target: threat.target,
                action: action.rawValue,
                reason: "Threat detected and enforcement action triggered"
            )
        }
    ...
    public func evaluate(_ threat: DetectedThreat) -> EnforcementAction? {
        ...
        if let manager = SecurityEventingService.shared {
            manager.logThreatEnforcement(
                ...
                target: threat.target,
                action: action.rawValue,
                reason: "Threat type matched policy: \(name)"
            )
        }
...
public actor PolicyEnforcementEngine {
    ...
    public func enforce(_ threat: DetectedThreat) async -> EnforcementDecision {
        ...
        if let log = auditLog {
            try? await log.recordEvent(
                id: UUID(),
                type: applied ? AuditEventType.policyEvaluated : AuditEventType.policyViolation,
    ...
    public func evaluateAction(
        ...
        if action.type == .callAPI && context.zone == SecurityZone.sandbox {
        ...
        // Log security event if not allowed
        if !allowed, let securityManager = SecurityEventingService.shared {
            securityManager.logCapabilityDecision(
                engineId: context.sessionId,
                capability: action.type.rawValue,
                granted: false,
                trustTier: trustTier.rawValue,
                zone: context.zone.rawValue,
                reason: policyViolations.first?.description ?? "Policy violation"
            )
        }
        ...
        if let log = auditLog {
            try? await log.recordEvent(
                id: UUID(),
                type: allowed ? AuditEventType.policyEvaluated : AuditEventType.policyViolation,
