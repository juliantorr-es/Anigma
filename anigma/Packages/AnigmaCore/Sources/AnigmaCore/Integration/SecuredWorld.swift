...
public actor SecuredWorld {
    ...
    public let security: SecurityInfrastructure
    ...
    /// Creates a new SecuredWorld with all infrastructure initialized.
    public static func create() async -> SecuredWorld {
        let world = World()
        let auditLog = NoOpAuditLogger()
    ...
    public func addComponent<C: Component>(
        ...
        try await governance.accessController.checkAccess(accessRequest)
        ...
        guard decision.allowed else {
            throw GovernanceError.writeBlocked(reason: decision.failedChecks.map { $0.message }.joined(separator: "; "))
        }
    ...
    */
    ...
    public func handleThreat(
        ...
        // Enforce the threat
        let decision = await security.enforcementEngine.enforce(threat)
        ...
        try? await governance.auditLog.recordEvent(
            ...
            principal: principal.id,
            module: principal.module,
            description: "Threat detected: \(threat.description) - Action: \(decision.action.rawValue)",
            metadata: [
                "threat_id": threat.id.uuidString,
                "threat_level": String(describing: threat.level),
                "enforcement_action": decision.action.rawValue
            ]
        )
    ...
    public func setMode(_ mode: OperatingMode, as principal: AccessPrincipal) async throws {
        ...
        await governance.setMode(mode, by: principal.id)
    }
    ...
    public func activateKillSwitch(reason: String, as principal: AccessPrincipal) async throws {
        ...
        await governance.killSwitch.activate(reason: reason, by: principal.id)
    }
    ...
    public func deactivateKillSwitch(as principal: AccessPrincipal) async throws {
        ...
        await self.governance.killSwitch.deactivate(by: principal.id)
    }
    ...
    private func checkKillSwitch(principal: AccessPrincipal) async throws {
        ...
        let allowed = await governance.killSwitch.isWriteAllowed(forProject: projectId)

        if !allowed {
            let status = await governance.killSwitch.status()
    ...
    private func checkWriteAllowed(
        ...
        if !decision.allowed {
            throw GovernanceError.writeBlocked(
...
public final class SecuredWorldObserver: WorldObserver, @unchecked Sendable {
    ...
    public func entityCreated(_ entity: EntityId) async {
        try? await securedWorld.governance.auditLog.recordEvent(
            id: UUID(),
            type: ContractsCore.AuditEventType.custom,
            principal: principal.id,
            module: principal.module,
