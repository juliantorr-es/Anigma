...
actor InferenceAuthorityImpl: InferenceAuthority {
    ...
    func rerank(
        ...
        let decision = await governance.canWrite(proposal)
        guard decision.allowed else {
            throw RuntimeError.governanceViolation("Rerank denied by policy")
        }
        
        // 2. Determine plane - rerank usually runs on worker plane as it's batch processing
        let plane = workerPlane
        
        // 3. Create inference request for rerank
        // Note: We need to map RerankRequest to InferenceRequest supported by the plane
        // This is a simplification; in a real implementation, planes would support rerank directly
        let inferenceRequest = InferenceRequest(
            task: .rerank,
            input: request.query,
            attachments: [], // Rerank attachments if any
            modelID: request.modelID,
            options: ["documents": .string(try! JSONEncoder().encode(request.documents).base64EncodedString())]
        )
        
        // 4. Perform
        let response = try await plane.perform(inferenceRequest)
        
        // 5. Parse response (mock)
        // In real impl, we parse the JSON output from the model
        return RerankResponse(
            results: request.documents.enumerated().map { index, doc in
                RerankResultItem(index: index, score: 0.9, document: doc)
            },
