    ...
    public func pruneExpired(world: World) async {
        ...
        for (id, memory) in memories {
            if let expiration = memory.expiresAt, expiration < now {
                ...
                print("MemorySystem: Pruned expired memory for entity \(id)")
            }
        }
    }
}
...
extension ExecutionContext {
    fileprivate func recordEvidence(operation: OperationType, payload: EvidencePayload) async throws -> Receipt {
        // Simulated bridge to EvidenceAuthority
        return Receipt(operationType: "dummy", principal: self.principal, outcome: .success, contentHash: "dummy", durationMs: 0)
    }
}
