...
public enum JobErrorClassification: String, Codable, Sendable, CaseIterable {
    ...
    public var isRetryable: Bool {
        ...
        case .networkTransient, .resourceTransient, .systemOverload, 
             .serviceUnavailable, .systemBug, .timeout, .unknown, .resourceExhaustion:
            return true
        case .networkPermanent, .resourcePermanent, .invalidInput,
             .missingDependencies, .serviceDecommissioned, .securityViolation, .configurationError:
            return false
        }
    }
...
public actor JobErrorClassifier {
    ...
    public init() {
        ...
        classificationRules[.contains("timed out")] = .networkTransient
        classificationRules[.contains("unreachable")] = .networkTransient
        
        // System overload
        classificationRules[.contains("too many requests")] = .systemOverload
        classificationRules[.contains("rate limit")] = .systemOverload
        classificationRules[.contains("capacity exceeded")] = .systemOverload
        
        // Resource exhaustion
        classificationRules[.contains("out of memory")] = .resourceExhaustion
        classificationRules[.contains("disk space")] = .resourceExhaustion
        
        // Invalid input
        classificationRules[.contains("invalid argument")] = .invalidInput
        classificationRules[.contains("bad request")] = .invalidInput
        classificationRules[.contains("parse error")] = .invalidInput
        
        // Configuration issues
        classificationRules[.contains("missing configuration")] = .configurationError
        classificationRules[.contains("invalid api key")] = .configurationError
    }
    ...
    private func classifyByErrorType(_ error: Error) -> JobErrorClassification {
        switch error {
        case is JobError:
            let jobError = error as! JobError
            switch jobError {
        ...
        case is WorkflowError:
            let workflowError = error as! WorkflowError
            switch workflowError {
            case .executionTimeout:
                return .timeout
            case .noWorkflowFound:
                return .missingDependencies
            case .systemNotFound:
                return .missingDependencies
            default:
...
public struct RecoveryStrategy: Sendable, Codable {
    ...
    public static func forClassification(_ classification: JobErrorClassification) -> RecoveryStrategy {
        ...
        case .resourceExhaustion:
            return RecoveryStrategy(classification: classification, action: .backoff, delay: 180, maxAttempts: 3)
        case .configurationError:
            return RecoveryStrategy(classification: classification, action: .fail, delay: 0, maxAttempts: 0)
        case .unknown:
