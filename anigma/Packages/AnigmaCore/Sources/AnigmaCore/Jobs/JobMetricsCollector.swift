...
public actor JobMetricsCollector: JobMetricsCollectorProtocol {
    // MARK: - Configuration
    ...
    public func getOverallStats() async -> JobOverallStats {
        ...
        let sub24 = recentMetrics.filter { $0.type == .jobSubmitted }.count
        let comp24 = recentMetrics.filter { $0.type == .jobCompleted }.count
        let fail24 = recentMetrics.filter { $0.type == .jobFailed }.count
        let comp7d = weeklyMetrics.filter { $0.type == .jobCompleted }.count
        let fail7d = weeklyMetrics.filter { $0.type == .jobFailed }.count
        ...
        return JobOverallStats(
            ...
            submittedLast24h: sub24,
            completedLast24h: comp24,
            failedLast24h: fail24,
            completedLast7d: comp7d,
            failedLast7d: fail7d,
    ...
    public func getPerformanceMetrics(timeRange: TimeRange, jobType: String? = nil) async -> [PerformanceDataPoint] {
        let cutoff: Date
        ...
        var relevantMetrics = metricsHistory.filter { $0.timestamp >= cutoff }
        
        if let jobType = jobType {
            relevantMetrics = relevantMetrics.filter { $0.jobType == jobType }
        }
        ...
        while currentTime < endTime {
            ...
            let avgTime: TimeInterval? = durations.isEmpty ? nil : durations.reduce(0, +) / Double(durations.count)
            let throughput: Double = Double(completed + failed) / interval
    ...
    public func exportMetrics(format: ExportFormat) async -> Data {
        ...
        case .auditlog:
            // Audit log format is not supported for metrics, returning JSON as fallback
            return try! JSONEncoder().encode(stats)
        }
    }
