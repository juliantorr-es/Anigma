...
public actor StateAccess {
    ...
    public func readState(
        ...
        securityZone: ContractsCore.SecurityZone = .selfHost
        ...
        let result = StateSlice(
            ...
            relations: currentState.relations.filter { relation in
                !quarantineList.contains(relation.fromEntity) &&
                !quarantineList.contains(relation.toEntity)
            },
        ...
        try await auditLog.recordEvent(
            id: UUID(),
            type: ContractsCore.AuditEventType.dataRead,
            principal: nil,
            module: "StateAccess",
    ...
    public func writeState(
        ...
        delta: StateDelta,
        trustTier: AnigmaPrimitives.TrustTier = .bronze,
        securityZone: ContractsCore.SecurityZone = .standard
    ) async throws -> StateSlice {
        let startTime = Date()
    ...
    private func evaluateAccessRequest(_ request: StateAccessRequest) async throws -> StateAccessDecision {
        ...
        _ = StepContext(
            ...
            securityZone: ContractsCore.SecurityZone(rawValue: request.securityZone.rawValue)!,
            deadline: nil
        )
        ...
        case .read:
            ...
            let allowedZones: [ContractsCore.SecurityZone] = [.selfHost, .inspiration]
            guard allowedZones.contains(ContractsCore.SecurityZone(rawValue: request.securityZone.rawValue)!) else {
                return StateAccessDecision(
                    allowed: false,
                    reason: "Read access not allowed in security zone: \(request.securityZone.rawValue)"
                )
            }
        ...
        case .write:
            ...
            guard request.trustTier.rawValue >= AnigmaPrimitives.TrustTier.silver.rawValue else {
                return StateAccessDecision(
                    allowed: false,
                    reason: "Write access requires at least Standard trust tier"
                )
            }
            ...
            guard protectedEntities.isEmpty || request.trustTier == .platinum else {
                return StateAccessDecision(
                    allowed: false,
                    reason: "Cannot write protected entities without Platinum trust tier"
                )
            }
        ...
        case .createSession:
            ...
            guard request.trustTier.rawValue >= AnigmaPrimitives.TrustTier.gold.rawValue else {
                return StateAccessDecision(
                    allowed: false,
                    reason: "Session creation requires at least Elevated trust tier"
                )
            }
        ...
        case .deleteSession:
            ...
            guard request.trustTier.rawValue >= AnigmaPrimitives.TrustTier.gold.rawValue else {
                return StateAccessDecision(
                    allowed: false,
                    reason: "Session deletion requires at least Elevated trust tier"
                )
            }
        ...
        case .quarantine, .liftQuarantine:
            ...
            guard request.trustTier == .platinum else {
                return StateAccessDecision(
                    allowed: false,
                    reason: "Quarantine operations require Platinum trust tier"
                )
            }
        }
