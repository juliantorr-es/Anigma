...
fileprivate let log = os.Logger(subsystem: "com.anigma.core", category: "telemetry")
...
public actor TelemetrySystem: System, Sendable {
    ...
    public func record(
        ...
        if level == .critical {
            log.critical("TELEMETRY CRITICAL: \(name, privacy: .public) - \(String(describing: metadata))")
        }
    }
