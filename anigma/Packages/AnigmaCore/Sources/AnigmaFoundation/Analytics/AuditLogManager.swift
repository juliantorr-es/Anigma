...
//  AnigmaFoundation
//
//  High-level audit log management moved to AnigmaFoundation to resolve
//  circular dependencies with AnigmaGovernance.
//
...
public struct AuditFilter: Sendable, Codable {
    ...
    public let eventTypes: [ContractsCore.AuditEventType]?
    ...
    public init(
        startDate: Date? = nil,
        endDate: Date? = nil,
        eventTypes: [ContractsCore.AuditEventType]? = nil,
    ...
    public func matches(_ entry: AuditEntry) -> Bool {
        ...
        if let types = eventTypes, !types.contains(ContractsCore.AuditEventType(rawValue: entry.operation) ?? .custom) { return false }
        if let p = principal, entry.command != p { return false }
        if let m = module, entry.engineId != m { return false }
        if let eId = entityId, entry.metadata?["entity_id"] != eId { return false }
        if let cType = componentType, entry.metadata?["component_type"] != cType { return false }
        return true
    }
}
...
public actor InMemoryAuditStorage: AuditLogStorage {
    ...
    public func query(filter: AuditFilter) async throws -> [AuditEntry] {
    ...
    public func queryLatestEntry() async throws -> AuditEntry? {
        entries.last
    }
...
public actor AuditLogManager: AuditLogging {
    private static let log = os.Logger(subsystem: "com.anigma.core", category: "AuditLogManager")
    ...
    public func recordEvent(
        ...
        do {
            ...
            Self.log.error("Audit log storage failure: \(error.localizedDescription)")
            throw error
        }
    }
    ...
    /// Gets recent entries.
    public func recent(count: Int = 100) async throws -> [AuditEntry] {
    ...
    public func entryCount() async -> Int {
        do {
            ...
            Self.log.error("Error getting entry count from storage: \(error.localizedDescription)")
            return 0
        }
    }
