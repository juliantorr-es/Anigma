...
public protocol ConfigProvider: Sendable {
    func load() async throws -> [String: ConfigValue]
}

public actor ConfigManager {
    private var providers: [any ConfigProvider] = []
    private var mergedConfig: [String: ConfigValue] = [:]
    private var watcher: ConfigWatcher?

    public init() {}

    /// Deep merge dictionaries where later values override earlier ones.
    private func deepMerge(_ base: [String: ConfigValue], with new: [String: ConfigValue]) -> [String: ConfigValue] {
        var merged = base
        for (key, newValue) in new {
            if let baseValue = merged[key]?.value as? [String: ConfigValue],
               let nextValue = newValue.value as? [String: ConfigValue] {
                merged[key] = ConfigValue(deepMerge(baseValue, with: nextValue))
            } else {
                merged[key] = newValue
            }
        }
    ...
    public func reload() async throws {
        var newConfig: [String: ConfigValue] = [:]
...
public struct FileConfigProvider: ConfigProvider {
    ...
    public func load() async throws -> [String: ConfigValue] {
        let data = try Data(contentsOf: url)
        // In a real implementation, use a YAML/JSON decoder
        return try JSONDecoder().decode([String: ConfigValue].self, from: data)
    }
}
    ...
    public func load() async throws -> [String: ConfigValue] {
        ...
        for (key, value) in ProcessInfo.processInfo.environment {
            if key.hasPrefix(prefix) {
                ...
                config[String(cleanKey)] = ConfigValue(value)
            }
        }
        return config
    }
...
public struct ConfigValue: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        } else if let dictionary = try? container.decode([String: ConfigValue].self) {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let bool = value as? Bool {
            ...
            try container.encode(array.map { ConfigValue($0) })
        } else if let dictionary = value as? [String: Sendable] {
            try container.encode(dictionary.mapValues { ConfigValue($0) })
        }
    }
}
