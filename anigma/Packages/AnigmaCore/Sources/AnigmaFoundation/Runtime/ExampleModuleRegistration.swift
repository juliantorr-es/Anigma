...
public struct ExampleWorkflow: PlatformWorkflow {
    ...
    public func execute(
        ...
        if processedData.count > 1000 {
            ...
            log.info("Stored artifact: \(artifactId, privacy: .public)")
        }
...
public struct ExampleAppShell {
    public static func main() async throws {
        ...
        log.info("Workflow completed: \(receipt.id, privacy: .public)")
        log.info("Outcome: \(String(describing: receipt.outcome), privacy: .public)")
        log.info("Duration: \(receipt.durationMs)ms")

        // 5. Check runtime status
        let status = await runtime.status()
        log.info("Runtime initialized: \(status.isInitialized)")
        log.info("Registered schemas: \(status.registeredSchemas)")
        log.info("Governance mode: \(String(describing: status.governanceStatus.operatingMode), privacy: .public)")
