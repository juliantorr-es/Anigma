...
public actor Scheduler {
    ...
    public init(
        ...
        circuitBreaker: CircuitBreaker = CircuitBreaker(state: .closed, failureCount: 0)
    ) {
    ...
    private func completeJob(jobId: JobId, result: JobResult, timedOut: Bool) async {
        ...
        await logJobCompleted(jobId: jobId, durationMs: Int(result.durationMs))
    }
    ...
    private func failJob(jobId: JobId, error: Error, timedOut: Bool) async {
        ...
        if record.canRetry && !timedOut && isRetryableError(error) {
            ...
            await logJobRetryScheduled(jobId: jobId, retryDelay: retryDelay)
        } else {
            await logJobFailedPermanently(jobId: jobId, error: error)
        }
    }
    ...
    private func persistRecord(_ record: JobRecord) async {
        ...
        do {
            ...
            await logFailedToPersistRecord(recordId: record.id.raw, error: error)
        }
    }
    ...
    public func cancel(jobId: JobId) async throws {
        ...
        await logJobCancelled(jobId: jobId)
    }
...
public struct CircuitBreaker: Sendable {
    public private(set) var state: CircuitBreakerState
    ...
    public init(state: CircuitBreakerState = .closed, failureCount: Int = 0) {
        self.state = state
...
private extension Scheduler {
    func logJobCancelled(jobId: JobId) async {
        logInfo("Job cancelled: \(jobId.raw)")
    }
    
    func logJobCompleted(jobId: JobId, durationMs: Int) async {
        logInfo("Job completed: \(jobId.raw) in \(durationMs)ms")
    }
    
    func logJobRetryScheduled(jobId: JobId, retryDelay: Date) async {
        logInfo("Job retry scheduled: \(jobId.raw) at \(retryDelay.iso8601)")
    }
    
    func logJobFailedPermanently(jobId: JobId, error: Error) async {
        logError("Job failed permanently: \(jobId.raw) - \(error.localizedDescription)")
    }
    
    func logFailedToPersistRecord(recordId: String, error: Error) async {
        logError("Failed to persist record \(recordId): \(error.localizedDescription)")
    }
}
