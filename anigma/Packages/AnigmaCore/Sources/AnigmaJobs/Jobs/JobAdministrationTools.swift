...
public actor JobAdministration {
    ...
    public init(
        ...
        metricsCollector: JobMetricsCollectorProtocol,
    ...
    public func getJobDetails(_ jobId: JobId, includeAudit: Bool = true) async -> JobDetails {
        let record = await unifiedQueue.getJob(jobId)
    ...
    public func getSystemOverview() async -> SystemOverview {
        let queueStats = await unifiedQueue.getStats()
    ...
    public func cancelJob(
        ...
        guard let record = await unifiedQueue.getJob(jobId) else {
    ...
    public func retryJob(
        ...
        retryReason: String,
        requestedBy: String,
        newPriority: AnigmaFoundation.JobPriority? = nil,
    ...
    public func getJobTypeAnalysis(_ jobType: String, timeRange: TimeRange) async -> JobTypeAnalysis {
        ...
        let performanceData = await metricsCollector.getPerformanceMetrics(timeRange: timeRange, jobType: jobType)
        let recentEntries = await auditTrail.getAuditEntries(
            startDate: timeRange.startDate,
            endDate: Date()
        ).filter { entry in
            // Filter entries related to this job type.
            // This would need job record lookup when available.
            entry.jobId != nil
        }
    ...
    public func bulkJobOperation(_ operation: BulkJobOperation) async throws -> BulkJobResult {
        ...
        await auditTrail.recordSystemEvent(
            ...
            systemInfo: [
                "operation": String(describing: operation.type),
    ...
    private func analyzeErrorPatterns(from entries: [JobAuditEntry]) -> [ReportedErrorPattern] {
...
public struct JobTypeAnalysis: Codable, Sendable {
    ...
    public let errorPatterns: [ReportedErrorPattern]
...
/// Reported error pattern for analysis.
public struct ReportedErrorPattern: Codable, Sendable {
...
public enum JobSortField: String, Codable, CaseIterable, Sendable {
    case createdAt
    case priority
    case status
    case attempts
...
public enum BulkOperationType: Codable, Sendable {
    case cancel(reason: String, cancelledBy: String)
    case retry(reason: String, requestedBy: String)
}

public enum MaintenanceType: String, Codable, Sendable {
    case planned
    case emergency
}

public enum AdminPermission: String, Codable, CaseIterable, Sendable {
    case viewJobs
    case cancelJobs
    case retryJobs
