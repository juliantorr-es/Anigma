...
public actor UnifiedJobQueue {
    ...
    public func start() async throws {
        ...
        logInfo("Job queue started with max concurrency: \(self.maxConcurrentJobs)", category: "jobs")
    }
    ...
    public func stop() async {
        ...
        logInfo("Job queue stopped", category: "jobs")
    }
    ...
    private func restoreJobs() async {
        do {
            ...
            logInfo("Restored \(activeJobs.count) jobs from persistence", category: "jobs")
            await recordMetric(.jobsRestored, count: activeJobs.count)

        } catch {
            logError("Failed to restore jobs: \(error.localizedDescription)")
        }
    }
    ...
    public func submit(
        ...
        logInfo("Job submitted: \(record.id) (type: \(job.typeId))")
        return record
    }
    ...
    public func submitBatch(_ jobs: [Job]) async throws -> [JobRecord] {
        ...
        logInfo("Job batch submitted: \(jobs.count) jobs")
    ...
    private func completeJob(jobId: JobId, result: JobResult, timedOut: Bool) async {
        ...
        logInfo("Job completed: \(jobId) in \(result.durationMs)ms")
    }
    ...
    private func failJob(jobId: JobId, error: Error, timedOut: Bool) async {
        ...
        if shouldRetry {
            ...
            logInfo("Job retry scheduled: \(jobId) at \(retryTime.iso8601)")
            
        } else {
            await recordMetric(.jobFailed, jobId: jobId, jobType: record.job.typeId)
            logError("Job failed permanently: \(jobId) - \(error.localizedDescription)", category: "jobs")
        }
    ...
    private func persistAndNotify(_ record: JobRecord) async {
        do {
            ...
            logError("Failed to persist record \(record.id): \(error.localizedDescription)", category: "jobs")
        }
    }
    ...
    public func retry(_ jobId: JobId) async throws {
        ...
        logInfo("Manually retrying job: \(jobId)")
    }
}
