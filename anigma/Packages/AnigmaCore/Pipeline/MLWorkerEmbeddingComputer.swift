...
public struct MLWorkerEmbeddingComputer: EmbeddingComputing {
    ...
    public func computeEmbeddings(
        ...
        for (index, inputString) in inputs.enumerated() {
            let inputHash = ContractKeyDerivation.sha256Hex(Data(inputString.utf8))
            ...
            let response = try execute(request: request)
...
private func execute(request: MLWorkerRequest) throws -> MLWorkerResponse {
    let process = Process()
    process.executableURL = URL(fileURLWithPath: requestExecutablePath())
...
private func requestExecutablePath() -> String {
    return ProcessInfo.processInfo.environment["ML_WORKER_PATH"] ?? "./.build/debug/ml-worker"
}
