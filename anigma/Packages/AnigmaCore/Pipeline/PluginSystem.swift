...
public enum ScriptRuntimeError: Error, LocalizedError, Sendable {
...
public struct UnavailableScriptRuntime: ScriptRuntime {
    ...
    public func execute(
        ...
        throw ScriptRuntimeError.unavailable("No script runtime registered.")
    }
}
...
public struct LocalProcessScriptRuntime: ScriptRuntime {
    ...
    public func execute(
        ...
        guard result.exitCode == 0 else {
            throw ScriptRuntimeError.executionFailed(result.stderr)
        }
...
private struct ScriptValue: Codable {
    ...
    static func from(portValue: AnyPortValue) throws -> ScriptValue {
        ...
        throw ScriptRuntimeError.executionFailed("Unsupported port type: \(portValue.typeId)")
    }
    ...
    return try await withCheckedThrowingContinuation { continuation in
        ...
        if timeout > 0 {
            DispatchQueue.global().asyncAfter(deadline: .now() + timeout) {
                ...
                process.terminate()
                finish(.failure(ScriptRuntimeError.executionFailed("Script timed out.")))
            }
        }
    }
}
