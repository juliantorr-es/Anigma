...
public struct AccessPrincipal: Sendable, Hashable, Codable {
    ...
    /// Creates a principal for a System.
    public static func system(_ name: String, module: String, roles: Set<String> = []) -> AccessPrincipal {
        AccessPrincipal(id: name, module: module, roles: roles)
    }
}
...
/// Protocol for access control policies.
...
public protocol AccessPolicy: Sendable {
    ...
    /// Evaluates whether this policy applies to the request.
    /// Returns nil if the policy doesn't apply (continue to next policy).
    func evaluate(_ request: AccessRequest) -> AccessDecision?
}
...
public struct RoleBasedPolicy: AccessPolicy, Sendable {
    ...
    public func evaluate(_ request: AccessRequest) -> AccessDecision? {
...
public actor AccessController {
    ...
    public func evaluate(_ request: AccessRequest) async -> AccessDecision {
        ...
        for policy in policies {
            if let result = policy.evaluate(request) {
                decision = result
                break
            }
        }
    ...
    public func listPolicies() -> [(id: String, priority: Int)] {
...
extension World {
    ...
    public func getComponentControlled<C: SensitiveComponent>(
        _ entity: EntityId,
        _ type: C.Type,
        principal: AccessPrincipal,
    ...
    public func queryControlled<C: SensitiveComponent>(
        _ type: C.Type,
        principal: AccessPrincipal,
        controller: AccessController,
        justification: String? = nil
