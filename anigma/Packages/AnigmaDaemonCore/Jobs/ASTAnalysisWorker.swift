...
public struct ASTAnalysisWorker: JobWorker {
    ...
    public func execute(
        ...
        return try await WorkerTooling.withTemporaryDirectoryAsync(prefix: "ast-analysis-job") { dir in
            ...
            let statusData = try JSONEncoder().encode(ASTServiceStatusPayload(status))
...
private struct ASTServiceStatusPayload: Codable, Sendable {
    let isAvailable: Bool
    let currentLoad: Double
    let cacheStats: ASTCacheStatsPayload?
    let engineInfo: ASTEngineInfoPayload?

    init(_ status: ASTServiceStatus) {
        self.isAvailable = status.isAvailable
        self.currentLoad = status.currentLoad
        self.cacheStats = status.cacheStats.map(ASTCacheStatsPayload.init)
        self.engineInfo = status.engineInfo.map(ASTEngineInfoPayload.init)
    }
}

private struct ASTCacheStatsPayload: Codable, Sendable {
    let entries: Int
    let sizeBytes: Int
    let hitRate: Double

    init(_ stats: ASTCacheStats) {
        self.entries = stats.entries
        self.sizeBytes = stats.sizeBytes
        self.hitRate = stats.hitRate
    }
}

private struct ASTEngineInfoPayload: Codable, Sendable {
    let engineId: String
    let version: String
    let swiftSyntaxVersion: String

    init(_ info: ASTEngineInfo) {
        self.engineId = info.engineId
        self.version = info.version
        self.swiftSyntaxVersion = info.swiftSyntaxVersion
    }
}
