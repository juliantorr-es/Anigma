...
public struct ASTTransformWorker: JobWorker {
    ...
    public func execute(
        ...
        // 5. Package Results
        let resultData = try JSONEncoder().encode(ASTTransformResultPayload(result))
        return [
            JobOutputPayload(
                data: resultData,
...
private struct ASTTransformResultPayload: Codable, Sendable {
    ...
    init(_ result: PipelineResult) {
        self.filesProcessed = result.filesProcessed
        self.filesModified = result.filesModified
        self.totalChanges = result.totalChanges
        self.errors = result.errors.map { $0.description }
        self.changes = result.changes.map { String(describing: $0) }
        self.outcomes = result.outcomes.map { String(describing: $0) }
        self.duration = result.duration
        self.verification = result.verification.map { VerificationPayload(success: $0.success, detail: $0.detail) }
    }

    struct VerificationPayload: Codable, Sendable {
        let success: Bool
        let detail: String
    }
}
