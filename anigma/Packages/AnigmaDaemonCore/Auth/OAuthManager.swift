...
public actor OAuthManager {
    ...
    private func loadOrGenerateKeys() async throws {
        ...
        if let row = rows.first, let keyData = row["private_key"]?.blobValue {
            ...
            try await database.execute(insert, parameters: [
                .text(keyId),
                .blob(keyData),
                .integer(Int(Date().timeIntervalSince1970))
            ])
        }
    }
    ...
    public func createUser(username: String, roles: [String]) async throws -> OAuthUser {
        ...
        try await database.execute(sql, parameters: [
            ...
            .text(user.username),
            .text(rolesString),
            .integer(Int(user.createdAt.timeIntervalSince1970))
        ])
    ...
    public func getUser(username: String) async throws -> OAuthUser? {
        ...
        guard let row = rows.first,
              ...
              let rolesString = row["roles"]?.textValue,
              let createdAtInt = row["created_at"]?.integerValue else {
            return nil
        }
    ...
    public func refreshToken(token: String) async throws -> TokenResponse {
        ...
        guard let row = rows.first,
              let userId = row["user_id"]?.textValue,
              let clientId = row["client_id"]?.textValue,
              let scopesString = row["scopes"]?.textValue,
              let expiresAtInt = row["expires_at"]?.integerValue,
    ...
    private func getUserById(id: String) async throws -> OAuthUser? {
        ...
        guard let row = rows.first,
              let username = row["username"]?.textValue,
              let rolesString = row["roles"]?.textValue,
              let createdAtInt = row["created_at"]?.integerValue else {
            return nil
        }
    ...
    public func validateJWT(_ token: String) throws -> JWTPayload {
