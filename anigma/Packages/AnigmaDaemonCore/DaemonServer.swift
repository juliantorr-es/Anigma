...
public actor DaemonServer {
    ...
    public init(configuration: DaemonConfiguration) async throws {
        ...
        let databaseAuthority = DatabaseAuthorityImpl(databaseActor: self.database, governance: governance, evidenceAuthority: nil)
    ...
    private func setupSignalHandlers() async {
        await signalManager.setupDefaultHandlers(
            shutdownHandler: { [weak self] in await self?.handleGracefulShutdown() },
            reloadHandler: { [weak self] in await self?.handleConfigurationReload() }
        )
    }
