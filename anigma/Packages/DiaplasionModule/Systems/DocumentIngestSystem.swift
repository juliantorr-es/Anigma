    ...
    public func update(world: World) async {
        ...
        for (entity, fileComp) in files {
            ...
            guard let path = fileComp.path else {
                logWarning("FileComponent has no path", category: "Diaplasion")
                continue
            }
            ...
            do {
                ...
                logInfo("Ingested document: \(path)", category: "Diaplasion")
            } catch {
                logError("Ingest failed for \(path): \(error)", category: "Diaplasion")
                // Add error state
