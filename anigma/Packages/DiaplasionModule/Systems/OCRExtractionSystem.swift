    ...
    public func update(world: World) async {
        ...
        for (entity, ingested) in documents {
            ...
            guard ingested.isComplete else {
                logWarning("Skipping OCR for incomplete document", category: "Diaplasion")
            ...
            if let textContent = ingested.textContent {
                ...
                if trimmed.isEmpty {
                    logWarning("Skipping OCR for empty extracted text", category: "Diaplasion")
                    await appendProcessingError(
                        config: AppendProcessingErrorConfiguration(
                            world: world,
                            entity: entity,
            ...
            guard !ingested.pageImagePaths.isEmpty else {
                logWarning("Skipping OCR for missing page images", category: "Diaplasion")
                await appendProcessingError(
                    config: AppendProcessingErrorConfiguration(
                        world: world,
                        entity: entity,
            ...
            do {
                ...
                logError("OCR failed: \(error)", category: "Diaplasion")
                await appendProcessingError(
                    config: AppendProcessingErrorConfiguration(
                        world: world,
                        entity: entity,
