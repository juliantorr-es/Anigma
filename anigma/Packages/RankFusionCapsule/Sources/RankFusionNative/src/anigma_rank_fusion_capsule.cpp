...
struct RankFusionCapsuleImpl {
    ...
    bool fuse(uint32_t k, double* out_scores, uint64_t* out_ids, size_t max_results, size_t* out_actual) const {
        ...
        for (const auto& list : rank_lists) {
            total_ids += list.size();
            for (const auto& entry : list) {
                max_rank = std::max(max_rank, static_cast<size_t>(entry.second));
            }
        }
        score_map.reserve(total_ids);

        std::vector<double> reciprocal_cache;
        try {
            reciprocal_cache.resize(max_rank + 1);
            const double kf = static_cast<double>(k);
            for (size_t rank = 0; rank <= max_rank; ++rank) {
                reciprocal_cache[rank] = 1.0 / (kf + static_cast<double>(rank));
            }
        } catch (...) {
            return false;
        }
        ...
        for (const auto& list : rank_lists) {
            for (const auto& entry : list) {
                const size_t rank = static_cast<size_t>(entry.second);
                const double contribution = reciprocal_cache[rank];
                auto insert_result = score_map.try_emplace(entry.first, contribution);
                if (!insert_result.second) {
                    insert_result.first->second += contribution;
                }
            }
        }
...
anigma_status_t anigma_rank_fusion_capsule_create(
    ...
    auto impl = std::make_unique<RankFusionCapsuleImpl>();
    if (!impl) {
        if (err) {
