...
private func renderPlanError(_ status: anigma_status_t, _ message: String) -> CapsuleNativeError {
    CapsuleNativeError(status: status, code: status, message: message)
}
...
public final class RenderPlanBuilder {
    ...
    public init(opCapacity: UInt32 = 1024, resourceCapacity: UInt32 = 256) throws {
        ...
        guard status == ANIGMA_OK else {
            ...
            throw renderPlanError(status, "render plan builder create failed")
        }
    }
    ...
    public func addClear(layerId: UInt32, color: ResourceRef) throws {
        guard let builder = builderPtr.pointee else { throw renderPlanError(ANIGMA_ERR_INVALID_ARG, "render plan builder not initialized") }
        let status = anigma_render_plan_builder_add_clear(builder, layerId, color.toCStruct())
        guard status == ANIGMA_OK else {
            throw renderPlanError(status, "render plan add clear failed")
        }
    }
    ...
    public func addRect(layerId: UInt32, rect: Rect, transform: Transform2D, material: ResourceRef) throws {
        ...
        guard status == ANIGMA_OK else {
            throw renderPlanError(status, "render plan add rect failed")
        }
    }
    ...
    public func build() throws -> RenderPlan {
        ...
        guard status == ANIGMA_OK, let plan = planPtr else {
            throw renderPlanError(status, "render plan build failed")
        }
...
public final class RenderPlan {
    ...
    public func computeHash() throws {
        ...
        guard status == ANIGMA_OK else {
            throw renderPlanError(status, "render plan compute hash failed")
        }
    }
    
    public static func generate(from scene: NativeSceneGraph, request: RenderRequest) throws -> RenderPlan {
        var planPtr: anigma_render_plan_t? = nil
        ...
        guard status == ANIGMA_OK, let plan = planPtr else {
            throw renderPlanError(status, "render plan generation failed")
        }
