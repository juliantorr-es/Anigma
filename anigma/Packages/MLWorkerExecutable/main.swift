...
class BackendRunner {
    ...
    func execute(
        ...
        throw RuntimeError("BackendRunner.execute() must be implemented by subclass")
    }
    ...
    func buildArgv(
        ...
        throw RuntimeError("BackendRunner.buildArgv() must be implemented by subclass")
    }
...
class LlamaBackendRunner: BackendRunner {
    ...
    private func executeServerChat(
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            ...
            throw RuntimeError("llama-server chat failed: HTTP \(status) \(bodyText)")
        }
    ...
    private func executeServerEmbed(
        ...
        guard let http = response as? HTTPURLResponse else {
            throw RuntimeError("llama-server embedding failed: no HTTP response")
        }
        ...
        do {
            ...
            let bodyHash = MLWorkerHasher.hashData(data)
            throw RuntimeError("llama-server embedding decode failed bodyHash=\(bodyHash)")
        }
        guard let vector = decoded.data.first?.embedding else {
            throw RuntimeError("llama-server embedding missing vector")
        }
        guard !vector.isEmpty else {
            throw RuntimeError("llama-server embedding empty vector")
        }
        var bin = Data()
        for v in vector {
            var f = Float(v)
...
class DeepSeekBackendRunner: BackendRunner {
    ...
    override func execute(
        ...
        guard !apiKey.isEmpty else {
            throw RuntimeError("DEEPSEEK_API_KEY not set")
        }

        switch task {
        case .embed:
            throw RuntimeError("DeepSeek embeddings are not supported by provider")
        case .chat:
            return try executeChat(
                inputFile: inputFile, outputFile: outputFile, options: options, requestId: requestId
            )
        default:
            throw RuntimeError("Task \(task.rawValue) not supported for DeepSeek")
        }
    }
    ...
    private func executeChat(
        ...
        let task = session.dataTask(with: request) { data, response, error in
            if let error = error {
                resultError = RuntimeError("DeepSeek request failed: \(error.localizedDescription)")
                semaphore.signal()
                return
            }
            guard let data = data else {
                resultError = RuntimeError("DeepSeek returned no data")
                semaphore.signal()
                return
            }
            ...
            if statusCode < 200 || statusCode >= 300 {
                let bodyText = String(data: data, encoding: .utf8) ?? "<non-utf8>"
                resultError = RuntimeError("DeepSeek HTTP \(statusCode): \(bodyText)")
                semaphore.signal()
                return
            }
        ...
        guard let data = resultData else {
            throw RuntimeError("DeepSeek returned no data")
        }
...
class MLXBackendRunner: BackendRunner {
        ...
        override func execute(
            ...
            case .chat:
                ...
                return BackendResult<String, String>(
                    ...
                    timedOut: false)
            default:
                throw RuntimeError("Task \(task.rawValue) not supported for MLX backend")
            }
        }
        ...
        override func execute(
            ...
            throw RuntimeError("MLX backend not available; rebuild with MLXLMCommon")
        }
...
class CoreMLBackendRunner: BackendRunner {
    ...
    override func execute(
        ...
        requestId: String
            ...
            case .embed:
                ...
                return BackendResult<String, String>(stdout: Data(), stderr: Data(), exitCode: 0, timedOut: false)
            case .chat:
                throw RuntimeError("Chat tasks not yet supported for CoreML backend")
            default:
                throw RuntimeError("Task \(task.rawValue) not supported for CoreML backend")
            }
        #else
            throw RuntimeError("CoreML backend not available; rebuild with CoreML support")
        #endif
    }
    ...
    private func generateEmbedding(modelPath: String, inputFile: URL, outputFile: URL, seed: Int) throws {
        ...
        guard let embeddingOutput = output.featureValue(for: "embedding")?.multiArrayValue else {
            throw RuntimeError("CoreML model output does not contain 'embedding' feature")
        }
    ...
    func readLimitedData(from handle: FileHandle, maxBytes: Int) throws -> Data {
        ...
        while true {
            ...
            if totalRead >= maxBytes {
                throw RuntimeError("Output exceeded maximum size limit (\(maxBytes) bytes)")
            }
        }
...
struct MLWorkerCommand: ParsableCommand {
    ...
    private func computeModelHash(at path: String) throws -> String {
        ...
        guard FileManager.default.fileExists(atPath: path, isDirectory: &isDirectory) else {
            throw RuntimeError("Model path does not exist: \(path)")
        }
        ...
        for input in request.inputs {
            ...
            case .chat:
                if engine == .llama {
                    ...
                    throw RuntimeError("Chat tasks not yet supported for engine \(engine.rawValue)")
                }
    ...
    private func processEmbedding(
        ...
        _ = sha256Hex(backendResult.stderr)
...
private func runBlocking<T: Sendable>(_ operation: @escaping @Sendable () async throws -> T) throws -> T {
    ...
    case .failure(let error):
        throw error
    case .none:
        throw RuntimeError("runBlocking failed to produce result")
    }
}
...
extension URLSession {
    fileprivate func syncData(for request: URLRequest) throws -> (Data, URLResponse) {
        ...
        guard let data = resultData, let response = resultResponse else {
            throw RuntimeError("URLSession syncData failed to produce result")
        }
