        ...
        if !chainValid {
            divergences.append(
                ReplayDivergence(
                    ...
                    type: .brokenChain,
                    description: "CoreReceipt chain integrity check failed",
                    severity: .critical
                ))
        }
    ...
    private func detectDivergence(receipt: ReceiptWire, event: ReplayEvent) -> ReplayDivergence? {
        ...
        if receipt.signature == nil || receipt.signature!.isEmpty {
            return ReplayDivergence(
                ...
                description: "CoreReceipt lacks cryptographic signature",
                severity: .high
            )
        }
        ...
        if !event.success {
            return ReplayDivergence(
                receiptID: receipt.receiptID,
                type: .replayFailed,
                description: "CoreReceipt replay did not succeed",
                severity: .medium
            )
        }
...
public struct ReplayDivergence: Codable, Sendable {
    /// CoreReceipt ID where divergence occurred
    public let receiptID: String
