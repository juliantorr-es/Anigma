...
public actor AnigmaAuthority: AuthorityRouting {
    ...
    public func updateIR(_ ir: PresentationIR, for surfaceId: SurfaceId) -> String? {
        if var state = activeSurfaces[surfaceId] {
            ...
            let canonicalData = (try? ir.contentOnlyEncodeToSerializedContent()) ?? Data()
    ...
    public func validateAndRouteIntent(_ intent: ActionIntent) async -> PlatformReceipt {
        ...
        let jobSpec = AnigmaJobSpec(
            kind: intent.action.rawValue,
            configCanonical: (try? encodeJobConfig(intent)) ?? Data(),
            inputs: []
        )

        let optimisticReceipt = PlatformReceipt(
            ref: ReceiptRef(id: UUID().uuidString, intentHash: intentHash),
            status: PlatformReceipt.Status.success,
            outcome: .string("Intent \(intent.action.rawValue) accepted and routed to daemon."),
            seal: ReceiptSeal.unsigned(hash: intentHash)
        )
    ...
    private func submitAndMonitorJob(
        ...
        do {
            ...
            guard let error = response.error, !error.code.isEmpty else {
                let jobId = response.jobId ?? UUID().uuidString
                print(
                    "Successfully submitted job to anigmad. ID: \(jobId) CoreReceipt: \(response.receiptHash ?? "")"
                )
                self.activeJobs[jobId] = JobState(
                    jobId: jobId,
                    intent: intent,
                    workspaceId: intent.header.surfaceId.rawValue,
                    submittedAt: Date(),
                    status: "QUEUED"
                )
                emitJobUpdate(jobId: jobId)
                await self.publishIRUpdateForJob(surfaceId: surfaceId)
                let eventStream = try await self.sidecarBridge.streamJobEvents(jobId: jobId)
                            ...
                            if let hash = jobState.finalReceiptHash, !hash.isEmpty {
                                do {
                                    ...
                                    if verificationResponse.ok {
                                        jobState.receiptVerificationStatus = ReceiptVerificationStatus.verified
                                        jobState.isVerified = true
                                        jobState.verifiedAt = Date()
                                        print("✅ CoreReceipt chain VERIFIED for job \(jobId)")
                                    } else {
                                        jobState.receiptVerificationStatus = ReceiptVerificationStatus.failed(
                                            reason: verificationResponse.message)
                                        jobState.verificationError = verificationResponse.message
                                        print(
                                            "❌ CoreReceipt chain FAILED verification for job \(jobId): \(verificationResponse.message)"
                                        )
                                    }
                                } catch {
                                    jobState.receiptVerificationStatus = ReceiptVerificationStatus.failed(
                                ...
                                jobState.receiptVerificationStatus = ReceiptVerificationStatus.unavailable
            ...
            print("anigmad rejected job: \(error.code) \(error.message)")
        } catch {
        ...
        for (jobId, jobState) in activeJobs {
            let jobNode = ViewNode(
                ...
                properties: [
                    ...
                    // CoreReceipt verification status (zero-trust model)
    ...
    private func makeDenial(for intent: ActionIntent, code: DenialCode) -> PlatformReceipt {
        let intentHash = calculateIntentHash(intent)
        return PlatformReceipt(
            ref: ReceiptRef(id: UUID().uuidString, intentHash: intentHash),
            status: PlatformReceipt.Status.blocked,
    ...
    private func calculateIntentHash(_ intent: ActionIntent) -> String {
        ...
        let intentData = (try? encoder.encode(intent)) ?? Data()
    ...
    private struct JobState {
        ...
        // CoreReceipt verification policy
    ...
    /// CoreReceipt verification status for zero-trust model.
...
extension AnigmaAuthority {
    ...
    public func updateWorkspace(id: String, name: String) throws {
        guard var workspace = workspaces[id] else {
            throw GenericCoreError.notFound("Workspace \(id) not found")
        }
        workspace.name = name
        workspaces[id] = workspace
...
extension AnigmaAuthority {
    ...
    public func uploadArtifact(workspaceId: String, name: String, data: Data) throws -> String {
        guard var workspace = workspaces[workspaceId] else {
            throw GenericCoreError.notFound("Workspace \(workspaceId) not found")
        }
    ...
    public func downloadArtifact(id: String) throws -> Data {
        guard let artifact = artifacts[id] else {
            throw GenericCoreError.notFound("Artifact \(id) not found")
        }
...
extension AnigmaAuthority {
    ...
    public func cancelJob(jobId: String) async throws {
        guard let job = activeJobs[jobId] else {
            throw GenericCoreError.notFound("Job \(jobId) not found")
        }
        ...
        if let error = response.error, !error.code.isEmpty {
            throw GenericCoreError.sidecarError(error.message)
        }
    }
    ...
    public func deleteJob(jobId: String) throws {
        ...
        guard job.isTerminal else {
            throw GenericCoreError.invalidIntent("Cannot delete non-terminal job \(jobId)")
        }
    ...
    public func verifyJob(jobId: String) async throws {
        ...
        guard let hash = job.finalReceiptHash, !hash.isEmpty else {
            throw GenericCoreError.invalidIntent("Job \(jobId) has no final receipt hash")
        }
