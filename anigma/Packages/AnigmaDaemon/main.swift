...
if CommandLine.arguments.contains("--worker") {
    ...
    // Initialize Registry and Workers
    ...
    do {
        ...
        let job = payload.job
        fputs("Worker: executing job \(job.id) (kind: \(job.spec.kind))\n", stderr)
        fflush(stderr)

        // Resolve worker
        guard let worker = await registry.worker(for: job.spec.kind) else {
            fputs("Worker: unknown job kind \(job.spec.kind)\n", stderr)
            exit(1)
        }
        ...
        let outputs = try await worker.execute(
            ...
            config: job.spec.configCanonical,
...
do {
    ...
    // Create daemon
    let daemon = try await DaemonServer(configuration: configuration)
