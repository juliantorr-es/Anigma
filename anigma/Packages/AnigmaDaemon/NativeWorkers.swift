...
@preconcurrency import SidecarPDFService
...
public struct HeavyPDFWorker: JobWorker {
    ...
    private let service = NativePDFService()
    ...
    public func execute(
        ...
        if cfg.operation == "merge" {
            ...
            let result = try service.merge(pdfs: payloads)
            return [JobOutputPayload(data: result, mediaType: "application/pdf", kind: "derived")]
        } else if cfg.operation == "split", let p = cfg.page, let input = inputs.first, let payload = vaultData[input.hash] {
            let (part1, part2) = try service.split(pdf: payload, at: p)
            return [
