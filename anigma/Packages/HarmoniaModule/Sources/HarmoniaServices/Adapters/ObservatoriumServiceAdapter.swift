...
import enum ObservatoriumModule.TimeRange

private typealias ObservatoriumTimeRange = TimeRange
...
public actor ObservatoriumServiceAdapter: ServiceHandler {
    ...
    public nonisolated let serviceId: String = "observatorium-service"
    private let coordinator: ObservatoriumCoordinator
    
    public nonisolated let descriptor: ServiceDescriptor
    ...
    private func getMetrics(input: [String: AnyCodable]) async throws -> [String: AnyCodable] {
        var timeRange = ObservatoriumTimeRange.lastDay
    ...
    private func parseTimeRange(_ rangeStr: String) -> ObservatoriumTimeRange {
        ...
        case "lastmonth":
            let now = Date()
            return .custom(DateInterval(start: now.addingTimeInterval(-30 * 24 * 60 * 60), end: now))
        case "custom":
            let now = Date()
            return .custom(DateInterval(start: now.addingTimeInterval(-24 * 60 * 60), end: now))
        default:
            return .lastDay
        }
    }
    ...
    private func encodeTimeRange(_ timeRange: ObservatoriumTimeRange) -> String {
        switch timeRange {
        case .lastHour:
            return "lastHour"
        case .lastDay:
