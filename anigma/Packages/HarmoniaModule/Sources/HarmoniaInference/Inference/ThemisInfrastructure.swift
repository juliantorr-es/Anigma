...
typealias ThemisCharter = InstitutionalCharter
typealias NemesisConstraint = ProhibitedOptimization
typealias EunomiaGuardrails = EthicalGuardrails

// Memory Layer (Mnemosyne = memory)
typealias LetheBuffer = ShortTermMemory
typealias MnemosyneArchive = LongTermMemory
typealias ArcheionStore = PersistentMemory

// Routing Layer (Moirae = fates)
typealias MoiraeRoutingPolicy = ArchitectureSelectionPolicy
typealias MoiraRoutingRule = TaskRoutingRule
typealias AnankeConstraint = RoutingConstraint

// Behavior Layer (Eunomia = good order)
typealias EunomiaGovernor = AgentBehaviorGovernor
typealias EunomiaConstraints = AgentBehaviorConstraints

// Transparency Layer (Aletheia = truth)
// Removed: AletheiaManifest (EnhancedTelemetryManifest deleted in Stage 3)
typealias AletheiaReceipt = ProcessingReceipt

// Learning Layer (Arete = excellence)
typealias AreteImpactMeasurer = LearningImpactMeasurer

// Testing Layer (Eris = strife/chaos)
typealias ErisTrialBuilder = InferencePlanePuzzleBuilder
typealias AgonScenario = AdversarialScenario
...
actor ThemisOrchestrator {
    ...
    init(config: ThemisConfig = .default) async {
        self.config = config
    ...
    static func create(config: ThemisConfig = .default) async -> ThemisOrchestrator {
        await ThemisOrchestrator(config: config)
    }

    /// Creates a Themis orchestrator with a preset configuration.
    static func create(preset: ThemisPreset) async -> ThemisOrchestrator {
    ...
    func runTask(
        ...
        // 10. Build Pipeline Stages for CoreReceipt
        let stages = [
    ...
    func registerCharter(_ charter: InstitutionalCharter) {
        charterRegistry.register(charter)
    }

    /// Gets the charter for a tenant.
    func getCharter(for tenantId: String) -> InstitutionalCharter? {
    ...
    func getHealth() -> ThemisHealth {
        health
    }

    /// Gets comprehensive status.
    func getStatus() async -> ThemisStatus {
    ...
    func runAdversarialAnalysis() async -> ErisReport {
...
final class ThemisCharterRegistry: Sendable {
    private let state = OSAllocatedUnfairLock(initialState: [String: InstitutionalCharter]())

    init() {}

    func register(_ charter: InstitutionalCharter) {
        state.withLock { dict in
            dict[charter.tenantId] = charter
        }
    }

    func get(for tenantId: String) -> InstitutionalCharter? {
...
struct ThemisSessionContext: Sendable {
...
struct ThemisResult: Sendable {
    ...
    let learningEligibility: LearningEligibility
...
struct LearningEligibility: Sendable {
...
/// Health status.
enum ThemisHealth: String, Sendable, Codable {
    case initializing
    case healthy
    case degraded
    case unhealthy
...
/// Errors from Themis.
enum ThemisError: Error, Sendable {
...
enum ErisSeverity: String, Sendable, Codable {
    case low
    case medium
    case high
    case critical
