...
public actor InferenceService {
    ...
    public func chat(
        messages: [HarmoniaInferenceContracts.ChatMessage],
        context: InferenceContext,
        constraints: InferenceConstraints = .internalDefault
    ) async throws -> String {
        let task = InferenceTask(
    ...
    public func registryStats() async -> ModelRegistryStatistics {
    ...
    private func registerBackends() async {
        ...
        log.info("Registered MLX and Mock backends")
    }
}
