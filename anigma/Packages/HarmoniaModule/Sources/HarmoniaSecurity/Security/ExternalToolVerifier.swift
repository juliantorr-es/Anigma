...
public actor ExternalToolVerifier {
    ...
    public func verify(path: String) throws -> Bool {
        ...
        let currentHash = try calculateBlake3(path: path)
        let verified = currentHash == trusted.artifactHash
        
        return verified
    }

    public func trust(path: String, user: String = "system") throws -> TrustedTool {
        let hash = try calculateBlake3(path: path)
        var tools = try listTrustedTools()

        // Remove existing entry for path if any
        tools.removeAll { $0.path == path }

        let tool = TrustedTool(path: path, artifactHash: hash, addedAt: Date(), addedBy: user)
        tools.append(tool)
    ...
    private func calculateBlake3(path: String) throws -> String {
