...
public actor WorkflowExecutor {
    ...
    public func execute() async {
        ...
        await coordinator.updateExecution(execution)
        ...
        // Handle finalization after workflow
        ...
        } else if execution.state == .failed, let error = execution.error {
            let duration = execution.endTime?.timeIntervalSince(execution.startTime ?? Date()) ?? 0
            await sharedEventBus.publish(
                WorkflowCompletedEvent(
                    workflowId: execution.id,
    ...
    private func executeWorkflow() async throws {
        ...
        while !remainingSteps.isEmpty && !cancelled {
            ...
            if definition.parallelizable {
                ...
                try await withThrowingTaskGroup(of: (String, Result<[String: AnyCodable], Error>).self) { group in
    ...
    private func executeStep(stepId: String, input: [String: [String: AnyCodable]]) async -> Result<[String: AnyCodable], Error> {
        ...
        // Update step state
        await updateStepState(stepId: stepId, to: .running)
        ...
        do {
            ...
            let service = try await registry.getService(id: stepDefinition.serviceId)
    ...
    private func processStepResult(
        ...
        case .success(let output):
            ...
            await updateStepState(stepId: stepId, to: .completed, output: output)
        ...
        case .failure(let error):
            ...
            if stepExecution.retryCount < stepDef.retryCount {
                ...
                // Mark as failed
                await updateStepState(stepId: stepId, to: .failed, error: error.localizedDescription)
                throw error
            }
        }
    }
    ...
    private func updateStepState(
        ...
        // Publish progress event for step completion
        if newState == .completed || newState == .failed || newState == .skipped {
            let completedSteps = execution.steps.filter { $0.state == .completed || $0.state == .failed || $0.state == .skipped }.count
            let totalSteps = execution.steps.count
            ...
            await sharedEventBus.publish(
                WorkflowProgressEvent(
                    workflowId: execution.id,
                    progress: progress,
                    message: newState == .completed ? "Step \"stepId\" completed" : newState == .failed ? "Step \"stepId\" failed" : "Step \"stepId\" skipped",
                    stepName: stepId
                )
            )
        }
