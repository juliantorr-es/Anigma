...
public actor HarmoniaCoordinator {
    ...
    public func registerService(_ service: ServiceHandler) async throws {
        try await registry.register(service: service)
    }
    
    public func getAvailableServices() async -> [ServiceDescriptor] {
        await registry.getAllServices()
    }
    
    public func getServiceHealth() async -> [String: HealthStatus] {
    ...
    public func cancelWorkflow(executionId: String) async throws {
    ...
    public func getStatistics() async -> CoordinatorStatistics {
        ...
        let averageDuration = allExecutions.isEmpty ? 0 : totalDuration / Double(allExecutions.count)
        ...
        return CoordinatorStatistics(
            ...
            registeredServices: await registry.registeredServiceCount
        )
    }
}
