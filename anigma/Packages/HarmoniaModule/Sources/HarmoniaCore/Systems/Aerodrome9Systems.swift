...
public struct InputSystem: System {
    ...
    public func update(world: World) async {
        ...
        for (entity, input, velocity) in controlled {
            ...
            updated.dx = clamp(updated.dx, min: -1.0, max: 1.0)
            updated.dy = clamp(updated.dy, min: -1.0, max: 1.0)
...
public struct MovementSystem: System {
    ...
    public func update(world: World) async {
        ...
        for (entity, position, velocity) in movers {
            ...
            updated.x = max(0.0, updated.x)
            updated.y = max(0.0, updated.y)
            if let bounds {
...
public struct RenderingSystem: System {
    ...
    public func update(world: World) async {
        ...
        let count = sprites.count
            logDebug("Prepared \(count) sprite(s) for render", category: "Aerodrome9")
    }
}

// MARK: - Component Definitions

/// Zone for interaction detection
public struct InteractionZone: Component, Sendable, Codable {
    public var radius: Double
    public init(radius: Double = 1.5) {
        self.radius = radius
    }
}

/// Marker for interactable entities
public struct Interactable: Component, Sendable, Codable {
...
/// History of player choices for narrative tracking
public struct ChoiceHistory: Component, Sendable, Codable {
...
/// Current world state with crisis flags
public struct WorldState: Component, Sendable, Codable {
    public var crisisFlags: [String: Bool]
    public init(crisisFlags: [String: Bool] = [:]) {
...
public struct PsychohistorySystem: System {
    ...
    public func update(world: World) async {
        ...
        let worldStates = await world.query(AerodromeWorldState.self)
...
private func resolveBounds(world: World) async -> SceneBounds? {
    ...
    let maxX = max(0.0, width - 1.0)
    let maxY = max(0.0, height - 1.0)
...
private func distance(from a: Position, to b: Position) -> Double {
    let dx = Swift.abs(a.x - b.x)
    let dy = Swift.abs(a.y - b.y)
    return max(dx, dy)
}
