...
public actor PlanCompiler {
    ...
    public func generateEvidenceProducedPlan(
        ...
        print("🏛️ Phase H: Generating evidence-produced plan for '\(request.operationType)'")
        ...
        guard evidenceValidation.hasSufficientEvidence else {
            let blockedPlan = EvidenceProducedPlan(
                ...
                evidenceDigest: calculateEvidenceDigest(
                    evidenceDependencies: evidenceValidation.evidenceDependencies),
                createdAt: Date(),
                executionLease: nil,
                conflictResolution: nil
            )

            print("🚫 Cathedral: Plan BLOCKED - insufficient evidence")
            print("   Required: \(evidenceRequirements.requirement.rawValue)")
            print("   Current: \(evidenceValidation.currentLevel.rawValue)")
            print("   Violations: \(evidenceValidation.violations.count)")
        ...
        print("✅ Cathedral: Evidence-produced plan generated")
        print("   Evidence dependencies: \(plan.evidenceDigest.dependencies.count)")
        print("   Plan hash: \(plan.evidenceDigest.hash)")
        print("   Execution lease: 300s")
    ...
    private func validateCurrentEvidence(
        ...
        let typeValid = requirements.allowedEvidenceTypes.allSatisfy {
            recentEvidence.types.contains($0)
        }

        // Check recency requirements
        let recencyValid = recentEvidence.allSatisfy {
            $0.ageSeconds <= requirements.recentTimeframeSeconds
        }
        ...
        // Collect violations
        var violations: [ContractsCore.EvidenceViolation] = []
        ...
        return EvidenceValidation(
            ...
            evidenceDependencies: recentEvidence.dependencies
        )
    }

    /// Gather recent evidence for a given set of requirements.
    private func gatherRecentEvidence(
        requirements: EvidenceRequirements,
        context: String?
    ) async throws -> RecentEvidence {
        let dependencies = requirements.allowedEvidenceTypes.map { type in
            EvidenceDependency(
                id: "\(type.rawValue)-\(UUID().uuidString.lowercased())",
                type: type,
        ...
        let types = Set(dependencies.map(\.type))
        let maxAge = dependencies.map(\.ageSeconds).max() ?? 0
    ...
    private func createEvidenceBackedPlan(
        ...
        let evidenceDigest = calculateEvidenceDigest(
            evidenceDependencies: evidenceValidation.evidenceDependencies)
        ...
        let plan = EvidenceProducedPlan(
            ...
            reason: .evidenceBacked(
                ...
                confidence: Double(evidenceValidation.currentLevel.level),
    ...
    private func calculateEvidenceDigest(evidenceDependencies: [EvidenceDependency])
        -> EvidenceDigest {
...
public struct RecentEvidence: Sendable {
    ...
    public func allSatisfy(_ condition: (EvidenceDependency) -> Bool) -> Bool {
        dependencies.allSatisfy(condition)
    }

    public var count: Int { dependencies.count }
    public var max: EvidenceDependency? { dependencies.max { $0.ageSeconds < $1.ageSeconds } }
}

/// Evidence dependency for plan verification
public struct EvidenceDependency: Sendable, Hashable, Codable {
...
public enum EvidenceType: String, CaseIterable, Codable, Sendable {
...
public enum PlanReason: Sendable, Codable {
    case insufficientEvidence([ContractsCore.EvidenceViolation])
    ...
    public init(from decoder: Decoder) throws {
        ...
        let type = try container.decode(ReasonType.self, forKey: .type)
        switch type {
        case .insufficientEvidence:
            let violations = try container.decode(
                [ContractsCore.EvidenceViolation].self, forKey: .violations)
            self = .insufficientEvidence(violations)
        case .evidenceBacked:
            let digest = try container.decode(EvidenceDigest.self, forKey: .evidenceDigest)
            ...
            self = .evidenceBacked(
                evidenceDigest: digest, confidence: confidence, assumptions: assumptions)
        case .executionFailed:
            let message = try container.decode(String.self, forKey: .message)
            self = .executionFailed(message)
        case .conflictDetected:
            let message = try container.decode(String.self, forKey: .message)
            self = .conflictDetected(message)
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        switch self {
        case .insufficientEvidence(let violations):
            try container.encode(ReasonType.insufficientEvidence, forKey: .type)
            try container.encode(violations, forKey: .violations)
        case .evidenceBacked(let digest, let confidence, let assumptions):
            try container.encode(ReasonType.evidenceBacked, forKey: .type)
            try container.encode(digest, forKey: .evidenceDigest)
            try container.encode(confidence, forKey: .confidence)
            try container.encode(assumptions, forKey: .assumptions)
        case .executionFailed(let message):
            try container.encode(ReasonType.executionFailed, forKey: .type)
            try container.encode(message, forKey: .message)
        case .conflictDetected(let message):
            try container.encode(ReasonType.conflictDetected, forKey: .type)
            try container.encode(message, forKey: .message)
        }
    }
}
