AnigmaAIConsole package targets and local APIs.
Invariants: respect governance boundaries, keep outputs deterministic, avoid direct IO outside authorities.

Entry points:
- `Packages/AnigmaAIConsole/Sources`

Public surface:
- SwiftPM target `AnigmaAIConsole` (see `Package.swift`).

Build/test:
- `swift build --target AnigmaAIConsole`
