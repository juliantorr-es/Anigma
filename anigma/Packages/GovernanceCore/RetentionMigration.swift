...
public enum RetentionMigration {
    public static func migrate(_ database: any DatabaseExecutor) async throws {
        try await database.executeAsync("BEGIN TRANSACTION")

        do {
            try await createPolicyStorage(database)
            try await createComplianceEvents(database)
            try await createIndexes(database)
            try await database.executeAsync("COMMIT TRANSACTION")
        } catch {
            _ = try? await database.executeAsync("ROLLBACK TRANSACTION")
            throw error
        }
    }

    private static func createPolicyStorage(_ database: any DatabaseExecutor) async throws {
    ...
    private static func createComplianceEvents(_ database: any DatabaseExecutor) async throws {
        try await database.executeAsync(
            """
    ...
    private static func createIndexes(_ database: any DatabaseExecutor) async throws {
        try await database.executeAsync(
            """
