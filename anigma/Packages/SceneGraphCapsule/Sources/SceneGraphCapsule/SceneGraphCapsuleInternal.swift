...
public struct Transform3D: Sendable, Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let values = try container.decode([Double].self, forKey: .matrix)
        guard values.count == 16 else {
            throw DecodingError.dataCorruptedError(forKey: .matrix, in: container, debugDescription: "Expected 16 matrix values.")
        }
        let c0 = SIMD4(values[0], values[1], values[2], values[3])
        let c1 = SIMD4(values[4], values[5], values[6], values[7])
        let c2 = SIMD4(values[8], values[9], values[10], values[11])
        let c3 = SIMD4(values[12], values[13], values[14], values[15])
        self.matrix = simd_double4x4(columns: (c0, c1, c2, c3))
    }
...
public struct CameraComponent: Sendable, Codable {
    public init() {}
}

private enum ComponentPayload: Codable {
    case transform(Transform3D)
    case mesh(TriangleMesh)
    case material(Material)
    case light(Light)
    case camera(CameraComponent)
    case boundingBox(BoundingBox3D)
    ...
    init(from decoder: Decoder) throws {
        ...
        switch type {
        case .transform:
            self = .transform(try container.decode(Transform3D.self, forKey: .value))
        case .mesh:
            self = .mesh(try container.decode(TriangleMesh.self, forKey: .value))
        case .material:
            self = .material(try container.decode(Material.self, forKey: .value))
        case .light:
            self = .light(try container.decode(Light.self, forKey: .value))
        case .camera:
            self = .camera(try container.decode(CameraComponent.self, forKey: .value))
        case .boundingBox:
            self = .boundingBox(try container.decode(BoundingBox3D.self, forKey: .value))
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .transform(let value):
            try container.encode(PayloadType.transform, forKey: .type)
            try container.encode(value, forKey: .value)
        case .mesh(let value):
            try container.encode(PayloadType.mesh, forKey: .type)
            try container.encode(value, forKey: .value)
        case .material(let value):
            try container.encode(PayloadType.material, forKey: .type)
            try container.encode(value, forKey: .value)
        case .light(let value):
            try container.encode(PayloadType.light, forKey: .type)
            try container.encode(value, forKey: .value)
        case .camera(let value):
            try container.encode(PayloadType.camera, forKey: .type)
            try container.encode(value, forKey: .value)
        case .boundingBox(let value):
            try container.encode(PayloadType.boundingBox, forKey: .type)
            try container.encode(value, forKey: .value)
        }
    }
    ...
    static func payload(for type: ComponentType, value: Any) throws -> ComponentPayload {
        switch type {
        case .transform:
            guard let typed = value as? Transform3D else {
                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Expected Transform3D for transform component."))
            }
            return .transform(typed)
        case .mesh:
            guard let typed = value as? TriangleMesh else {
                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Expected TriangleMesh for mesh component."))
            }
            return .mesh(typed)
        case .material:
            guard let typed = value as? Material else {
                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Expected Material for material component."))
            }
            return .material(typed)
        case .light:
            guard let typed = value as? Light else {
                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Expected Light for light component."))
            }
            return .light(typed)
        case .camera:
            guard let typed = value as? CameraComponent else {
                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Expected CameraComponent for camera component."))
            }
            return .camera(typed)
        case .boundingBox:
            guard let typed = value as? BoundingBox3D else {
                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Expected BoundingBox3D for bounding box component."))
            }
            return .boundingBox(typed)
        }
    }
}

/// Scene graph node
public struct SceneNode: Codable, @unchecked Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.parentID = try container.decodeIfPresent(NodeID.self, forKey: .parentID)
        self.childIDs = try container.decode([NodeID].self, forKey: .childIDs)
        let decodedComponents = try container.decodeIfPresent([ComponentType: ComponentPayload].self, forKey: .components) ?? [:]
        var resolved: [ComponentType: Any] = [:]
        resolved.reserveCapacity(decodedComponents.count)
        for (key, payload) in decodedComponents {
            resolved[key] = payload.anyValue
        }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        var payloads: [ComponentType: ComponentPayload] = [:]
        payloads.reserveCapacity(components.count)
        for (key, value) in components {
            payloads[key] = try ComponentPayload.payload(for: key, value: value)
        }
        try container.encode(payloads, forKey: .components)
    }
}
...
public struct SceneObject: @unchecked Sendable {
    public let id: NodeID
    public let boundingBox: BoundingBox3D
    public let data: Any
...
/// KD-tree node
public final class KDTreeNode: @unchecked Sendable {
    public let axis: Int
...
/// Spatial grid
public struct CellCoordinate: Sendable, Hashable {
    public let x: Int
    public let y: Int
    public let z: Int
...
internal final class SceneGraphCapsuleInternal: Sendable {
    ...
    private func cellCoordinate(for point: Point3D, cellSize: Vector3D, origin: Point3D) -> CellCoordinate {
        return CellCoordinate(
            x: Int(floor((point.x - origin.x) / cellSize.x)),
            y: Int(floor((point.y - origin.y) / cellSize.y)),
            z: Int(floor((point.z - origin.z) / cellSize.z))
        )
    }
    ...
    internal func rayBoxIntersection(_ ray: Ray, boundingBox: BoundingBox3D) -> RayIntersection? {
        let boxMin = boundingBox.min
        let boxMax = boundingBox.max
        ...
        var tmin = Swift.min(t1, t2)
        var tmax = Swift.max(t1, t2)
        
        t1 = (boxMin.y - ray.origin.y) * invDir.y
        t2 = (boxMax.y - ray.origin.y) * invDir.y
        tmin = Swift.max(tmin, Swift.min(t1, t2))
        tmax = Swift.min(tmax, Swift.max(t1, t2))
        
        t1 = (boxMin.z - ray.origin.z) * invDir.z
        t2 = (boxMax.z - ray.origin.z) * invDir.z
        tmin = Swift.max(tmin, Swift.min(t1, t2))
        tmax = Swift.min(tmax, Swift.max(t1, t2))
        
        if tmax >= Swift.max(0, tmin) {
            let point = Point3D(
    ...
    internal func rayTriangleIntersection(_ ray: Ray, triangle: (Point3D, Point3D, Point3D)) -> RayIntersection? {
        ...
        if Swift.abs(a) < 0.00001 { return nil } // Ray parallel to triangle
    ...
    internal func calculatePhongLighting(
        ...
        for light in lights {
            switch light.type {
            case .ambient:
                let ambientContribution = (material.baseColor * light.color) * (light.intensity * 0.2)
                ambient = ambient + ambientContribution
                
            case .directional:
                if let lightDir = light.direction {
                    let lightIntensity = Swift.max(0, dotProduct(normalizedNormal, lightDir.normalized * -1))
                    let diffuseContribution = (material.baseColor * light.color) * (light.intensity * lightIntensity)
                    diffuse = diffuse + diffuseContribution
                    
                    // Specular term
                    let reflectDir = reflect(lightDir.normalized * -1, normal: normalizedNormal)
                    let specIntensity = pow(Swift.max(0, dotProduct(normalizedViewDir, reflectDir)), 32.0)
            ...
            case .point:
                if let lightPos = light.position {
                    ...
                    let attenuation = light.range.map { Swift.max(0, 1 - distance / $0) } ?? 1.0
                    let lightIntensity = Swift.max(0, dotProduct(normalizedNormal, lightDir.normalized)) * attenuation
                    ...
                    let reflectDir = reflect(lightDir.normalized * -1, normal: normalizedNormal)
                    let specIntensity = pow(Swift.max(0, dotProduct(normalizedViewDir, reflectDir)), 32.0)
                    specular = specular + ((light.color * (light.intensity * specIntensity * attenuation)) * material.metallic)
                }
            ...
            case .spot:
                if let lightPos = light.position, let lightDir = light.direction {
                    ...
                    let innerCone = light.innerConeAngle ?? Foundation.cos(0.1)
                    let outerCone = light.outerConeAngle ?? Foundation.cos(0.3)
                    
                    let spotIntensity = smoothstep(innerCone, outerCone, spotEffect)
                    let attenuation = light.range.map { Swift.max(0, 1 - distance / $0) } ?? 1.0
                    let lightIntensity = Swift.max(0, dotProduct(normalizedNormal, toLight.normalized)) * spotIntensity * attenuation
                    ...
                    let specIntensity = pow(Swift.max(0, dotProduct(normalizedViewDir, reflectDir)), 32.0)
                    specular = specular + ((light.color * (light.intensity * specIntensity * spotIntensity * attenuation)) * material.metallic)
                }
            }
        }
