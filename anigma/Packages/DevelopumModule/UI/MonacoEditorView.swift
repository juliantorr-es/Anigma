...
public struct MonacoEditorView: PlatformViewRepresentable {
    ...
    private func makeWebView(context: Context) -> WKWebView {
        ...
        // Load Monaco editor HTML from package resources when available.
        let candidateBundles: [Bundle] = [
            Bundle(for: Coordinator.self),
            .main
        ]

        if let monacoHtmlPath = candidateBundles.compactMap({ $0.path(forResource: "monaco-editor", ofType: "html") }).first {
