...
public final class TextChunkingCapsuleWrapper {
    ...
    private enum BoundaryMetric {
        static let create = "capsule.textchunking.create"
        static let processBytes = "capsule.textchunking.process_bytes"
        static let finalize = "capsule.textchunking.finalize"
        static let reset = "capsule.textchunking.reset"
        static let boundaryCount = "capsule.textchunking.get_boundary_count"
        static let chunkInfo = "capsule.textchunking.get_chunk_info"
        static let destroy = "capsule.textchunking.destroy"
    }
    ...
    public init(
        ...
        guard status == ANIGMA_OK, let finalHandle = rawHandle else {
            resolvedDiagnostics.event(
                ...
                correlationID: nil,
                metadata: ["algorithm_version": Self.algorithmVersion]
            )
    ...
    public func processBytes(_ data: Data) throws {
        ...
        try handle.withHandle { rawHandle in
            CrossLanguageCallMetrics.record(boundary: BoundaryMetric.processBytes)
    ...
    public func finalize() throws {
        ...
        var error = anigma_capsule_error_t()
        try handle.withHandle { rawHandle in
            CrossLanguageCallMetrics.record(boundary: BoundaryMetric.finalize)
    ...
    public func reset() throws {
        ...
        var error = anigma_capsule_error_t()
        try handle.withHandle { rawHandle in
            CrossLanguageCallMetrics.record(boundary: BoundaryMetric.reset)
    ...
    public func getBoundaries() throws -> [anigma_chunk_boundary_t] {
        ...
        do {
            ...
            var count: Int = 0
            CrossLanguageCallMetrics.record(boundary: BoundaryMetric.boundaryCount)
            ...
            var actual: Int = 0
            CrossLanguageCallMetrics.record(boundary: BoundaryMetric.chunkInfo)
...
private func capsuleDestroyer(
    ...
    { ptr in
        CrossLanguageCallMetrics.record(boundary: "capsule.textchunking.destroy")
