...
public class MLWorker {
    ...
    public func performTask(_ request: MLWorkerRequest) throws -> MLWorkerResponse {
        ...
        let modelHash = (try? computeFileHash(at: URL(fileURLWithPath: modelPath))) ?? "blake3:unknown"
    ...
    private func computeFileHash(at url: URL) throws -> String {
        guard FileManager.default.fileExists(atPath: url.path) else { return "blake3:missing" }
            ...
            while let fileURL = enumerator?.nextObject() as? URL {
                if let values = try? fileURL.resourceValues(forKeys: [.isRegularFileKey]), values.isRegularFile == true {
                    if let data = try? Data(contentsOf: fileURL, options: .mappedIfSafe) {
                        hashes.append(BLAKE3Digest.hex(of: data))
                    }
                }
            }
            let combined = hashes.sorted().joined()
            return "blake3:\(BLAKE3Digest.hex(of: Data(combined.utf8)))"
        } else {
            let data = try Data(contentsOf: url, options: .mappedIfSafe)
            return "blake3:\(BLAKE3Digest.hex(of: data))"
        }
    }
}
