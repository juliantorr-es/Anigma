...
public final class MigrationTraceReporter {
    ...
    /// 
    /// This is allowed for CLI tools which are approved composition roots.
    /// For consistency, prefer using `init(database:)` with a DatabaseExecutor from the CLI's composition root.
    /// 
