...
struct GC: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        let policy: DatabaseCore.RetentionPolicy
        if let policyFile = policyFile {
            let policyUrl = URL(fileURLWithPath: policyFile)
            policy = try DatabaseCore.RetentionPolicy.load(from: policyUrl)
            print("📋 Loaded retention policy from: \(policyFile)")
        } else {
            policy = DatabaseCore.RetentionPolicy.production
            print("📋 Using production retention policy")
        }

        // Initialize database
        let dbPath = getDatabasePath()
        let masterDb = DatabaseActor(dbPath: dbPath)
