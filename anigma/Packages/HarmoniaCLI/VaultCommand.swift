...
struct VaultStatus: AsyncParsableCommand {
    ...
    func run() async throws {
        let db: any DatabaseExecutor = DatabaseActor(path: resolveDatabasePath(options.dbPath))
        try await db.open()
...
struct VaultVerify: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        for artifact in slice {
            do {
                _ = try await vault.open(hash: artifact.hashHex)
                verified += 1
            } catch {
                failures.append(
                    VaultVerifyFailure(hash: artifact.hashHex, error: error.localizedDescription))
            }
        }
...
private func loadPolicy(path: String?) throws -> ContractsCore.RetentionPolicy {
    if let path, !path.isEmpty {
        return try ContractsCore.RetentionPolicy.load(from: URL(fileURLWithPath: path))
    }
    return ContractsCore.RetentionPolicy.production
}
