        ...
        for (index, (intent, report)) in results.enumerated() {
            ...
            case .runTask:
                if let report = report {
                    ...
                    print("      • Health: \(String(format: "%.2f", report.metrics.healthScore))")
    ...
    public func run() async throws {
        ...
        if !filesWithWork.isEmpty {
            ...
            for file in filesWithWork.prefix(detailed ? 20 : 5) {
                print("• \(file.path):")
                print("  - Findings: \(file.findings.count)")
                ...
                if detailed {
                    for task in file.tasks.filter({ $0.status == .pending }).prefix(3) {
                        print("  - Task: \(task.id.uuidString.prefix(8)) (\(task.status))")
                    }
                    if file.tasks.filter({ $0.status == .pending }).count > 3 {
                        print(
                            "  - ... and \(file.tasks.filter { $0.status == .pending }.count - 3) more tasks"
                        )
                    }
                }
            }
        ...
        if !taintedFiles.isEmpty {
            ...
            for file in taintedFiles.prefix(5) {
                print("• \(file.path)")
                if let lastOutcome = file.lastOutcome {
                    print(
                        "  - Last session: \(lastOutcome.sessionIndex) (\(String(format: "%.2f", lastOutcome.healthScore)))"
                    )
                }
            }
        ...
        if detailed {
            ...
            print("---------------------------")
            let engine = BasicSwift6StepEngine()
    ...
    public func run() async throws {
        ...
        if !scenesWithWork.isEmpty {
            ...
            for scene in scenesWithWork.prefix(detailed ? 20 : 5) {
                ...
                print("  - Tests: \(scene.tests.count) total, \(scene.failingTestCount) failing")
                ...
                if detailed {
                    for test in scene.tests.filter({ $0.status == .failed || $0.status == .broken })
                        .prefix(3) {
                        print("  - Test: \(test.name) (\(test.status))")
                    }
                    for task in scene.tasks.filter({ $0.status == .pending }).prefix(3) {
                        print("  - Task: \(task.title) (\(task.status))")
                    }
                    if scene.tasks.filter({ $0.status == .pending }).count > 3 {
                        print(
                            "  - ... and \(scene.tasks.filter { $0.status == .pending }.count - 3) more tasks"
                        )
                    }
                }
            }
        ...
        if !taintedScenes.isEmpty {
            ...
            for scene in taintedScenes.prefix(5) {
                print("• \(scene.name) (\(scene.id))")
                if let lastOutcome = scene.lastOutcome {
                    print(
                        "  - Last playtest: session \(lastOutcome.sessionIndex) (\(String(format: "%.2f", lastOutcome.healthScore)))"
                    )
                }
            }
        ...
        if detailed {
            print("\n🔧 Engine would choose from:")
            print("---------------------------")
            let engine = BasicGameStepEngine()
            let policy = GameProjectPolicy.default
            let intent = engine.chooseNextStep(from: state, policy: policy)
            print("• Next intent: \(intent.description)")
        }
    }
