...
public struct DiagnosticRedactionRules {
    ...
    /// Whitelist of keys that are never redacted
    ...
    ///   - deepClean: If true, also redacts hex keys and base64 blobs
    ...
    public static func sanitize(_ string: String, deepClean: Bool = false) -> String {
        ...
        // Apply deep clean patterns if requested
        if deepClean {
    ...
    /// Sanitize a dictionary by redacting sensitive values for keys not in the whitelist
    /// - Parameters:
    ///   - metadata: The dictionary to sanitize
    ///   - deepClean: If true, also redacts hex keys and base64 blobs
    /// - Returns: A sanitized dictionary
    public static func sanitizeMetadata(_ metadata: [String: String], deepClean: Bool = false) -> [String: String] {
        var sanitized: [String: String] = [:]
        
        for (key, value) in metadata {
            let normalizedKey = key.lowercased()
            if safeKeys.contains(normalizedKey) {
                sanitized[key] = value
            } else {
    ...
    public static func containsSensitiveData(_ string: String, deepClean: Bool = false) -> Bool {
        ...
        if deepClean {
            for (pattern, _) in deepCleanPatterns {
                if pattern.firstMatch(in: string, options: [], range: range) != nil {
                    return true
                }
            }
    ...
    public static func detectPatterns(_ string: String, deepClean: Bool = false) -> [(String, String)] {
        ...
        var patternsToCheck = patterns
        if deepClean {
            patternNames.append(contentsOf: ["HEX_KEY", "BASE64_BLOB"])
            patternsToCheck.append(contentsOf: deepCleanPatterns)
        }
        
        for (index, (pattern, _)) in patternsToCheck.enumerated() {
