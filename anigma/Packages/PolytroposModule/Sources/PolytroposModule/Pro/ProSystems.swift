...
public struct ColorGradingSystem: System {
    ...
    public func update(world: World) async {
        ...
        let timelines = await world.query(MultiTrackTimelineComponent.self, TimelineComponent.self)
...
public struct AudioProcessingSystem: System {
    ...
    public func update(world: World) async {
        ...
        let timelines = await world.query(MultiTrackTimelineComponent.self, AudioDuckingComponent.self)
...
public struct ProxyGenerationSystem: System {
    ...
    public func update(world: World) async {
        ...
        let projects = await world.query(ProxySettingsComponent.self)
...
public struct MediaRelinkSystem: System {
    ...
    public func update(world: World) async {
        ...
        let projects = await world.query(MediaRelinkComponent.self)
...
public struct ScopesAnalysisSystem: System {
    ...
    public func update(world: World) async {
        ...
        let timelines = await world.query(ScopesComponent.self, MultiTrackTimelineComponent.self)
...
public struct ExportQueueSystem: System {
    ...
    public func update(world: World) async {
        ...
        let jobs = await world.query(ProExportJobComponent.self)

        let queuedJobs = jobs.filter { $0.1.status == .queued }
        let activeStatuses: Set<ProExportStatus> = [.preparing, .rendering, .encoding]
        let activeJobs = jobs.filter { activeStatuses.contains($0.1.status) }
...
public struct EditHistorySystem: System {
    ...
    public func update(world: World) async {
        ...
        let timelines = await world.query(EditHistoryComponent.self)
...
public struct ProAudioAnalysisSystem: System {
    ...
    public func update(world: World) async {
        ...
        let assets = await world.query(MediaAssetComponent.self)

        for (entity, asset) in assets {
            if await world.hasComponent(entity, AssetAudioAnalysisComponent.self) {
                continue
            }
