...
public actor VideoIngestionService {
    ...
    /// Media fingerprint capsule for perceptual hashing.
    private let fingerprintCapsule: MediaFingerprintCapsule?
    ...
    public init(
        ...
        fingerprintCapsule: MediaFingerprintCapsule? = nil,
        workDirectory: URL? = nil
    ) {
        self.configuration = configuration
        self.containerCapsule = containerCapsule
        self.fingerprintCapsule = fingerprintCapsule
        ...
        for stream in report.streams {
            ...
            case .video:
                if let videoInfo = try? capsule.getVideoStreamInfo(at: stream.index) {
                    ...
                    // Convert VideoCodec from MediaContainerCapsule to the expected type
                    let mappedCodec = MediaContainerCapsule.VideoCodec(rawValue: videoInfo.codec.rawValue) ?? .unknown
                    codec = mapVideoCodec(mappedCodec)
    ...
    private func extractKeyFrames(
        ...
        while currentTime < metadata.duration && fingerprints.count < maxFrames {
            ...
            // Extract actual frame data using AVAssetImageGenerator
            if let imageRef = try? generator.copyCGImage(at: time, actualTime: nil) {
                ...
                if let tiffData = image.tiffRepresentation,
                   ...
                   let result = try? fingerprintCapsule?.generateImageFingerprint(jpegData) {
                    pHash = result.hashData.map { String(format: "%02x", $0) }.joined()
                }
            }
