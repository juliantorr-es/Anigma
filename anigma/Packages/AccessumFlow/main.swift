...
struct AccessumFlow: AsyncParsableCommand {
    ...
    private static func runDiaplasionStep(
        specURL: URL,
        specSource: SpecSource,
        repoRoot: URL,
        pipelineVersion: String,
        inputContext: InputHashContext,
        artifactDir: URL,
        env: [String: String]
    ) throws -> (step: AccessumStep, trace: DiaplasionTrace, artifactDir: URL) {
        let batchStart = Date()
        let binPath = try buildProduct("diaplasion-pipeline", repoRoot: repoRoot, env: env)
        let executable = binPath.appendingPathComponent("diaplasion-pipeline")
        let outputBase = repoRoot.appendingPathComponent("Artifacts/diaplasion")
        let args = [
            "--spec", specURL.path,
        ...
        let artifacts = [
            ...
            "trace": pipelineTraceURL.path
        ]
        let hashes = [
            "plainTextHash": trace.plainTextHash
        ]
        ...
        return (
            ...
            trace: trace,
            artifactDir: pipelineArtifactDir
        )
    }

    private static func runMLStep(
        runId: String,
        repoRoot: URL,
        artifactDir: URL,
        pipelineVersion: String,
        diTrace: DiaplasionTrace,
        ...
        let plainTextPath = pipelineArtifactDir.appendingPathComponent("plain.txt").path
        artifacts["input"] = plainTextPath
        hashes["plainTextHash"] = diTrace.plainTextHash
        _ = repoRoot
        _ = pipelineVersion
        _ = env
        ...
        return AccessumStep(
            ...
            command: "ml-worker --engine mlx embed <skipped>",
            status: "completed",
            exitCode: 0,
            durationMs: 0,
            artifacts: artifacts,
            hashes: hashes,
            detail: "ML worker step deferred: compile-compatible placeholder"
        )
    }
    ...
    private static func runOutlineumStep(
        ...
        let pdfHash = try blake3Hex(of: pdfPath)
        let hashes = [
            "pdfHash": pdfHash
        ]
    ...
    private static let canonicalOutlineumSpecRelativePath = "Packages/OutlineumModule/TestFiles/zine-minimal/spec.json"
    private static let outlineumSpecOverrideEnvironmentKey = "ANIGMA_OUTLINEUM_SPEC"

    private static func outlineumSpecURL(repoRoot: URL) throws -> URL {
        let envOverride = ProcessInfo.processInfo.environment[Self.outlineumSpecOverrideEnvironmentKey]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let sourcePath: String
        if let envOverride, !envOverride.isEmpty {
            sourcePath = envOverride
        } else {
            sourcePath = Self.canonicalOutlineumSpecRelativePath
        }

        let resolved = URL(fileURLWithPath: sourcePath, relativeTo: repoRoot).standardizedFileURL
        guard FileManager.default.fileExists(atPath: resolved.path) else {
            throw AccessumError.missingOutlineumSpec(
                path: resolved.path,
                envKey: Self.outlineumSpecOverrideEnvironmentKey
            )
        }
        return resolved
    }

    private static func blake3Hex(of fileURL: URL) throws -> String {
...
extension AccessumFlow {
    struct Admin: AsyncParsableCommand {
        ...
        @Option(
            ...
            help: "Path to the migration trace database (default: harmonia_harness.db)"
        )
        ...
        func run() async throws {
            ...
            if let taskId = migrationTaskId {
                let reporter = MigrationTraceReporter(dbPath: migrationDbPath ?? "harmonia_harness.db")
...
actor AccessumRunStore {
    ...
    init(dbURL: URL) {
        self.database = DatabaseActor(path: dbURL.path)
...
private enum AccessumError: Error, LocalizedError {
    ...
    case missingOutlineumSpec(path: String, envKey: String)
    ...
    var errorDescription: String? {
        ...
        case .missingOutlineumSpec(let path, let envKey):
            return "Outlineum spec missing at \(path). Set \(envKey) to override the canonical default path."
