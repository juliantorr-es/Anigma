...
public actor MLBackendCoordinator {
    ...
    private var lastQuantizedSelection: QuantizedModelSelectionPolicy.Decision?
    ...
    private func chatWithBackend(
        ...
        switch backend {
        case .mlx:
            // Use ml-worker via MLWorkerService with deterministic quantized model policy.
            let decision = resolveQuantizedMLXModelSelection(
                prompt: prompt,
                systemPrompt: systemPrompt,
                maxTokens: maxTokens
            )
            lastQuantizedSelection = decision
            // Note: MLWorkerService handles the execution
            return try await MLWorkerService.shared.chat(
                modelPath: decision.modelID,
                prompt: prompt,
                maxTokens: maxTokens,
    ...
    /// Validation hook to inspect deterministic quantized model selection.
    public func previewQuantizedSelection(
        prompt: String,
        systemPrompt: String? = nil,
        maxTokens: Int = 1024
    ) -> QuantizedModelSelectionPolicy.Decision {
        resolveQuantizedMLXModelSelection(
            prompt: prompt,
            systemPrompt: systemPrompt,
            maxTokens: maxTokens
        )
    ...
    public func latestQuantizedSelection() -> QuantizedModelSelectionPolicy.Decision? {
        lastQuantizedSelection
    }
    ...
    private func isBackendAvailable(_ backend: ChatBackend) -> Bool {
        switch backend {
        case .mlx: return MLWorkerService.isWorkerBinaryAvailable()
    ...
    private func resolveQuantizedMLXModelSelection(
        prompt: String,
        systemPrompt: String?,
        maxTokens: Int
    ) -> QuantizedModelSelectionPolicy.Decision {
        let env = ProcessInfo.processInfo.environment
        let profile = QuantizedModelSelectionPolicy.Profile(
            rawValue: env["ANIGMA_MODEL_PROFILE"]?.lowercased() ?? ""
        ) ?? .balanced
        let explicitOverride = env["ANIGMA_LOCAL_CHAT_MODEL"] ?? env["MLX_MODEL_ID_CHAT"] ?? env["MLX_MODEL_ID"]
        let combinedPrompt = (systemPrompt ?? "") + prompt

        return QuantizedModelSelectionPolicy.decide(
            context: .init(
                promptCharacters: combinedPrompt.count,
                maxTokens: maxTokens,
                physicalMemoryBytes: ProcessInfo.processInfo.physicalMemory,
                profile: profile,
                isAppleSilicon: isAppleSilicon(),
                explicitModelOverride: explicitOverride
            )
        )
    }
