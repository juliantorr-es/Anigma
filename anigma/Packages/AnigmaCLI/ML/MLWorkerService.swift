...
public actor MLWorkerService {
    ...
    public nonisolated static func isWorkerBinaryAvailable() -> Bool {
        resolveWorkerPath() != nil
    }
    ...
    public func chat(
        ...
        let response = try await runWorker(
            request: request,
    ...
    private func runWorker(
        ...
        onProgress: (@Sendable (String) -> Void)? = nil
    ) async throws -> MLWorkerCommon.MLWorkerResponse {
        guard let workerPath = Self.resolveWorkerPath() else {
        ...
        if request.engine == .mlx {
            env["MLX_MODEL_ID_CHAT"] = modelOverride
            env["MLX_MODEL_ID"] = modelOverride
        } else if request.engine == .llama {
            env["LLAMA_MODEL_PATH"] = modelOverride
        }
    ...
    private nonisolated static func resolveWorkerPath() -> String? {
