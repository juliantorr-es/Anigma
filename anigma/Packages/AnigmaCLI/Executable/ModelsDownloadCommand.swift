...
struct ModelsDownloadCommand: AsyncParsableCommand {
    ...
    private func registerDownloadedModel(
        ...
        let backendKind = try backend.map { raw -> ContractsCore.MLBackend in
            guard let kind = ContractsCore.MLBackend(rawValue: raw) else {
    ...
    private func inferBackend(installPath: String) -> ContractsCore.MLBackend {
