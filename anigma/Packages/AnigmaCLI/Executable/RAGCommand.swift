...
struct IndexRAG: AsyncParsableCommand {
    ...
    var db: String = DatabaseConfiguration.defaultDatabasePath()
...
struct StatsRAG: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        let store = try await CLIVectorStore(dbPath: db)
        let stats = try await store.statistics()

        print("   Total chunks: \(stats.totalChunks)")
        print("   Indexed files: \(stats.indexedFiles)")
        print("   Languages:")
        for (lang, count) in stats.languages.sorted(by: { $0.value > $1.value }) {
            print("     - \(lang): \(count)")
        }
