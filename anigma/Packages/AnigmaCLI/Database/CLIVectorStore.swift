...
/// Vector storage using sqlite-vec extension for CLI
public actor CLIVectorStore {
    private var dbHandle: DBHandle?
    private var db: OpaquePointer? { dbHandle?.pointer }
    private let dbPath: String
    private let dimension: Int
    
    // MARK: - Static Extension Registration
    ...
    public static func registerExtension(with db: UnsafeMutableRawPointer) throws {
        // Load vec0 extension - this is a no-op if already loaded or statically linked
        let dbPtr = OpaquePointer(db)
        var error: UnsafeMutablePointer<CChar>?
        sqlite3_exec(dbPtr, "SELECT load_extension('vec0')", nil, nil, &error)
        if let error = error {
            let msg = String(cString: error)
            sqlite3_free(error)
            // Don't throw - extension may already be loaded or statically linked
            _ = msg
        }
    }
