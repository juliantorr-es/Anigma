...
/// CoreReceipt manager for generating and verifying cryptographic receipts.
public actor CLIReceiptManager {
    ...
    // MARK: - CoreReceipt Retrieval

    /// List receipts for a run.
    public func listReceipts(runID: String) async throws -> [CoreReceipt] {
    ...
    public func getReceipt(receiptID: String) async throws -> CoreReceipt? {
        let rows = try await db.query("""
    ...
    public func verifyChain(runID: String) async throws -> ChainVerification {
        ...
        for receipt in receipts {
            if receipt.type != .runComplete, receipt.requestHash == nil {
                errors.append("CoreReceipt \(receipt.receiptID): missing request hash")
            }
        }
        ...
        for i in 1..<receipts.count {
            if receipts[i].timestamp < receipts[i - 1].timestamp {
                errors.append("CoreReceipt \(receipts[i].receiptID): timestamp out of order")
            }
        }
    ...
    private func decodeReceipt(_ row: CLIRow) throws -> CoreReceipt {
...
public struct CoreReceipt: Sendable {
