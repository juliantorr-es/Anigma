...
public actor GarbageCollectorEngine {
    ...
    public init(database: DatabaseActor, policyManager: RetentionPolicyManager) {
        self.db = database
    ...
    private func identifyEligibleArtifacts(
        ...
        _ = try await db.makeContentAddressedStore()
        let ledger = try await db.makeMasterLedgerStore()
