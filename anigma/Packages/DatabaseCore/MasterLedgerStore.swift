...
public actor MasterLedgerStore {
    ...
    private let segmentationManager: MasterLedgerSegmentationManager
    private var searchManager: SemanticSearchManager?
    private var eventIndexer: LedgerEventIndexer?

    public init(
        database: any DatabaseExecutor,
        searchBackend: SemanticSearchBackend? = nil
    ) async throws {
        self.db = database
        self.segmentationManager = MasterLedgerSegmentationManager(database: database)
    ...
    public func recordToolCallWithVerification(
        toolName: String,
        sessionID: String,
        status: String,
        ...
        // Compute hashes
        let preconditionHash = preconditionData.map { ContentHashing.computeHash($0) }
        
        var resultContentHash: String?
        if let content = resultContent {
            resultContentHash = ContentHashing.computeHash(content)
        }

        // Record the event
        return try await recordToolCallEvent(
            toolName: toolName,
            sessionID: sessionID,
            status: status,
    ...
    private func setupSchema() async throws {
        ...
        try await segmentationManager.initializeSchema()
    ...
    public func recordToolCallEvent(
        ...
        try await segmentationManager.writeEventToSegment(ledgerEvent)
    ...
    public func eventReferencesContent(contentHash: String) async throws -> Bool {
        ...
        do {
            let crossSegmentQuery = CrossSegmentQueryExecutor(database: db, segmentManager: segmentationManager)
    ...
    public func getSegmentationStats() async throws -> LedgerSegmentationStats? {
        return try await segmentationManager.getSegmentationStats()
    }

    /// Perform migration to segmented ledger if needed.
    public func migrateToSegmentation() async throws -> MigrationResult {
        return try await segmentationManager.migrateToSegmentation()
    }
