...
public actor ContentAddressedStore {
    ...
    public func storeContent<T: ContentAddressable>(
        _ content: T,
        parentKey: String,
        parentType: String,
        metadata: Data? = nil
        ...
        let storedArtifact = try await storeArtifact(artifact)
        ...
        _ = try await createReference(reference)
    ...
    public func storeContent(
        ...
        let hash = ContentHashing.computeHash(data)
    ...
    public func getContent(byHash hash: String) async throws -> Data? {
        guard let artifact = try await getArtifact(byHash: hash) else {
            return nil
        }
    ...
    public func hasContent(withHash hash: String) async throws -> Bool {
        let artifact = try await getArtifact(byHash: hash)
        return artifact != nil
    }
