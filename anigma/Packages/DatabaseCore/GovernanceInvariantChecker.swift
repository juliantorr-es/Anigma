...
public actor GovernanceInvariantChecker {
    ...
    private func verifyContentAddressedIntegrity() async throws -> (orphaned: Int, missing: Int) {
        let orphanedRows = try await db.query("""
            SELECT COUNT(*) as count FROM artifact_references
            WHERE content_hash NOT IN (SELECT content_hash FROM content_addressed_artifacts)
            AND deleted_at IS NULL
        """)
        let orphanedCount = orphanedRows.first?.int(for: "count") ?? 0

        let missingRows = try await db.query("""
            SELECT COUNT(*) as count FROM content_addressed_artifacts
            WHERE reference_count = 0
        """)
        let missingCount = missingRows.first?.int(for: "count") ?? 0

        return (orphaned: orphanedCount, missing: missingCount)
    }
