...
public actor VaultAuthority {
    ...
    public func ingest(
        ...
        let objectURL = try layout.objectURL(forSHA256Hex: hash)
    ...
    public func quarantine(data: Data, actorId: String? = nil) async throws -> QuarantineTicket {
        ...
        return QuarantineTicket(
            ticketId: ticketId, sha256Hex: hash, byteLen: data.count, createdAt: now)
    }
    ...
    public func promote(
        ...
        guard fileManager.fileExists(atPath: metadataURL.path),
            ...
            try await recordAccess(
                action: "promote", hash: ticket.sha256Hex, decision: .deny,
        ...
        let plaintext = try VaultCrypto.decrypt(
            envelope: encrypted, hashHex: metadata.sha256Hex, keyMaterial: keyMaterial)
        if sha256Hex(plaintext) != metadata.sha256Hex {
            try await recordAccess(
                action: "promote", hash: metadata.sha256Hex, decision: .deny,
                reason: "integrity_mismatch", actorId: actorId)
            throw VaultError.integrityMismatch
        }

        let objectURL = try layout.objectURL(forSHA256Hex: metadata.sha256Hex)
        if !fileManager.fileExists(atPath: objectURL.path) {
        ...
        try await recordAccess(
            action: "promote", hash: metadata.sha256Hex, decision: .allow, reason: nil,
            actorId: actorId)
        return ref
    }
    ...
    public func exportBundle(
        ...
        for hash in request.hashes {
            ...
            let plaintext = try await open(hash: record.sha256Hex, actorId: actorId)
            let plaintextHash = sha256Hex(plaintext)
            let envelopeData = try Data(contentsOf: objectURL)
            let envelopeHash = sha256Hex(envelopeData)
            let destinationURL = bundleURL.appendingPathComponent(
                "\(record.sha256Hex).enc", isDirectory: false)
    ...
    public func gc(
        policy: ContractsCore.RetentionPolicy,
        dryRun: Bool,
        actorId: String? = nil
        ...
        for artifact in allArtifacts {
            ...
            if !dryRun {
                ...
                try await indexStore.deleteArtifact(hash: artifact.sha256Hex)
                try await recordAccess(
                    action: "gc", hash: artifact.sha256Hex, decision: .allow, reason: nil,
                    actorId: actorId)
            }
            deletedHashes.append(artifact.sha256Hex)
        }
    ...
    private func recordAccess(
        ...
        let receipt = VaultReceipt(
            ...
            decision: decision,
            reason: reason,
            sha256Hex: hash.isEmpty ? nil : hash,
            actorId: actorId,
            occurredAt: clock()
        )
        ...
        try await indexStore.recordAccess(
            ...
            actorId: actorId,
            action: action,
            sha256Hex: hash.isEmpty ? "unknown" : hash,
            decision: decision,
            reason: reason
        )
        return receipt
