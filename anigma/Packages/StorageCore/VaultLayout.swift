...
public struct VaultLayout: Sendable {
    ...
    /// Returns the on-disk path for a BLAKE3 hex hash.
    /// Renamed from forSHA256Hex to support Saturated Architecture migration.
    public func objectURL(forHash hash: String) throws -> URL {
        guard hash.count >= 4 else { throw VaultError.invalidHash }
        let prefix = String(hash.prefix(2))
        let next = String(hash.dropFirst(2).prefix(2))
        return objectsURL
            .appendingPathComponent("blake3", isDirectory: true)
