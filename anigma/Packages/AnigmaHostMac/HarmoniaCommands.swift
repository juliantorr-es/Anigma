...
extension HarmoniaClient {
    ...
    public func daemonStatus() async throws -> DaemonStatusResponse {
        return try await execute<DaemonStatusResponse>(["daemon", "status", "--format", "json"])
    }
}
...
extension HarmoniaClient {
    ...
    public func vaultStatus() async throws -> VaultStatusResponse {
        return try await execute<VaultStatusResponse>(["vault", "status", "--format", "json"])
    }

    /// Verify vault integrity.
    public func vaultVerify() async throws -> VaultVerificationResponse {
        return try await execute<VaultVerificationResponse>(["vault", "verify", "--format", "json"])
    }
...
extension HarmoniaClient {
    ...
    public struct TechDebtAuditResponse: Codable {
        ...
        public init(totalIssues: Int, criticalIssues: Int, highIssues: Int, mediumIssues: Int, lowIssues: Int, categories: [String: Int]) {
    ...
    /// Run tech debt audit with latency measurement.
    public func techDebtAudit(path: String) async throws -> (response: TechDebtAuditResponse, latency: TimeInterval) {
        let startTime = DispatchTime.now()
        let response: TechDebtAuditResponse = try await execute(["techdebt", "audit", path, "--format", "json"])
        let endTime = DispatchTime.now()
        
        let nanoTime = endTime.uptimeNanoseconds - startTime.uptimeNanoseconds
        let latency = TimeInterval(nanoTime) / 1_000_000_000
        
        return (response, latency)
    }

    /// Run tech debt audit (backward compatible version without latency).
    @available(*, deprecated, message: "Use techDebtAudit(path:) with latency measurement instead")
    public func techDebtAuditLegacy(path: String) async throws -> TechDebtAuditResponse {
        let result = try await techDebtAudit(path: path)
...
extension HarmoniaClient {
    ...
    public struct SearchResult: Codable {
        ...
        public init(id: String, type: String, timestamp: Date = Date(), relevance: Double = 1.0, preview: String = "") {
            self.id = id
            self.type = type
    ...
    public struct SearchResponse: Codable {
        ...
        public init(query: String, results: [SearchResult] = [], totalMatches: Int = 0) {
            self.query = query
    ...
    public func search(query: String, limit: Int = 10) async throws -> SearchResponse {
        return try await execute<SearchResponse>(["search", query, "--limit", "\(limit)", "--format", "json"])
    }
}
...
extension HarmoniaClient {
    ...
    public func toolStatus(toolName: String) async throws -> ToolStatusResponse {
        return try await execute<ToolStatusResponse>(["tool", "status", toolName, "--format", "json"])
    }
...
extension HarmoniaClient {
    ...
    public func stackStatus() async throws -> StackStatusResponse {
        return try await execute<StackStatusResponse>(["stack", "status", "--format", "json"])
    }
