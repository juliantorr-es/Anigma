...
typedef struct anigma_color_transform_s* anigma_color_transform_t;

/**
 * Create a color transform from source profile to dest profile.
 */
anigma_result_t anigma_color_transform_create(
    anigma_ctx_t* ctx,
...
anigma_result_t anigma_color_transform_destroy(anigma_color_transform_t handle);

/**
 * Apply transform to pixel buffer in-place.
 */
anigma_result_t anigma_color_transform_apply(
