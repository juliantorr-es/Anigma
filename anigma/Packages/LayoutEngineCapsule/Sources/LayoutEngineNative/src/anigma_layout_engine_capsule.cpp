...
static void detectHeaders(const std::vector<anigma_text_segment_t>& segments,
    ...
    for (const auto& seg : segments) {
        ...
        if (row >= 0 && col >= 0) {
            // PDFium font flags: 1 << 0 is fixed-pitch, 1 << 1 is serif, 1 << 2 is symbolic,
            // 1 << 3 is script, 1 << 5 is nonsymbolic, 1 << 6 is italic, 1 << 18 is allcap,
            // 1 << 19 is smallcap, 1 << 20 is forcebold.
            // Using the 'ForceBold' flag (bit 18 is often used for bold in some PDF variants, 
            // but bit 262144 / 1 << 18 is standard in PDF spec for font descriptors).
            // However, the internal layout engine flag mapping uses:
            const uint32_t PDF_FONT_BOLD = 1 << 18; 
            if (seg.font_flags & PDF_FONT_BOLD) {
                headerRows[row] = true;
...
anigma_status_t anigma_layout_engine_capsule_analyze_pdf(
    ...
    for (size_t i = 0; i < pages_to_copy; ++i) {
        ...
        // Deep copy table bounding boxes
        if (src.table_count > 0 && src.table_bboxes) {
            dst.table_bboxes = static_cast<anigma_bounding_box_t*>(malloc(sizeof(anigma_bounding_box_t) * src.table_count));
            if (dst.table_bboxes) {
                memcpy(dst.table_bboxes, src.table_bboxes, sizeof(anigma_bounding_box_t) * src.table_count);
            }
        } else {
            dst.table_bboxes = nullptr;
        }
        
        // Deep copy figure bounding boxes
        if (src.figure_count > 0 && src.figure_bboxes) {
            dst.figure_bboxes = static_cast<anigma_bounding_box_t*>(malloc(sizeof(anigma_bounding_box_t) * src.figure_count));
            if (dst.figure_bboxes) {
                memcpy(dst.figure_bboxes, src.figure_bboxes, sizeof(anigma_bounding_box_t) * src.figure_count);
            }
        } else {
