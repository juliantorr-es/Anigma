...
public actor QuickBooksConnector: ConnectorProtocol {
    ...
    // STUB_TRACK: quickbooks-write-back – Write-back operations not implemented for QuickBooks
    public func execute(intent: any GovernanceIntent) async throws -> Data {
        // STUB: Write-back operations are not implemented for QuickBooks connector
        print("⚠️  STUB INVOKED: QuickBooksConnector.execute() - write-back not implemented")
        print("   Intent type: \(type(of: intent))")
        print("   This connector only supports read operations (sync, sourceInventory)")
