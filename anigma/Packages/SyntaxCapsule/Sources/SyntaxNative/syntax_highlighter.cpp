...
static syntax_token_t* create_token(syntax_token_type_t type, syntax_language_t language, const char* text, uint32_t length, uint32_t line, uint32_t column) {
    syntax_token_t* token = (syntax_token_t*)malloc(sizeof(syntax_token_t));
...
int32_t syntax_parse(const char* input, uint32_t length, syntax_language_t language, syntax_parse_result_t** result) {
    ...
    // Create result
    syntax_parse_result_t* parse_result = (syntax_parse_result_t*)malloc(sizeof(syntax_parse_result_t));
    if (!parse_result) {
    ...
    while (pos < length) {
        ...
        if (isalpha(c) || c == '_') {
            ...
            char* word = (char*)malloc(pos - start + 1);
            if (word) {
    ...
    parse_result->metadata = (char*)malloc(strlen(metadata) + 1);
...
syntax_token_t* syntax_get_error_tokens(const syntax_parse_result_t* result) {
    ...
    while (token) {
        if (token->type == SYNTAX_TOKEN_ERROR) {
            syntax_token_t* error = (syntax_token_t*)malloc(sizeof(syntax_token_t));
            if (error) {
