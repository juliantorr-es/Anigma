...
internal final class GeometryCapsuleInternal: Sendable {
    ...
    /// Check if two meshes intersect using Octree
    /// - Parameters:
    ///   - mesh1: First mesh
    ///   - mesh2: Second mesh
    /// - Returns: True if meshes intersect
    internal func meshesIntersect(_ mesh1: TriangleMesh, _ mesh2: TriangleMesh) -> Bool {
        // 1. Quick bounding box check
        if !mesh1.boundingBox.intersects(mesh2.boundingBox) {
            return false
        }
        
        // 2. Build Octree for the smaller mesh (heuristic)
        let (smaller, larger) = mesh1.vertices.count < mesh2.vertices.count ? (mesh1, mesh2) : (mesh2, mesh1)
        let octree = Octree(mesh: smaller)
        
        // 3. Test triangles of larger mesh against Octree of smaller mesh
        for i in stride(from: 0, to: larger.indices.count, by: 3) {
            let v0 = larger.vertices[larger.indices[i]]
            let v1 = larger.vertices[larger.indices[i+1]]
            let v2 = larger.vertices[larger.indices[i+2]]
            let triangle = [v0, v1, v2]
            
            if octree.intersects(triangle: triangle) {
                return true
            }
        }
...
internal final class OctreeNode: @unchecked Sendable {
...
public final class Octree: Sendable {
    ...
    public func intersects(triangle: [Point3D]) -> Bool {
        return queryIntersection(triangle: triangle, node: root)
    }
    
    private func queryIntersection(triangle: [Point3D], node: OctreeNode) -> Bool {
        // 1. Check if triangle bounds intersects node bounds
        let triBounds = calculateTriangleBounds(triangle)
        if !node.bounds.intersects(triBounds) {
            return false
        }
        
        // 2. If leaf, check actual triangle-triangle intersection
        if node.isLeaf() {
            for idx in node.triangles {
                let nodeTri = getTriangle(index: idx)
                if trianglesIntersect(t1: triangle, t2: nodeTri) {
                    return true
                }
            }
            return false
        ...
        // 3. If internal node, recurse
        guard let children = node.children else { return false }
        for child in children {
            if queryIntersection(triangle: triangle, node: child) {
                return true
            }
        }
    ...
    private func calculateTriangleBounds(_ t: [Point3D]) -> BoundingBox3D {
        let minX = min(t[0].x, min(t[1].x, t[2].x))
        let minY = min(t[0].y, min(t[1].y, t[2].y))
        let minZ = min(t[0].z, min(t[1].z, t[2].z))
        
        let maxX = max(t[0].x, max(t[1].x, t[2].x))
        let maxY = max(t[0].y, max(t[1].y, t[2].y))
        let maxZ = max(t[0].z, max(t[1].z, t[2].z))
    ...
    private func getTriangle(index: Int) -> [Point3D] {
        let v1 = meshVertices[meshIndices[index]]
        let v2 = meshVertices[meshIndices[index+1]]
        let v3 = meshVertices[meshIndices[index+2]]
        return [v1, v2, v3]
    ...
    // Simplified robust intersection test using bounding boxes as fallback
    private func trianglesIntersect(t1: [Point3D], t2: [Point3D]) -> Bool {
        if !calculateTriangleBounds(t1).intersects(calculateTriangleBounds(t2)) {
            return false
        }
        return true 
    }
