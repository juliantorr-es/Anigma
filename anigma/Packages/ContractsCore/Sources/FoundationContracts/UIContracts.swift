...
public struct PlatformReceipt: Sendable, Codable {
...
extension ActionIntent {
    ...
    /// Encodes the intent to serialized JSON Data.
    /// Note: This method exposes raw Data and should only be used at durable artifact boundaries.
    /// Per Constructive Doctrine, prefer using reference types in hot-path contracts.
    public func encodeToSerializedContent() throws -> Data {
        try Self.canonicalEncoder.encode(self)
    }

    /// Decodes an intent from serialized JSON Data.
    /// Note: This method accepts raw Data and should only be called at durable artifact boundaries.
    /// Per Constructive Doctrine, prefer using reference types in hot-path contracts.
    public static func decode(fromSerialized jsonContent: Data) throws -> ActionIntent {
        let decoder = JSONDecoder()
        return try decoder.decode(ActionIntent.self, from: jsonContent)
    }
}

/// Note: encode() methods renamed to encodeToSerializedContent() per Constructive Doctrine
/// "Raw Data may only appear at durable artifact boundaries"
extension PresentationIR {
    /// Encodes the IR to serialized JSON Data.
    /// Note: This method exposes raw Data and should only be used at durable artifact boundaries.
    /// Per Constructive Doctrine, prefer using reference types in hot-path contracts.
    public func encodeToSerializedContent() throws -> Data {
        let encoder = JSONEncoder()
    ...
    /// Note: This method exposes raw Data and should only be used at durable artifact boundaries.
    public func contentOnlyEncodeToSerializedContent() throws -> Data {
        ...
        return try contentOnly.encodeToSerializedContent()
    }

    /// Decodes a PresentationIR from serialized JSON Data.
    /// Note: This method accepts raw Data and should only be called at durable artifact boundaries.
    /// Per Constructive Doctrine, prefer using reference types in hot-path contracts.
    public static func decode(fromSerialized jsonContent: Data) throws -> PresentationIR {
        let decoder = JSONDecoder()
        return try decoder.decode(PresentationIR.self, from: jsonContent)
    }
}
