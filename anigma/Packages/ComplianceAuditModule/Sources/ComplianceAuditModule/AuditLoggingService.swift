...
public actor AuditLoggingService {
    ...
    public init(database: DatabaseActor, configuration: AuditConfiguration = AuditConfiguration.default) {
    ...
    public func queryEvents(filters: AuditReportFilters) async throws -> [AuditEvent] {
        ...
        let results = try await database.fetchAuditEvents(query)
        return results.map { decodeAuditEvent($0) }
    }
    ...
    public func getStatistics(filters: AuditReportFilters?) async throws -> AuditReportSummary {
        ...
        let query = buildStatisticsQuery(activeFilters)
        let stats = try await database.fetchAuditStatistics(query)
        return decodeAuditStatistics(stats, filters: activeFilters)
    }
    ...
    private func storeDocumentAccessMetadata(_ event: DocumentAccessAuditEvent) async throws {
        ...
        let parameters: [DatabaseValue] = [
            ...
            .integer(event.fileSize.map(Int64.init)),
            .text(event.mimeType),
    ...
    private func storeGovernanceDecisionMetadata(_ event: GovernanceDecisionAuditEvent) async throws {
        ...
        let parameters: [DatabaseValue] = [
            ...
            .integer(event.automatedReview.map { $0 ? 1 : 0 })
        ]
    ...
    private func buildQuery(_ filters: AuditReportFilters) -> String {
        ...
        if let userIds = filters.userIds, !userIds.isEmpty {
            let placeholders = userIds.map { "?" }.joined(separator: ", ")
        ...
        if let sessionIds = filters.sessionIds, !sessionIds.isEmpty {
            let placeholders = sessionIds.map { "?" }.joined(separator: ", ")
        ...
        if let eventTypes = filters.eventTypes, !eventTypes.isEmpty {
            let placeholders = eventTypes.map { "?" }.joined(separator: ", ")
    ...
    private func buildStatisticsQuery(_ filters: AuditReportFilters) -> String {
        // Build a query that returns aggregate statistics
        let baseQuery = buildQuery(filters)
        return """
        SELECT 
    ...
    private func decodeAuditEvent(_ row: [String: DatabaseValue]) -> AuditEvent {
        // Implementation to decode database row to AuditEvent
        // This would need to handle the specific database value types
        fatalError("Implementation needed")
    }
    
    private func decodeAuditStatistics(_ stats: [[String: DatabaseValue]], filters: AuditReportFilters) -> AuditReportSummary {
        // Implementation to decode database statistics to AuditReportSummary
        // This would need to handle the specific database value types
        fatalError("Implementation needed")
    }
}
