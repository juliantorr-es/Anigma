...
public actor CathedralDatabasePersistence {
    ...
    public func persistQueryRecord(_ record: QueryRecord) async throws {
        ...
        let recordHash = record.id.blake3Hash
    ...
    // MARK: - Evidence Ring Persistence

    /// Persist evidence ring state
    public func persistRing(_ ring: EvidenceRing) async throws {
        let peaksData = try JSONEncoder().encode(ring.peaks)
        let metadataData = try JSONEncoder().encode(ring.metadata)
        
        try await database.executeAsync(
            """
            INSERT OR REPLACE INTO evidence_rings (
                ring_id, peaks_json, root_hash, ring_size,
                hashing_policy, updated_at, metadata_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            parameters: [
                .text(ring.ringId),
                .text(String(data: peaksData, encoding: .utf8) ?? "[]"),
                .text(ring.rootHash),
                .int(ring.size),
                .text(ring.hashingPolicy),
                .int(Int(ring.updatedAt.timeIntervalSince1970)),
                .text(String(data: metadataData, encoding: .utf8) ?? "{}")
            ]
        )
    }

    /// Retrieve evidence ring by ID
    public func getRing(id: String) async throws -> EvidenceRing? {
        let rows = try await database.query(
            """
            SELECT ring_id, peaks_json, root_hash, ring_size,
                   hashing_policy, updated_at, metadata_json
            FROM evidence_rings
            WHERE ring_id = ?
            """,
        ...
        let peaksString = row.string(for: "peaks_json") ?? "[]"
        let metadataString = row.string(for: "metadata_json") ?? "{}"
        
        let peaks = try JSONDecoder().decode([String].self, from: peaksString.data(using: .utf8) ?? Data())
        let metadata = try JSONDecoder().decode([String: String].self, from: metadataString.data(using: .utf8) ?? Data())
        
        return EvidenceRing(
            ringId: row.string(for: "ring_id") ?? id,
            peaks: peaks,
            rootHash: row.string(for: "root_hash") ?? "",
            size: row.int(for: "ring_size") ?? 0,
            hashingPolicy: row.string(for: "hashing_policy") ?? "blake3Tier1V2",
            updatedAt: Date(timeIntervalSince1970: TimeInterval(row.int(for: "updated_at") ?? 0)),
            metadata: metadata
        )
    }
