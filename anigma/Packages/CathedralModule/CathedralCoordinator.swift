...
public actor CathedralCoordinatorImpl: CathedralCoordinator {
    ...
    public func createExecutionLease(
        ...
        let leaseEvidence = Evidence(
            ...
            contentHash: lease.id.sha256Hash,
    ...
    public func executeWithEvidence(
        ...
        let resultEvidence = Evidence(
            ...
            sessionId: operation.sessionId,
            agentId: operation.agentId,
            contentHash: result.id.sha256Hash,
            metadata: EvidenceMetadata(
                source: "CathedralCoordinator",
    ...
    private func executeOperation(
        ...
        // Fallback: Simulate operation execution (no ML service configured)
        return OperationResult(
            id: UUID().uuidString,
            operationId: operation.id,
            status: .success,
            timestamp: Date(),
            data: ["note": "No ML service configured - operation simulated"]
        )
    }
}
