...
///   - context: context of the expansion.
///   - indentationWidth: The indentation that should be added for each additional
struct ExpandAttachedMacroWithoutCollapsingConfiguration: Sendable {
    let definition: Definition
    let macroRole: MacroRole
    let attributeNode: AttributeSyntax
    let declarationNode: Syntax
    let parentDeclNode: Syntax?
    let extendedType: TypeSyntax?
    let conformanceList: InheritedTypeListSyntax?
    let context: Context
    ...
    init(
        ...
        declarationNode: Syntax,
        parentDeclNode: Syntax?,
        ...
        self.macroRole = macroRole
        self.attributeNode = attributeNode
        self.declarationNode = declarationNode
        self.parentDeclNode = parentDeclNode
        self.extendedType = extendedType
        self.conformanceList = conformanceList
...
func expandAttachedMacroWithoutCollapsing(
    config: ExpandAttachedMacroWithoutCollapsingConfiguration
) -> [String]? {
    // Implementation using config properties
    return expandAttachedMacroWithoutCollapsing(
        definition: config.definition,
        macroRole: config.macroRole,
        attributeNode: config.attributeNode,
        node: config.declarationNode,
        parentDeclNode: config.parentDeclNode,
        extendedType: config.extendedType,
        conformanceList: config.conformanceList,
        in: config.context,
        indentationWidth: config.indentationWidth
    )
}
...
// Then modify the function signature to:
// func expandAttachedMacroWithoutCollapsing(config: ExpandAttachedMacroWithoutCollapsingConfiguration) -> [String]
      guard
...
/// Expand `@attached(XXX)` macros.
/// Configuration for expanding an attached macro
struct ExpandAttachedMacroConfiguration: Sendable {
    /// A type that conforms to one or more attached `Macro` protocols.
    let definition: any Macro.Type
    
    /// Indicates which `Macro` protocol expansion should be performed
    let macroRole: MacroRole
    
    /// Attribute syntax node (e.g. `@macroName(argument)`).
    let attributeNode: AttributeSyntax
    
    /// Target declaration syntax node to apply the expansion.
    /// This is either a declaration or a closure syntax node.
    let declarationNode: Syntax
    
    /// Only used for `MacroRole.memberAttribute`.
    /// The parent context node of `declarationNode`.
    let parentDeclNode: DeclSyntax?
    
    /// The extended type for extension macros
    let extendedType: TypeSyntax?
    
    /// The conformance list for extension macros
    let conformanceList: InheritedTypeListSyntax?
    
    /// Context of the expansion
    let context: MacroExpansionContext
    
    /// Indentation width for formatting
    let indentationWidth: Trivia
    
    init(
        definition: any Macro.Type,
        macroRole: MacroRole,
        attributeNode: AttributeSyntax,
        declarationNode: Syntax,
        parentDeclNode: DeclSyntax? = nil,
        extendedType: TypeSyntax? = nil,
        conformanceList: InheritedTypeListSyntax? = nil,
...
/// - Parameter config: Configuration for expanding the attached macro
func expandAttachedMacro(config: ExpandAttachedMacroConfiguration) throws -> [CodeBlockItemSyntax] {
    // Implementation would call the original function with extracted parameters
    // or be refactored to use the config directly
    return try expandAttachedMacro(
        definition: config.definition,
        macroRole: config.macroRole,
        attributeNode: config.attributeNode,
        declarationNode: config.declarationNode,
        parentDeclNode: config.parentDeclNode,
        extendedType: config.extendedType,
        conformanceList: config.conformanceList,
...
// Updated function signature:
func expandAttachedMacro(config: ExpandAttachedMacroConfiguration) -> Syntax? {
    // Implementation using config.definition, config.macroRole, etc.
}
    switch mode {
    case .auto:
