...
extension RawSyntax {
  ...
  ///   - kind: Token kind.
  ///   - text: Token text.
  ///   - triviaPieces: Trivia pieces buffer.
  ///   - numLeadingTrivia: Number of leading trivia pieces.
  ///   - byteLength: Byte length.
  ///   - presence: Presence.
  ///   - tokenDiagnostic: Token diagnostic.
  ///   - arena: Arena.
  ...
  ///   - tokenDiagnostic: Token diagnostic.
  ///   - arena: RawSyntaxArena to the result node data resides.
  ///   - initializingLeadingTriviaWith: A closure that initializes the leading trivia.
  ///   - initializingTrailingTriviaWith: A closure that initializes the trailing trivia.
  public static func makeMaterializedToken(
    ...
    initializingLeadingTriviaWith(
      UnsafeMutableBufferPointer(start: triviaBuffer.baseAddress, count: leadingTriviaPieceCount)
    )
    initializingTrailingTriviaWith(
      UnsafeMutableBufferPointer(
        start: triviaBuffer.baseAddress?.advanced(by: leadingTriviaPieceCount),
        count: trailingTriviaPieceCount
      )
    )

    var byteLength = text.count
    for i in 0..<totalTriviaCount {
      byteLength += Int(triviaBuffer[i].byteLength)
    }

    return .materializedToken(
      kind: kind,
      text: text,
      triviaPieces: ArenaAllocatedBufferPointer(triviaBuffer),
