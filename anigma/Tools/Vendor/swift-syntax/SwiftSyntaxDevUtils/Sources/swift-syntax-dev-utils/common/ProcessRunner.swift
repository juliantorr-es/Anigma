...
class SigIntListener {
  ...
  /// Protected with nonisolated(unsafe) because signal handlers cannot safely
  /// interact with async code. Access is coordinated: signal handler reads,
  /// main thread writes. Set operations are atomic for practical purposes.
  nonisolated(unsafe) static var runningSubprocesses: Set<Process> = []

  /// Whether a `SIGINT` signal has been received by this script.
  /// Protected with nonisolated(unsafe) for same reason as runningSubprocesses.
  nonisolated(unsafe) static var hasReceivedSigInt: Bool = false
