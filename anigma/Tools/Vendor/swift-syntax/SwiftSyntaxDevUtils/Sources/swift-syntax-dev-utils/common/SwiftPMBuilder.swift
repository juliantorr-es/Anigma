...
struct InvokeSwiftPMConfiguration: Sendable {
    let action: String
    let packageDir: URL
...
func invokeSwiftPM(config: InvokeSwiftPMConfiguration) throws -> ProcessResult {
    var args = [config.action]
    if config.action == "test" {
        args += ["--disable-testable-imports"]
    }
    args += ["--package-path", config.packageDir.path]
