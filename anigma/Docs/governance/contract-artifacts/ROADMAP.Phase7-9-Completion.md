...
- MVP tool set including read_file, apply_patch, swift_build, swift_test, git_diff, trace_query, each with a contract and stable error taxonomy.
- Session DB ATTACH and merge with deterministic conflict resolution and idempotent merges.
- Recovery UX including governed recovery for interrupted sessions and BuildIngest recovery.
...
- Recovery correctness: resumption does not re-execute already evidenced steps unless configured for replay.
...
Deliverables
- Renderer adapters exist for COSMIC or libcosmic, WinUI or Avalonia, and Compose, implemented as presentation-only adapters without creating new cores.
- Renderers consume a governed UI schema and action boundary and do not bypass policy gates for governed actions.
- Inference connectors exist for MLX, llama.cpp or MLC, ONNX, and Core ML via the existing ML Runtime Bridge.
...
- Backend interchangeability: the same request can route to each backend connector and produces comparable evidence records with correct identities and hashes.
...
- Replay verifier integrated as a gating suite validating canonical fixtures and real session traces.
...
- Policy evolution staging with evidence-backed propose, validate, adopt lifecycle and explicit stop conditions.
- Evidence hooks for autonomous loops comparable to tool step evidence.
- Canonical event-log fixtures maintained for replay checks.
- Single-commit invariant for autonomous patch proposals, including deterministic artifact identity and rollback hooks.
- Canonical replay fixture stored at `Artifacts/phase9-fixtures/canonical-fixture.json` and exercised via `Scripts/ci-phase9-verifier.sh` using `Phase9ReplayVerifier`.

Acceptance tests
- Replay determinism: canonical fixtures replay with identical canonical event log hashes across repeated runs.
