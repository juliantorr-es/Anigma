...
- [ ] **Normalization CI gates** – Add CI gates that enforce canonical formatting for:
  ...
  - [ ] SQL (`sqlite-format` with consistent spacing/quoting)
...
- [ ] **Session DBs + Merge-to-Master**
  - [ ] DatabaseConfiguration for session DB paths (`ANIGMA_SESSION_ID` creates `Data/sessions/<id>.sqlite`)
  - [ ] CLI commands:
  ...
  - [ ] Merge using ATTACH DATABASE (append-only tables only)
