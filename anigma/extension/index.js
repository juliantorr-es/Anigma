 ...
 * 2. Local Binary (anigma-mcp) via subprocess
 */
...
async function executeViaBinary(name, args) {
  return new Promise((resolve, reject) => {
    // In a real setup, ensure anigma-mcp is in your PATH or use absolute path
    const process = spawn('anigma-mcp', [name, JSON.stringify(args)]);
    let output = '';
    let error = '';
