...
KernelContext::KernelContext(anigma_evidence_ring_t* ring) {
    // MANDATE: Native kernel fails to start without mmap-backed logging ring
    if (!ring) {
        logging_ring.metadata = nullptr;
        logging_ring.packets = nullptr;
        return;
    }
    
    if (!ring->metadata || !ring->packets) {
        logging_ring.metadata = nullptr;
        logging_ring.packets = nullptr;
        return;
    }
    ...
    logging_ring = *ring;
    memset(mission_id, 0, 16);
    
    // Emit initial start packet
    emit_heartbeat(1, nullptr, 0); // ANIGMA_PACKET_START
}

KernelContext::~KernelContext() {
    if (is_valid()) {
        emit_heartbeat(3, nullptr, 0); // ANIGMA_PACKET_END
    }
}

// Static validation - fails kernel start without valid mmap ring
std::optional<KernelContext::InitError> KernelContext::validate_ring(anigma_evidence_ring_t* ring) {
    if (!ring) return InitError::noLoggingRing;
    if (!ring->metadata) return InitError::invalidMetadata;
    if (!ring->packets) return InitError::invalidPackets;
    return std::nullopt;
}

// Zero-copy residency check - verifies mmap is still mapped
bool KernelContext::is_resident() const {
    if (!is_valid()) return false;
    // Check if the metadata pointer is still valid by accessing it
    // If page was unmapped, accessing would SIGBUS
    return logging_ring.metadata->ring_size > 0;
}

// Compute BLAKE3 hash of ring state for ContractReceipt
void KernelContext::ring_hash(uint8_t out_hash[32]) const {
    blake3_hasher hasher;
    blake3_hasher_init(&hasher);
    
    // Hash metadata
    if (logging_ring.metadata) {
        blake3_hasher_update(&hasher, 
            reinterpret_cast<const uint8_t*>(logging_ring.metadata), 
            sizeof(anigma_evidence_ring_metadata_t));
    }
    
    // Hash packet headers (not payloads to keep it fast)
    if (logging_ring.packets && logging_ring.metadata) {
        size_t header_size = sizeof(anigma_evidence_packet_t) * logging_ring.metadata->ring_size;
        size_t payload_offset = offsetof(anigma_evidence_packet_t, payload_hash);
        for (size_t i = 0; i < logging_ring.metadata->ring_size; i++) {
            blake3_hasher_update(&hasher,
                reinterpret_cast<const uint8_t*>(&logging_ring.packets[i]) + payload_offset,
                32); // Only hash the payload_hash field
        }
    }
...
void KernelContext::emit_heartbeat(uint32_t packet_type, const uint8_t* payload, size_t payload_size) {
    if (!is_valid()) return;

    uint32_t slot = logging_ring.metadata->producer_index % logging_ring.metadata->ring_size;
    anigma_evidence_packet_t& packet = logging_ring.packets[slot];

    memcpy(packet.mission_id, mission_id, 16);
    ...
    if (payload && payload_size > 0) {
        blake3_hasher hasher;
        blake3_hasher_init(&hasher);
        blake3_hasher_update(&hasher, payload, payload_size);
        blake3_hasher_finalize(&hasher, packet.payload_hash, 32);
    } else {
        memset(packet.payload_hash, 0, 32);
    }

    packet.power_watts = 0.0f;
    packet.ops_per_joule = 0.0f;

    logging_ring.metadata->producer_index++;
}
...
anigma_status_t KernelContext::apply_diff_batch(anigma_blob_t batch) {
    ...
    if (is_valid()) {
        emit_heartbeat(2, batch.ptr, batch.size); // ANIGMA_PACKET_HEARTBEAT
    }
...
anigma_status_t KernelContext::generate_render_plan(anigma_blob_t viewport_request, anigma_mut_blob_t* out_plan) {
    ...
    if (is_valid()) {
        emit_heartbeat(2, viewport_request.ptr, viewport_request.size); // ANIGMA_PACKET_HEARTBEAT
    }
...
anigma_status_t anigma_kernel_initialize(anigma_ctx_t ctx, anigma_blob_t config, anigma_evidence_ring_t* logging_ring, anigma_kernel_instance_t* out_instance) {
    (void)ctx; (void)config;
    if (out_instance == nullptr) return ANIGMA_ERR_INVALID_ARG;
    
    // MANDATE: Native kernel fails to start without mmap-backed logging ring
    if (logging_ring == nullptr || logging_ring->metadata == nullptr || logging_ring->packets == nullptr) {
        return ANIGMA_ERR_SECURITY_VIOLATION;
    }

    auto* kernel = new anigma_kernel_t(logging_ring);
    if (!kernel->ctx.is_valid()) {
        delete kernel;
        return ANIGMA_ERR_SECURITY_VIOLATION;
    }

    *out_instance = reinterpret_cast<anigma_kernel_instance_t>(kernel);
    return ANIGMA_OK;
}
...
anigma_status_t anigma_kernel_state_hash(anigma_kernel_instance_t instance, uint8_t out_hash32[32]) {
    if (!instance || !out_hash32) return ANIGMA_ERR_INVALID_ARG;
    auto* kernel = reinterpret_cast<anigma_kernel_t*>(instance);
    kernel->ctx.ring_hash(out_hash32);
    return ANIGMA_OK;
}
