...
struct LayoutEngineState {
        ...
        for (int page_idx = 0; page_idx < page_count; ++page_idx) {
            ...
            for (int i = 0; i < char_count; ++i) {
                ...
                if (nameLen > 0 && nameLen <= sizeof(fontNameBuffer)) {
                    ...
                    if (it != font_name_cache.end()) {
                        ...
                        char* allocated = safe_strdup(key.c_str());
                ...
                for (size_t i = 1; i <= chars.size(); ++i) {
                    ...
                    if (newCluster) {
                        ...
                        if (count > 0) {
                            ...
                            segment.font_name = chars[start].fontName ? safe_strdup(chars[start].fontName) : nullptr;
            ...
            if (!images.empty()) {
                ...
                for (size_t i = 0; i < images.size(); ++i) {
                    ...
                    // Copy filter string
                    if (src.filter) {
                        dst.filter = safe_strdup(src.filter);
                    } else {
                        dst.filter = nullptr;
                    }
                }
    ...
    char* internText(const std::string& text) {
        ...
        char* allocated = safe_strdup(text.c_str());
    ...
    for (size_t i = 0; i < pages_to_copy; ++i) {
        ...
        if (src.segment_count > 0 && src.segments) {
            ...
            for (size_t j = 0; j < src.segment_count; ++j) {
                ...
                // Copy text string
                dst_seg.text = src_seg.text ? safe_strdup(src_seg.text) : nullptr;
                
                // Copy font name
                dst_seg.font_name = src_seg.font_name ? safe_strdup(src_seg.font_name) : nullptr;
        ...
        if (src.image_count > 0 && src.images) {
            ...
            for (size_t j = 0; j < src.image_count; ++j) {
                ...
                // Copy filter string
                if (src_img.filter) {
                    dst_img.filter = safe_strdup(src_img.filter);
                } else {
