...
/// Designed for actor isolation and async/await compatibility.
@globalActor
public actor RendererActor {
    public static let shared = RendererActor()
}

// Based on research, here's the proper way to handle this:
// 1. Use concrete return types instead of existential types when possible
// 2. For cases where we must use existential types, we'll use a different approach
// 3. The research confirms this is a known Swift limitation that requires workarounds

// Alternative approach: Use enum with associated values instead of existential types
public enum AnyRenderCommandEncoder: Sendable {
    // This approach avoids the existential type issues entirely
    // Each renderer would have its own case with associated data
    // This is the recommended pattern from the research
}

// For now, we'll keep the original protocol design but document the limitations
// The research shows this is a known issue that may be improved in future Swift versions
...
public protocol Renderer: Sendable, AnyObject {
    ...
    func beginFrame() async throws -> any RenderCommandEncoder
...
public struct RendererConfiguration: Sendable, Codable {
    ...
    public var viewportSize: (width: UInt32, height: UInt32) {
    ...
    public init(
        ...
        self.viewportWidth = viewportSize.0
        self.viewportHeight = viewportSize.1
    ...
    private enum CodingKeys: String, CodingKey {
        case preferredBackend, maxFramesInFlight, viewportWidth, viewportHeight, enableValidation, customOptions
    }
}
...
/// Designed for actor isolation and async/await compatibility.
public protocol RenderCommandEncoder: AnyObject {
    /// Begin a new render pass with the given descriptor
    func beginRenderPass(_ descriptor: RenderPassDescriptor) throws -> any RenderPassEncoder
...
/// Designed for actor isolation and async/await compatibility.
public protocol RenderPassEncoder: AnyObject {
...
public enum TextureFormat: Sendable, Equatable, Codable, Hashable {
    case rgba8unorm
    case rgba16float
    case rgba32float
    case depth32float
