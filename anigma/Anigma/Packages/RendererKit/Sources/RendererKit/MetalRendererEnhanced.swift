...
final actor MetalRenderPassEncoder: RenderPassEncoder {
    ...
    private func createMetalRenderPassDescriptor() -> MTLRenderPassDescriptor {
        ...
        for (index, colorAttachment) in descriptor.colorAttachments.enumerated() {
            if index < descriptor.colorAttachments.count {
                let attachment = descriptor.colorAttachments[index]!
                attachment.loadAction = metalLoadAction(descriptor.loadAction)
                attachment.storeAction = metalStoreAction(descriptor.storeAction)
