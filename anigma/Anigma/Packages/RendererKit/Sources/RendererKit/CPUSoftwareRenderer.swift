...
public actor CPUSoftwareRenderer: Renderer {
    public let id: UUID
...
final actor CPUSoftwareCommandEncoder: RenderCommandEncoder {
    private var frameBuffer: [UInt32]?
...
final actor CPUSoftwareRenderPassEncoder: RenderPassEncoder {
    private var frameBuffer: [UInt32]
    ...
    func drawPrimitives(
        ...
        let width = Int(sqrt(Double(frameBuffer.count)))
        let height = width
        ...
        case .point:
            for i in vertexStart..<vertexStart + vertexCount {
                ...
                if x >= 0 && x < width && y >= 0 && y < height {
                    frameBuffer[y * width + x] = 0xFF0000FF // Red point
                }
            }
        ...
        case .triangle:
            if vertexCount >= 3 {
                ...
                rasterizeTriangle(v0: v0, v1: v1, v2: v2, frameBuffer: frameBuffer, width: width, height: height)
            }
        ...
        case .line:
            if vertexCount >= 2 {
                for i in stride(from: 0, to: vertexCount - 1, by: 2) {
                    ...
                    drawLine(x0: x0, y0: y0, x1: x1, y1: y1, frameBuffer: frameBuffer, width: width, height: height)
                }
            }
