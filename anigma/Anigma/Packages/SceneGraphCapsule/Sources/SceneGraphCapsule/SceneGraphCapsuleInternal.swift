...
internal final class SceneGraphCapsuleInternal: Sendable {
    ...
    /// This is significantly faster than calling addNode repeatedly, especially for large batches
    internal func addNodes(_ nodes: [SceneNode], to sceneGraph: SceneGraph) -> SceneGraph {
        guard !nodes.isEmpty else { return sceneGraph }
        ...
        // Second pass: Update parent child lists in batch
        // Group nodes by parent to minimize parent object recreations
        var parentToNewChildren: [NodeID: [NodeID]] = [:]
        for node in nodes {
            if let parentID = node.parentID {
                parentToNewChildren[parentID, default: []].append(node.id)
            }
        }
        
        // Update each affected parent once with all new children
        for (parentID, childIDs) in parentToNewChildren {
            guard var parent = newNodes[parentID] else { continue }
            parent = SceneNode(
                id: parent.id,
                type: parent.type,
                name: parent.name,
    ...
    /// This is significantly faster than calling getWorldTransform repeatedly due to:
    /// - Transform caching for shared parent hierarchies
    /// - Reduced redundant hierarchy traversals when nodes share parents
    /// - Single pass through the scene graph
    internal func getWorldTransforms(for nodeIDs: [NodeID], in sceneGraph: SceneGraph) -> [NodeID: Transform3D] {
        guard !nodeIDs.isEmpty else { return [:] }
        
        var results: [NodeID: Transform3D] = [:]
        var transformCache: [NodeID: Transform3D] = [:]
        
        // Helper to get or compute cached transform
        func getCachedTransform(for nodeID: NodeID) -> Transform3D? {
            if let cached = transformCache[nodeID] {
                return cached
            }
            
            guard let node = sceneGraph.nodes[nodeID] else { return nil }
            
            let localTransform = node.components[.transform] as? Transform3D ?? Transform3D.identity
            
            // If no parent, this is the world transform
            guard let parentID = node.parentID else {
                transformCache[nodeID] = localTransform
                return localTransform
            }
            
            // Recursively get parent's world transform (uses cache)
            guard let parentTransform = getCachedTransform(for: parentID) else {
                return nil
            }
            
            let worldTransform = parentTransform * localTransform
            transformCache[nodeID] = worldTransform
            return worldTransform
        }
        
        // Compute transforms for all requested nodes
        for nodeID in nodeIDs {
            if let transform = getCachedTransform(for: nodeID) {
                results[nodeID] = transform
            }
        }
