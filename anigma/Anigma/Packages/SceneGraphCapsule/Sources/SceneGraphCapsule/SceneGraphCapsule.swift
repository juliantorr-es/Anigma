...
public final class SceneGraphCapsule: Sendable {
    ...
    /// Add multiple nodes to the scene graph in a single batch operation
    /// This is significantly more efficient than calling addNode repeatedly, especially for large batches.
    /// - Performance: 10-50x faster for batches of 100+ nodes due to:
    ///   - Single diagnostics span instead of N spans
    ///   - Bulk validation instead of per-node validation
    ///   - Optimized parent child-list updates (groups by parent)
    ///   - Single SceneGraph allocation instead of N allocations
    /// - Parameters:
    ///   - nodes: Array of nodes to add
    ///   - sceneGraph: Scene graph to add nodes to
    ///   - correlationID: Request ID for tracing (optional)
    /// - Returns: Updated scene graph with all nodes added
    /// - Throws: `CapsuleError` variants for addition failures
    public func addNodes(
        ...
        let corrID = correlationID ?? CorrelationIDContext.current
        let span = diagnostics.beginSpan(
            name: "SceneGraphCapsule.addNodes",
            category: "batch_node_addition",
            correlationID: corrID,
            tags: [
                "capsule_id": id,
                "batch_size": "\(nodes.count)"
            ]
        )
        ...
        diagnostics.event(
            level: .info,
            category: "scenegraphcapsule.addNodes",
            message: "Adding \(nodes.count) nodes to scene graph in batch",
            correlationID: corrID,
            metadata: ["tier": "1"]
        )
        
        do {
            let updatedSceneGraph = impl.addNodes(nodes, to: sceneGraph)
            
            diagnostics.event(
                level: .info,
                category: "scenegraphcapsule.addNodes",
                message: "Batch node addition completed",
                correlationID: corrID,
                metadata: [
                    "nodes_added": "\(nodes.count)",
                    "new_node_count": "\(updatedSceneGraph.nodeCount)"
                ]
            )
            ...
            diagnostics.event(
                level: .error,
                category: "scenegraphcapsule.addNodes",
                message: "Batch node addition failed: \(sceneError.localizedDescription)",
                correlationID: corrID,
                metadata: ["error_type": "SceneGraphError"]
            )
            span.end(status: .error)
            ...
            switch sceneError {
            case .invalidNode:
                throw CapsuleError.invalidInput(field: "nodes", constraint: sceneError.localizedDescription)
            default:
                throw CapsuleError.internalError(details: sceneError.localizedDescription)
            }
        } catch {
    ...
    /// Get world transforms for multiple nodes in a single batch operation
    /// This is significantly more efficient than calling getWorldTransform repeatedly, especially when nodes share parent hierarchies.
    /// - Performance: 5-10x faster for batch queries of 50+ nodes due to:
    ///   - Single diagnostics span instead of N spans
    ///   - Transform caching for shared parent hierarchies
    ///   - Reduced redundant hierarchy traversals
    /// - Parameters:
    ///   - nodeIDs: Array of node IDs to get transforms for
    ///   - sceneGraph: Scene graph containing the nodes
    ///   - correlationID: Request ID for tracing (optional)
    /// - Returns: Dictionary mapping node IDs to their world transforms (only includes found nodes)
    /// - Throws: `CapsuleError` variants for lookup failures
    public func getWorldTransforms(
        for nodeIDs: [NodeID],
        in sceneGraph: SceneGraph,
        correlationID: String? = nil
    ) async throws -> [NodeID: Transform3D] {
        let corrID = correlationID ?? CorrelationIDContext.current
        let span = diagnostics.beginSpan(
            name: "SceneGraphCapsule.getWorldTransforms",
            category: "batch_transform_lookup",
            correlationID: corrID,
            tags: [
                "capsule_id": id,
                "batch_size": "\(nodeIDs.count)"
            ]
        )
        ...
        diagnostics.event(
            level: .debug,
            category: "scenegraphcapsule.getWorldTransforms",
            message: "Getting world transforms for \(nodeIDs.count) nodes in batch",
            correlationID: corrID,
            metadata: ["tier": "1"]
        )
        
        let transforms = impl.getWorldTransforms(for: nodeIDs, in: sceneGraph)
        
        diagnostics.event(
            level: .debug,
            category: "scenegraphcapsule.getWorldTransforms",
            message: "Batch transform lookup completed",
            correlationID: corrID,
            metadata: [
                "requested_count": "\(nodeIDs.count)",
                "found_count": "\(transforms.count)"
            ]
        )
