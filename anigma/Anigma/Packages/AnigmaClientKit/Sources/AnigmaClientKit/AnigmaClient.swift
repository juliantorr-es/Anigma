...
public struct EnhancedAnigmaClient {
    ...
    /// Job priority levels
    public enum JobPriority: String, Codable, CaseIterable {
        case low = "low"
        case normal = "normal"
        case high = "high"
        case urgent = "urgent"
    ...
    private func executeRequest(_ request: URLRequest) async throws -> (Data, HTTPURLResponse) {
        ...
        for attempt in 0...configuration.retryCount {
            do {
                ...
                if case ClientError.authenticationRequired = error ||
                   case ClientError.invalidURL = error ||
                   case ClientError.invalidResponse = error {
                    throw error
                }
