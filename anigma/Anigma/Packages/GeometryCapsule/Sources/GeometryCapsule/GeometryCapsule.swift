...
public final class GeometryCapsule: Sendable {
    ...
    public func transformPoint(
        ...
        do {
            let transformedPoint = try impl.transformPoint(point, with: transform)
    ...
    public func transformMesh(
        ...
        do {
            let transformedMesh = try impl.transformMesh(mesh, with: transform)
    ...
    public func calculateBoundingBox(
        ...
        do {
            let boundingBox = try impl.calculateBoundingBox(for: points)
    ...
    public func calculateMeshVolume(
        ...
        do {
            let volume = try impl.calculateMeshVolume(mesh)
    ...
    /// Check if two triangle meshes intersect
    /// - Parameters:
    ///   - mesh1: First mesh
    ///   - mesh2: Second mesh
    ///   - correlationID: Request ID for tracing (optional)
    ...
    /// - Throws: `CapsuleError` variants for calculation failures
    public func checkIntersection(
        between mesh1: TriangleMesh,
        and mesh2: TriangleMesh,
        correlationID: String? = nil
    ) async throws -> Bool {
        let corrID = correlationID ?? CorrelationIDContext.current
        let span = diagnostics.beginSpan(
            name: "GeometryCapsule.checkIntersectionMeshes",
            category: "intersection_check",
            correlationID: corrID,
            tags: [
                "capsule_id": id,
                "mesh1_vertices": "\(mesh1.vertices.count)",
                "mesh2_vertices": "\(mesh2.vertices.count)"
            ]
        )
        ...
        diagnostics.event(
            level: .debug,
            category: "geometrycapsule.checkIntersectionMeshes",
            message: "Checking mesh intersection",
            correlationID: corrID,
            metadata: ["tier": "3"]
        )
        
        // No explicit error handling needed as impl method is non-throwing, 
        // but we wrap in do-catch if we change that later or for consistency.
        let intersects = impl.meshesIntersect(mesh1, mesh2)
        
        // Success event
        diagnostics.event(
            level: .info,
            category: "geometrycapsule.checkIntersectionMeshes",
            message: "Mesh intersection check completed",
            correlationID: corrID,
            metadata: [
                "result": "\(intersects)"
            ]
