...
func makeTextChunkingBenchmarks(iterations: Int?) throws -> [BenchmarkCase] {
    if let iterations {
        return [try TextChunkingLargeInputBenchmark(iterations: iterations)]
    }
    return [try TextChunkingLargeInputBenchmark()]
}
...
do {
    ...
    let report = try runner.run(benchmarks: benchmarks)
