...
private let macShellLogger = Logger(subsystem: "com.anigma.app", category: "mac-shell")
                ...
                if let data = await store.downloadArtifact(id: id),
                   let artifact = store.artifacts.first(where: { $0.id == id }) {
                    ...
                    do {
                        ...
                        macShellLogger.error("Failed to write preview file: \(error)")
                    }
                }
            case .entity(let id):
                ...
                do {
                    ...
                    macShellLogger.error("Failed to generate entity preview: \(error)")
                }
            case .job(let id):
                // Preview job details
...
struct ToastLayer: View {
    ...
    var body: some View {
        ...
        .onChange(of: store.activeToasts.count) { oldValue, newValue in
            if newValue > oldValue, let lastToast = store.activeToasts.last {
                // Announce new toast
            }
        }
    }
...
struct UserSurfaceView: View {
    ...
    var body: some View {
        ...
        case .aiConsole:
            AIConsoleView(client: store.aiConsoleClient)
        case .develop, .developFiles, .developSearch, .developChanges, .developRuns, .developReview, .developTasks, .developAgents:
            DevelopView()
        case .activity:
            ActivityView()
        }
