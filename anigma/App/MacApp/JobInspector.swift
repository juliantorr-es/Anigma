...
struct JobInspector: View {
    ...
    @State private var cancelEvaluation: IntentEvaluation = .allowed
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if let hash = job.finalReceiptHash {
                        InfoRow(label: "Receipt", value: hash)
                    }
                }
                ...
                VStack(spacing: 8) {
                    ...
                    if case .denied(let reason) = cancelEvaluation,
                        job.status.uppercased() == "RUNNING" {
                        Text(reason)
                            .font(.caption)
                            .foregroundStyle(.red)
                    }

                    if case .denied(let reason) = deleteEvaluation, job.isTerminal {
                        Text(reason)
                            .font(.caption)
                            .foregroundStyle(.red)
                    }
    ...
    private func evaluateActions() async {
        ...
        if job.status.uppercased() == "SUCCEEDED" || job.status.uppercased() == "FAILED"
            || job.status.uppercased() == "CANCELED" {
    ...
    private struct LogsSheet: View {
        ...
        var body: some View {
            NavigationStack {
                VStack {
                    Text("Job logs are not yet available.")
                        .padding()
                    // Placeholder for future log integration
                    Text("Job ID: \(job.id)")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                .navigationTitle("Logs for \(job.name)")
                .toolbar {
