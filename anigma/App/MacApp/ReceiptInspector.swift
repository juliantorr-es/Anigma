            ...
            if isLoading {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        // Seal Type
                        HStack {
                            let (icon, color, label) = sealInfo(for: receipt.seal)
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            InfoRow(label: "Receipt ID", value: receipt.ref.id)
                            ...
                            InfoRow(
                                label: "Timestamp",
                                value: Date(
                                    timeIntervalSince1970: Double(receipt.ref.timestamp) / 1000
                                ).formatted())
                                .accessibilityElement(children: .combine)
                        }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(receipt.outcome.description)
                                .font(.system(.body, design: .monospaced))
                                .padding()
                                .background(Color.secondary.opacity(0.1))
                                .cornerRadius(8)
                                .accessibilityLabel("Outcome Data")
                                .accessibilityValue(receipt.outcome.description)
                        }
                ...
                ContentUnavailableView("Receipt Not Found", systemImage: "doc.text")
            }
        }
        .padding()
        .task {
