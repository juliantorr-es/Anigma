...
private let agentRunnerLogger = Logger(subsystem: "com.anigma.app", category: "governance")
...
actor SandboxManager {
    ...
    func createSandbox(for workspace: RepoWorkspace) throws -> URL {
        ...
        agentRunnerLogger.info("Created sandbox at \(sandboxURL.path, privacy: .public)")
        return sandboxURL
    }
    ...
    func destroySandbox(at url: URL) {
        do {
            ...
            agentRunnerLogger.error("Failed to clean up sandbox: \(error.localizedDescription, privacy: .public)")
        }
    }
}
