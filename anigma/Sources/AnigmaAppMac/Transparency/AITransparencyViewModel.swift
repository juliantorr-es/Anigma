...
class AITransparencyViewModel: ObservableObject {
    ...
    func loadOperations() {
        ...
        Task {
            do {
                // Get current session statistics from AIReceiptIntegration
                let integration = await AIReceiptIntegration.shared
                let sessionStats = integration.getSessionStats()
                
                // Convert to AI operations (mock data for now)
                let operations = generateMockOperations(from: sessionStats)
                ...
                print("Failed to load AI operations: \(error)")
            }
        }
    }
    ...
    private func generateMockOperations(from stats: SessionStats) -> [AIOperation] {
        var operations: [AIOperation] = []
        let now = Date()
        
        // Generate some mock operations based on session stats
        for i in 0..<stats.totalOperations {
            ...
            let operation = AIOperation(
                ...
                taskType: stats.taskTypeStats.keys.randomElement()?.rawValue ?? "llm_chat",
