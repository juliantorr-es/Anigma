    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if stats.providerStats.isEmpty {
                ...
                .chartAngleSelection(value: .constant(nil as String?))
                .chartForegroundStyleScale(range: {
                    if stats.providerStats.count == 1 {
                        ...
                        return [Color.blue, Color.green, Color.orange, Color.purple, Color.red]
                    }
                }())
                .frame(height: 200)
...
struct ChainIntegrityView: View {
    @ObservedObject var viewModel: AITransparencyViewModel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                HStack {
                    Image(systemName: viewModel.chainVerificationStatus.icon)
                        .font(.title)
                        .foregroundStyle(viewModel.chainVerificationStatus.color)
                    
                    VStack(alignment: .leading, spacing: 4) {
                        Text("CoreReceipt Chain")
                            .font(.subheadline)
                            .fontWeight(.medium)
                        Text(viewModel.chainVerificationStatus.displayName)
                            .font(.caption)
                            .foregroundStyle(viewModel.chainVerificationStatus.color)
                    }
                    ...
                    .disabled(viewModel.chainVerificationStatus == .pending)
                }
                
                if viewModel.chainVerificationStatus == .verified {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("✅ All receipts in the chain are cryptographically verified")
                            .font(.caption)
                            .foregroundStyle(.green)
                        
                        if viewModel.verificationTimeMs > 0 {
                            Text(String(format: "Verified 10MB evidence in %.2fms (Parallel BLAKE3)", viewModel.verificationTimeMs))
...
struct RecentVerificationsView: View {
    private let recentVerifications: [(String, Date, VerificationStatus)] = [
        ("CoreReceipt Chain", Date().addingTimeInterval(-300), .verified),
        ("Individual CoreReceipt", Date().addingTimeInterval(-900), .verified),
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                VerificationToolButton(
                    title: "Verify CoreReceipt Chain",
