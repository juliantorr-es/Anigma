...
struct ReceiptChainView: View {
    ...
    @State private var receipts: [CoreReceipt] = []
...
struct ReceiptNodeView: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    Text(receipt.ref.id)
                        ...
                        .fontWeight(.bold)
                    Spacer()
                    Text(formatDate(Int64(receipt.ref.timestamp.timeIntervalSince1970 * 1000)))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
        ...
        .accessibilityLabel("Receipt \(receipt.ref.id). Status: \(receipt.status.rawValue). Time: \(formatDate(Int64(receipt.ref.timestamp.timeIntervalSince1970 * 1000))).")
