...
struct GovernanceChip: View {
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 4) {
                // Compute
                ChromeStatusPill(label: "Compute: Local", icon: "cpu", color: .secondary)

                // Data
                let hasExternal = store.sources.contains { $0.type == .email }
                ChromeStatusPill(label: hasExternal ? "Data: External" : "Data: Local", icon: "cylinder.split.1x2", color: hasExternal ? .orange : .secondary)

                // Network
                let networkOpen = store.privacySettings["allowCloudAI"] ?? false
                ChromeStatusPill(label: networkOpen ? "Net: Open" : "Net: Off", icon: "network", color: networkOpen ? .orange : Bauhaus.Color.textTertiary)
            }
            .padding(4)
...
struct HarmoniaCliButton: View {
    @Environment(AppStore.self) private var store
    
    var body: some View {
        Button {
            store.isHarmoniaCliPresented = true
        } label: {
        ...
        .buttonStyle(.plain)
        .help("Open Harmonia CLI")
        .accessibilityLabel("Open Harmonia CLI")
    }
}
...
struct JobCenterButton: View {
    ...
    var body: some View {
        ...
        let currentVerb = store.jobs.first { $0.status.uppercased() == "RUNNING" }?.name.components(separatedBy: " ").first ?? "Active"
        ...
        Button {
            ...
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(store.hasRunningJobs ? Bauhaus.Color.accent.opacity(0.1) : Color.black.opacity(0.001))
            .background(.ultraThinMaterial)
            .overlay(
                RoundedRectangle(cornerRadius: Bauhaus.Grid.cornerRadius)
                    .stroke(store.hasRunningJobs ? Bauhaus.Color.accent.opacity(0.3) : Bauhaus.Color.border, lineWidth: 0.5)
            )
