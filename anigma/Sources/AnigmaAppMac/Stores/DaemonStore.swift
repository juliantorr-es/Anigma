...
final class DaemonStore {
    ...
    func initializeDaemon() async {
        do {
            ...
            try await step1_ensureDaemonRunning()
            
            // Step 2: Establish Sidecar Bridge
            try await step2_establishBridge()
            
            // Step 3: Create Authority
            try await step3_createAuthority()
            
            // Step 4: Register surface
            try await step4_registerSurface()
            
            // Step 5: Create client
            try await step5_createClient()
            
            // Step 6: Bootstrap default workspace
            try await step6_bootstrapWorkspace()
    ...
    private func step1_ensureDaemonRunning() async throws {
    ...
    private func step2_establishBridge() async throws {
    ...
    private func step3_createAuthority() async throws {
    ...
    private func step4_registerSurface() async throws {
    ...
    private func step5_createClient() async throws {
        guard let authority = authority, let sid = surfaceId, let actorId = actorId, let token = capabilityToken else {
            throw DaemonStoreError.authorityNotInitialized
        }
    ...
    private func step6_bootstrapWorkspace() async throws {
        guard let authority = authority else {
            throw DaemonStoreError.authorityNotInitialized
        }
    ...
    func streamJobEvents(jobId: String) async throws -> AsyncThrowingStream<AnigmaJobEvent, Error> {
        ...
        return try await bridge.streamJobEvents(jobId: jobId)
    }
