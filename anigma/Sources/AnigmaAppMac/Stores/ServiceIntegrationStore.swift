...
final class ServiceIntegrationStore {
    ...
    init(
        ...
        // Initialize PipelineStore with daemon bridge
        guard let daemonBridge = daemonStore.daemonBridge else {
            preconditionFailure("Daemon bridge must be available before initializing PipelineStore")
        }
        self.pipelineStore = PipelineStore(daemonBridge: daemonBridge)
        ...
        self.astServicesStore = ASTServicesStore(
            astServicesClient: astServicesClient,
            daemonStore: daemonStore
        )
