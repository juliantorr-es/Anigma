...
public struct ActiveDownloadsView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            if downloads.isEmpty {
                ...
                HStack {
                    ...
                    Button("Cancel All") {
                        Task {
                            await appStore.downloadQueue.cancelAll()
                        }
                    }
                    .buttonStyle(.bordered)
                }
    ...
    private func refreshDownloads() async {
        let status = await appStore.downloadQueue.getQueueStatus()
        await MainActor.run {
            self.downloads = status.activeCount > 0 ? await appStore.downloadQueue.getAllDownloads() : []
        }
    }
