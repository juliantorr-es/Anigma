...
// NonPersistent
struct PrivacyConsole: View, Sendable {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: Bauhaus.Grid.x6) {
                    ...
                    PrivacyCard(
                        ...
                        icon: "server.rack",
                        status: (store.privacySettings["allowCloudAI"] ?? false) ? "HYBRID" : "LOCAL ONLY",
                        statusColor: (store.privacySettings["allowCloudAI"] ?? false)
                            ? Bauhaus.Color.warning : Bauhaus.Color.success
                    ) {
                        Toggle(isOn: .init(
                            get: { store.privacySettings["allowCloudAI"] ?? false },
                            set: { _ in /* Placeholder */ }
                        )) {
                        ...
                        .accessibilityLabel("Allow Third-Party Cloud AI")
                        .accessibilityHint("Toggles whether data can be sent to approved cloud providers")
                    }
                    ...
                    PrivacyCard(
                        ...
                        Toggle(
                            "Share Crash Reports", isOn: .init(get: { false }, set: { _ in }))
                            .accessibilityHint("Sends crash logs to developers to help fix bugs")
                        Toggle("Share Anonymous Usage", isOn: .init(get: { false }, set: { _ in }))
                            .accessibilityHint("Sends anonymous usage data to improve the app")
                    }
                }
                .padding(Bauhaus.Grid.x6)
            }
        }
        .frame(width: 500, height: 600) // OK: Sheet size
