    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Bauhaus.Grid.x3) {
            ...
            if let results = store.searchResults {
                resultsView(results)
            } else if store.ledgerSearchProjection.isSearching {
                loadingView
            ...
            if let error = store.ledgerSearchProjection.errorMessage {
                errorView(error)
            }
        }
    ...
    private var searchBarView: some View {
        VStack(alignment: .leading, spacing: Bauhaus.Grid.x2) {
            HStack(spacing: Bauhaus.Grid.x2) {
                ...
                .primaryButtonStyle()
                .disabled(query.isEmpty || store.ledgerSearchProjection.isSearching)

                if store.ledgerSearchProjection.isSearching {
                    ProgressView()
                        .controlSize(.small)
                }
            }
