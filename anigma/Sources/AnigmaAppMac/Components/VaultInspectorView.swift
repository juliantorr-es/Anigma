...
struct VaultInspectorView: View, Sendable {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Bauhaus.Grid.x3) {
            ...
            statisticsView(store.vaultStatus)
            actionsView
    ...
    private func statisticsView(_ vault: AppStore.VaultStatusInfo) -> some View {
        VStack(alignment: .leading, spacing: Bauhaus.Grid.x2) {
            ...
            statRow(
                label: "Storage Used",
                value: formatBytes(Int64(vault.storageUsed)),
            ...
            statRow(
                label: "Status",
                value: vault.status,
                icon: vault.status == "ready" ? "checkmark.circle.fill" : "exclamationmark.circle.fill",
                color: vault.status == "ready" ? Bauhaus.Color.success : Bauhaus.Color.warning
            )
        }
    }
