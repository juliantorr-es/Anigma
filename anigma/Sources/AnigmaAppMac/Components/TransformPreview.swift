...
// NonPersistent
struct ComponentPreview: View, Sendable {
    let proposal: TransformProposal
    let onConfirm: () -> Void
    let onCancel: () -> Void

    var body: some View {
        // Placeholder body for ComponentPreview
        Text("Component Preview for: \(proposal.title)")
    }
}

// NonPersistent
struct TransformPreview: View, Sendable {
    let proposal: TransformProposal
    let onConfirm: () -> Void
    let onCancel: () -> Void
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: Bauhaus.Grid.x3) {
                Button("Cancel", role: .cancel, action: onCancel)
                    ...
                    .accessibilityHint("Discards the proposed transformation")
                ...
                .accessibilityLabel("Confirm and run")
                .accessibilityHint(isBlocked ? "Action blocked by governance policy" : "Executes the proposed transformation")
            }
            .padding(Bauhaus.Grid.x4)
            .background(Bauhaus.Color.cardBackground)
        }
