...
struct OrchestratorView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showToolDiscovery) {
            ToolDiscoverySheet(orchestrator: orchestrator)
        }
    }
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: Bauhaus.Grid.x2) {
                        ...
                        ForEach(Array(plan.steps.enumerated()), id: \.element.id) { index, step in
                            PlanStepRow(
                                step: step,
                                isActive: index == orchestrator.currentStepIndex && orchestrator.isProcessing
                            )
                        }
                    }
                    .padding()
    ...
    private func submitTask() {
        ...
        guard let workspace = store.activeWorkspace else {
            orchestrator.messages.append(OrchestratorMessage(
                role: .system,
                content: "⚠️ No workspace selected. Please open a repository first."
            ))
            return
        }

        Task {
            await orchestrator.submitTask(task, workspace: workspace)
        }
    }
}
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .primaryAction) {
                    Button {
                        orchestrator.refreshAvailableTools()
                    } label: {
