...
private struct ActiveIngestionView: View {
    ...
    private var activeJob: AnigmaJob? {
        store.localJobs.first { $0.title.contains(source.name) }
    }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: Bauhaus.Grid.unit) {
                    if let job = activeJob, job.status == .completed {
                        ReportRow(label: "Status", value: "Complete", icon: "checkmark.circle", color: Bauhaus.Color.success)
                    } else if let job = activeJob, job.status == .failed {
                        ReportRow(label: "Status", value: "Failed", icon: "xmark.circle", color: Bauhaus.Color.error)
                    } else {
                        ReportRow(label: "Status", value: "Running", icon: "gearshape.fill", color: Bauhaus.Color.accent)
                    }
                    ReportRow(label: "Storage", value: "Local Only", icon: "shield.fill", color: Bauhaus.Color.trusted)
                }
            }
            .padding(Bauhaus.Grid.x2)
            .background(Bauhaus.Color.surface)
