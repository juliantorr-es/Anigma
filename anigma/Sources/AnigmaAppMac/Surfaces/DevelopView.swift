    ...
    var body: some View {
        if let workspace = store.activeWorkspace {
            HSplitView {
                ...
                DevelopRepoNavigator(workspace: workspace)
...
struct DevelopRepoNavigator: View {
...
struct DevelopBrowseView: View {
    @Environment(AppStore.self) private var store
    @State private var currentURL = URL(string: "https://huggingface.co")!
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack {
                Text("Browser view not yet implemented")
...
struct AgentManagement: View {
    ...
    private var profilesView: some View {
        VStack(spacing: 0) {
            ...
            Button(action: { isCreatingProfile = true }, label: { // plain ButtonStyle
                Label("New Agent Profile", systemImage: "plus.circle")
            })
            .buttonStyle(.plain)
            .padding(Bauhaus.Grid.x2)
            .accessibilityLabel("Create new agent profile")
            .accessibilityHint("Opens form to create a new agent profile")
            .accessibilityElement(children: .combine)
        }
            ...
            ZStack {
                ...
                case .editor:
                    ChunkedCodeEditorDemoView(
                        fileURL: URL(fileURLWithPath: "/tmp/code.swift"),
                        artifactService: store.developArtifactService
                    )
                case .review:
        ...
        List(store.changeSets.filter { $0.workspaceId == workspace.id }) { changeSet in
            Button { store.developWorkbenchTab = .review } label: {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack {
                        DevelopStatusBadge(status: changeSet.status.rawValue)
                        Spacer()
        ...
        if store.workItems.isEmpty {
            ...
            List(store.workItems) { item in
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack {
                        DevelopStatusBadge(status: item.status)
                ...
                VStack(alignment: .leading, spacing: 20) {
                    HStack {
                        ...
                        if first.status == .applying {
                            ...
                            DevelopStatusBadge(status: first.status.rawValue)
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: Bauhaus.Grid.x3) {
                            ...
                            if let hash = first.receiptHash {
                                ProvenanceChip(icon: "checkmark.shield", label: "CoreReceipt", value: hash, isAccent: true)
                            }
                        }
                    }
...
private struct DevelopStatusBadge: View {
    let status: String
    var body: some View {
