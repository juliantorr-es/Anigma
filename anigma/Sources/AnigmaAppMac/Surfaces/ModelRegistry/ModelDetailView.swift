...
public struct ModelDetailView: View {
            ...
            HStack(spacing: 8) {
                ...
                if model.downloads > 0 {
                    HStack(spacing: 4) {
                        ...
                        Text("\(model.downloads.formatted())")
                    }
                    .font(.caption)
                    .foregroundStyle(.secondary)
                }
    ...
    private func loadModelDetails() async {
        ...
        do {
            guard let hfClient = appStore.hfClient else { return }
            let detail = try await hfClient.getModelInfo(repo: model.id)
            await MainActor.run {
    ...
    private func runBenchmark() async {
        guard let systemBenchmark = appStore.systemBenchmark else { return }
