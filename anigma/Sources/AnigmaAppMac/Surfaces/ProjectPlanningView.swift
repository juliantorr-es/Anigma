...
struct ProjectPlanningView: View {
    ...
    private func createProject() {
        let rootPath = store.contexts.first(where: { $0.id == selectedContextId })?.name ?? projectName
        store.currentProject = WorkbenchProject(name: projectName, rootPath: rootPath)
