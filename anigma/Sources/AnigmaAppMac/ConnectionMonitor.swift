...
public final class ConnectionMonitor: ObservableObject {
    ...
    private func checkConnection() async {
        ...
        if isOnline != newStatus {
            ...
            if newStatus {
                ...
                print("✅ [ConnectionMonitor] Daemon reconnected")
                consecutiveFailures = 0
                NotificationCenter.default.post(name: .daemonReconnected, object: nil)
            } else {
                // Daemon went offline
                print("⚠️ [ConnectionMonitor] Daemon went offline")
    ...
    private func attemptRecovery() async {
        ...
        print("🔄 [ConnectionMonitor] Attempting daemon recovery (failure count: \(consecutiveFailures))...")

        do {
            try await capability.ensureDaemonRunning()
            print("✅ [ConnectionMonitor] Daemon recovery successful")
            // Don't reset consecutiveFailures here - let checkConnection() do it on next successful check
        } catch {
            print("❌ [ConnectionMonitor] Daemon recovery failed: \(error.localizedDescription)")
        }
    }
}
