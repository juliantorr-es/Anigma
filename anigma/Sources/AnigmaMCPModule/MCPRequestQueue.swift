...
public actor MCPRequestQueue {
    ...
    private let logger = Logger(subsystem: "com.anigma.mcp", category: "request-queue")
    ...
    public func enqueueRequest(_ context: MCPRequestContext) async -> (accepted: Bool, throttleHint: ThrottleHint?, reason: String?) {
        ...
        if !canAccept {
            ...
            logger.warning(
                "Request rejected \(context.lifecycleMetadataString(stage: "rejected", reason: "client_quota_exceeded", queueDepth: self.queue.count), privacy: .public)"
            )
        ...
        if !decision.accept {
            let reason = decision.reason ?? "throttled"
            logger.warning(
                "Request rejected \(context.lifecycleMetadataString(stage: "rejected", reason: reason, queueDepth: self.queue.count), privacy: .public)"
            )
        ...
        logger.info(
            "Request enqueued \(context.lifecycleMetadataString(stage: "queued", queueDepth: self.queue.count), privacy: .public)"
        )
    ...
    public func recordCompletion(_ requestId: MCPRequestId, clientId: String) {
        if let session = clientSessions[clientId] {
            logger.info(
                "Request completed request_id=\(requestId.stringValue, privacy: .public) client_id=\(clientId, privacy: .public)"
            )
            Task {
    ...
    public func purgeExcessRequests() -> Int {
        ...
        if purged > 0 {
            logger.warning(
                "Purged excess requests queue_depth=\(self.queue.count, privacy: .public) purged_count=\(purged, privacy: .public)"
            )
        }
