...
extension AnigmaMCPServer {
    func handleChat(arguments: [String: Value]?) async -> CallTool.Result {
        ...
        do {
            let input = try await makeInputFromPrompt(message)
            defer {
    ...
    func handleRunModel(
        ...
        do {
            ...
            let input = try await resolveInput(prompt: prompt, inputPath: inputPath)
    ...
    func handleRunEmbedding(
        ...
        do {
            ...
            let input = try await resolveInput(prompt: text, inputPath: inputPath)
    ...
    private func resolveInput(prompt: String?, inputPath: String?) async throws -> ModelInput {
        if let inputPath {
        ...
        return try await makeInputFromPrompt(prompt)
    }

    private func makeInputFromPrompt(_ prompt: String) async throws -> ModelInput {
        let normalizedPrompt = try await preprocessPrompt(prompt)
        let tempFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("mcp_prompt_\(UUID().uuidString).txt")
        try normalizedPrompt.write(to: tempFile, atomically: true, encoding: .utf8)
        let hash = MLWorkerHasher.hashData(Data(normalizedPrompt.utf8))
        return ModelInput(path: tempFile.path, hash: hash, isTemporary: true)
    }

    private func preprocessPrompt(_ prompt: String) async throws -> String {
        let policyOptions = try TextPreprocessingPolicyOptions.fromEnvironment()
        let preprocessingConfiguration = try policyOptions.applying()
        let textPreprocessor = TextPreprocessingSystem(useCapsuleFallback: true)
        let preprocessed = await textPreprocessor.preprocessForStorage(
            prompt,
            configuration: preprocessingConfiguration
        )
        return preprocessed.normalizedText
    }
    ...
    private func blake3Hex(url: URL) throws -> String {
        let data = try Data(contentsOf: url)
    ...
    private func buildArtifactHashes(for path: URL) throws -> (installPath: String, hashes: [String: String], bytes: Int64) {
        ...
        if !isDirectory.boolValue {
            let hash = try blake3Hex(url: path)
        ...
        while let fileURL = enumerator?.nextObject() as? URL {
            ...
            let hash = try blake3Hex(url: fileURL)
