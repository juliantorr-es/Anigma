...
public actor AnigmaMCPServer {
    ...
    internal var observatorium: ObservatoriumModule?
    ...
    private nonisolated(unsafe) var initializationTask: Task<Void, Never>?
    private var hasInitializationStarted = false

    public init(autoInitialize: Bool = true) {
        self.server = Server(
            name: "Anigma",
            version: "1.0.0",
        ...
        if autoInitialize {
            self.hasInitializationStarted = true
        ...
        // Register initial tool contracts after stored properties are initialized.
        registerDefaultTools()
    }

    internal func ensureInitialized() async {
        self.startModuleInitializationIfNeeded()
        await self.initializationTask?.value
    }

    private func startModuleInitializationIfNeeded() {
        guard !self.hasInitializationStarted else { return }
        self.hasInitializationStarted = true
        self.initializationTask = Task { [weak self] in
            guard let self = self else { return }
            await self.initializeModules()
    ...
    internal func initializeRuntime() async throws {
        do {
            ...
            let dbPath = anigmaDir.appendingPathComponent("platform_runtime.postgres").path
            ...
            try await runtime.registerModule(ContextumRuntimeModule.self)
    ...
    internal func initializeContextum() async throws {
        ...
        let preprocessingPolicyOptions = try TextPreprocessingPolicyOptions.fromEnvironment()
        let contextum = try await Contextum(
    ...
    internal func initializeArtifactStore() async throws {
        ...
        let artifactDb = try await ArtifactStoreDatabase(dbActor: runtime.databaseExecutor())
    ...
    internal func initializeCathedral() async throws {
        ...
        let coordinator = try await CathedralModule.create(
            ...
            database: runtime.databaseExecutor()
        )
    ...
    public func listToolsForGemini() async throws -> [String: AnigmaPrimitives.AnyCodable] {
        await ensureInitialized()
        let tools = getTools()
        var functionDeclarations: [[String: AnigmaPrimitives.AnyCodable]] = []

        for tool in tools {
            functionDeclarations.append([
                "name": AnigmaPrimitives.AnyCodable(tool.name),
                "description": AnigmaPrimitives.AnyCodable(tool.description ?? ""),
                "parameters": AnigmaPrimitives.AnyCodable(tool.inputSchema)
            ])
        }

        return ["function_declarations": AnigmaPrimitives.AnyCodable(functionDeclarations)]
    }

    /// Call a tool using Gemini-compatible parameters.
    public func callToolForGemini(name: String, arguments: [String: AnigmaPrimitives.AnyCodable]) async throws -> [String: AnigmaPrimitives.AnyCodable] {
        await ensureInitialized()
        ...
        return [
            "name": AnigmaPrimitives.AnyCodable(name),
            "response": AnigmaPrimitives.AnyCodable([
                "content": content,
    ...
    public func getToolsInternal() -> [MCP.Tool] {
        return getTools()
    }
