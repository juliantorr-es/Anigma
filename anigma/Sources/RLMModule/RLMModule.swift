...
public struct RLMModule: Sendable {
    ...
    public func createGovernor(
        ...
        let env = environment ?? try await ContextEnvironment(
    ...
    public func registerCapability(
        with registry: any CapabilityRegistry,
        evidenceAuthority: (any EvidenceAuthority)? = nil,
        capsules: [String: Any] = [:]
    ) async throws {
        // Create environment
        ...
        print("[RLMModule] RLMCapability registered with ID: \(CapabilityIds.rlm)")
    }
}