...
// This file contains core daemon infrastructure organized by clear MARK sections.
// UI components have been extracted to UI.swift to maintain separation of concerns.
...
// Initialize telemetry with console sink for daemon logging
let consoleSink = ConsoleTelemetrySink(id: "daemon-console")
...
// Log daemon startup with structured telemetry
...
// Log connection handling error
_ = await telemetry.emitError(
module: "daemon.server",
error: "connection_error",
errorCode: "DAEMON_002",
additionalValues: [
"error_description": .hashedToken(TelemetryHash(input: error.localizedDescription))
]
)
...
guard socket >= 0 else {
// swiftlint:disable:next anigma_error_schema
...
// swiftlint:disable:next anigma_error_schema
throw SocketError.failedToEncodeResponse
}

try await writeData(responseData)
...
private let configLogger = Logger(subsystem: "com.anigma.daemon", category: "config")
...
configLogger.warning("Failed to load config: \(error.localizedDescription, privacy: .public) - using defaults")
return .default
}
}
...
private let osLogger = Logger(subsystem: "com.anigma.daemon", category: "filelogger")
...
currentLogSize = size
            ...
            osLogger.warning(
                "Failed to open log file log_path=\(logURL.path, privacy: .public): \(error.localizedDescription, privacy: .public)"
            )
        }
...
rotateLogsIfNeeded()
    ...
    func log(
     ...
     let consoleMessage = "[\(level.rawValue.uppercased())] \(message) (\(file):\(line))"
     let metadataSuffix = metadataFieldsSuffix(metadata)
     
     osLogger.log(level: level.osLogType, "\(consoleMessage, privacy: .public)\(metadataSuffix, privacy: .public)")
...
try? FileManager.default.removeItem(at: file)
            ...
            osLogger.warning(
                "Failed to rotate logs log_directory=\(logDir.path, privacy: .public): \(error.localizedDescription, privacy: .public)"
            )
        }
    }
...
rotateLogsIfNeeded()
            ...
            osLogger.error(
                "Failed to rotate log file log_path=\(currentLogURL.path, privacy: .public): \(error.localizedDescription, privacy: .public)"
            )
        }
    }

    private func metadataFieldsSuffix(_ metadata: [String: String]) -> String {
...
self.logger = logger
    ...
    func start() async {
        await logger.log(.info, "Starting AI service manager", metadata: ["lifecycle_state": "starting"])
...
healthCheckTask = nil
        ...
        await logger.log(.info, "Stopped AI service manager", metadata: ["lifecycle_state": "stopped"])
    }
    ...
    func registerService(_ service: AIService) {
        ...
        Task {
            await logger.log(
                .info,
                "Registered AI service: \(service.name)",
                metadata: ["lifecycle_state": "registered", "service_name": service.name]
            )
        }
    }
...
loadConfig()
    ...
    func start() async {
        ...
        let logger = FileLogger(config: config)
        self.logger = logger
        await logger.log(
            .info,
            "Starting Anigma Daemon",
            metadata: [
                "lifecycle_state": "starting",
                "server_port": String(config.serverPort)
            ]
        )
...
startMetricsCollection()
        ...
        await logger.log(
            .info,
            "Anigma Daemon started successfully",
            metadata: [
...
await server?.stop()
        ...
        await logger?.log(
            .info,
            "Stopping Anigma Daemon",
            metadata: ["lifecycle_state": "stopping"]
        )
        isRunning = false
...
try config.save()
        ...
        await logger?.log(
            ...
            metadata: [
                ...
                "server_port": String(newConfig.serverPort)
            ]
        )
