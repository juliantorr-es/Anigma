...
actor DaemonServer {
    ...
    private func runServer() async {
        do {
            ...
            // Replace raw print with structured log event info
            let serverStartMessage = "Daemon server started on port \(port)"
            // Context: runId for entire server session, will be propagated to jobs
            print(serverStartMessage)  // Keep for visibility during bootstrap
    ...
    private func handleConnection(_ socket: AsyncSocket) async {
        do {
            ...
            // Generate a runId for this connection to correlate all jobs/requests in it
            let connectionRunId = UUID().uuidString
            let response = await handleRequest(request, runId: connectionRunId)
    ...
    private func handleRequest(_ request: HTTPRequest, runId: String) async -> HTTPResponse {
        ...
        case "/job/submit":
            ...
            let jobResponse: [String: String] = [
                "jobId": jobId,
                "runId": runId,  // Propagate connection-level runId to job
                "status": "QUEUED",
        ...
        case "/job/status":
            ...
            let statusResponse: [String: Any] = [
                "jobId": jobId,
                "runId": runId,  // Propagate connection-level runId for correlation
                "status": "RUNNING",
                "progress": 0.5,
                "createdAt": ISO8601DateFormatter().string(from: Date()),
