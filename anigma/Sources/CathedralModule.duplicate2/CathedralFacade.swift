...
public actor CathedralFacade {
    ...
    public func exportEvidenceBundle(
        ...
        return EvidenceBundle(
            ...
            bundleHash: computeBundleHash(sessionId: sessionId, evidence: evidence)
        )
    }

    private func computeBundleHash(sessionId: String, evidence: [Evidence]) -> String {
        let hashInput = "\(sessionId)|\(evidence.count)|\(Date().timeIntervalSince1970)"
