...
public actor CompactionSystem {
    ...
    private func compactTelemetryLogs() async throws -> (segmentsProcessed: Int, bytesReclaimed: Int64) {
        guard let telemetryDir = telemetryDirectory else {
            print("ℹ️ [CompactionSystem] No telemetry directory configured, skipping log compaction")
            return (0, 0)
        }

        // Ensure directory exists
        guard FileManager.default.fileExists(atPath: telemetryDir.path) else {
            print("⚠️ [CompactionSystem] Telemetry directory does not exist: \(telemetryDir.path)")
            return (0, 0)
        }
        ...
        for fileURL in files {
            ...
            do {
                ...
                print("⚠️ [CompactionSystem] Failed to get attributes for \(fileURL.lastPathComponent): \(error)")
                continue
            }
        }
        ...
        for segment in segmentsToArchive {
            do {
                ...
                print("✅ [CompactionSystem] Archived segment \(segment.url.lastPathComponent) (reclaimed \(bytesReclaimed) bytes)")
            } catch {
                print("❌ [CompactionSystem] Failed to archive segment \(segment.url.lastPathComponent): \(error)")
            }
        }

        if segmentsProcessed > 0 {
            print("🗜️ [CompactionSystem] Compacted \(segmentsProcessed) telemetry segments, reclaimed \(totalBytesReclaimed) bytes")
        }
    ...
    private func archiveSegment(_ segmentURL: URL, originalSize: Int64) async throws -> Int64 {
        ...
        do {
            ...
            let compressor = try CompressionCapsuleWrapper(config: config)
            compressedData = try compressor.compress(originalData)
        } catch {
...
extension ContextumDatabase {
    func vacuum() async throws {
        _ = try await dbActor.executeAsync("VACUUM")
    }

    func optimizeFTS() async throws {
        _ = try await dbActor.executeAsync("INSERT INTO fts_chunks(fts_chunks) VALUES('optimize')")
    }
    ...
    private func getDatabaseSizeInternal() async -> Int64 {
        guard let path = databasePath else {
            print("⚠️ [ContextumDB] Database path not set, cannot calculate size")
            return 0
        }
        ...
        do {
            ...
            if let fileSize = attrs[FileAttributeKey.size] as? Int64 {
                ...
                print("⚠️ [ContextumDB] Could not read file size attribute from database")
                return 0
            }
        } catch {
            print("❌ [ContextumDB] Failed to get database size at '\(path)': \(error)")
