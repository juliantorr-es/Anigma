...
// Using canonical MediaFingerprintCapsule types directly avoids reviving the older nested API surface.
public typealias CapsuleMediaType = MediaType
public typealias CapsuleFingerprintAlgorithm = FingerprintAlgorithm
public typealias CapsuleFingerprintResult = FingerprintResult
public typealias CapsuleSimilarityResult = SimilarityResult
...
public struct MediaDeduplicationSystem: Sendable {
    public struct MediaFingerprint: Codable, Sendable {
        ...
        public let mediaType: CapsuleMediaType
        public let algorithm: CapsuleFingerprintAlgorithm
        ...
        public init(
            ...
            algorithm: CapsuleFingerprintAlgorithm,
            hashData: Data,
            hashSize: UInt32,
            confidence: Double,
    ...
    public enum DeduplicationError: Error, Sendable {
        ...
        case unsupportedMediaType(CapsuleMediaType)
    }
    ...
    private func generateFingerprintWithCapsule(
        ...
        let algorithm: CapsuleFingerprintAlgorithm
        let fingerprintResult: CapsuleFingerprintResult
    ...
    private func findDuplicatesWithCapsule(
        ...
        let queryFingerprint = CapsuleFingerprintResult(
        ...
        let similarityResults: [CapsuleSimilarityResult]
        do {
            similarityResults = capsule.batchCompareFingerprints(
    ...
    public func isDuplicate(
        ...
        if let capsule = self.capsule {
            ...
            let results = capsule.batchCompareFingerprints(
                query: queryFingerprint,
                candidates: candidateFingerprints,
                threshold: Double(threshold)
            )
            if results.contains(where: { result in
                result.isDuplicate || result.similarityScore >= Double(threshold)
            }) {
                return true
            }

            for existing in existingFingerprints {
                let candidateFingerprint = CapsuleFingerprintResult(
                    algorithm: existing.algorithm,
                    hashSize: existing.hashSize,
                    hashData: existing.hashData,
                    confidence: existing.confidence,
                ...
                do {
                    let result = capsule.compareFingerprints(
                        queryFingerprint,
    ...
    private func calculateHammingDistance(_ data1: Data, _ data2: Data) -> UInt32 {
        ...
        distance += UInt32(Swift.abs(bytes1.count - bytes2.count) * 8)
        return distance
    }
    ...
    public func getMediaFingerprints(
        ...
        return rows.compactMap { row -> MediaDeduplicationSystem.MediaFingerprint? in
            ...
            guard let mediaTypeRaw = row.string(for: "media_type"),
                  ...
                  let mediaType = CapsuleMediaType(rawValue: mediaTypeUInt8) else {
                return nil
            }

            guard let algorithmRaw = row.string(for: "algorithm"),
                  let algorithmUInt8 = UInt8(algorithmRaw),
                  let algorithm = CapsuleFingerprintAlgorithm(rawValue: algorithmUInt8) else {
                return nil
            }
