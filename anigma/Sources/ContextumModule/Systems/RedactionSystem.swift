...
public actor RedactionSystem {
    ...
    public init(database: ContextumDatabase, telemetry: TelemetryClient? = nil) {
    ...
    private func applyRule(_ rule: RedactionRule, to content: String) -> String {
        ...
        do {
            ...
            // Log invalid regex pattern error
            Task {
                _ = await telemetry.emitError(
                    module: "ContextumModule.RedactionSystem",
                    error: "invalid_regex_pattern",
                    errorCode: "REDACT_001",
                    privacyClassification: .internal,
                    additionalValues: [
                        "rule_id": .hashedToken(TelemetryHash(input: rule.ruleID)),
