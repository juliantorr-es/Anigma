...
public actor ContextumRankFusionCapsuleWrapper {
    ...
    public init(config: RankFusionConfig = .default) throws {
        ...
        guard status == ANIGMA_OK, let rawHandle = rawHandle else {
            throw rankFusionCapsuleError(status: status, error: error)
        }
...
private func rankFusionCapsuleError(status: anigma_status_t, error: anigma_capsule_error_t) -> CapsuleNativeError {
    let message = error.message.map { String(cString: $0) } ?? "Rank fusion capsule error"
