...
public final class AnomalyScanSystem {
    private static let logger = Logger(subsystem: "com.anigma.ContextumModule", category: "AnomalyScanSystem")
    ...
    public func scanForAnomalies(
        ...
        if !anomalies.isEmpty {
            Self.logger.warning("Detected \(anomalies.count, privacy: .public) anomalies")
            for anomaly in anomalies {
                Self.logger.warning("[\(anomaly.severity.rawValue.uppercased(), privacy: .public)] \(anomaly.detectorID, privacy: .public) for \(anomaly.subjectKey, privacy: .public)")
            }
        }
