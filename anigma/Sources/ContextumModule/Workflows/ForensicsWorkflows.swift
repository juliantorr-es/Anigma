...
private let forensicsWorkflowLogger = Logger(
    subsystem: "com.anigma.ContextumModule",
    category: "ForensicsWorkflows"
)
...
public struct FailureReportWorkflow {
    public static func execute(
        ...
        if returnedChunkHashes.isEmpty {
            ...
            supportingFacts["chunk_count"] = AnyCodable(0)
        ...
        if let lastFailure = failureEvents.last,
           ...
           errorCode.contains("model") || errorCode.contains("timeout") {
            var supportingFacts: [String: AnyCodable] = [:]
            supportingFacts["error_code"] = AnyCodable(errorCode)
            supportingFacts["duration_ms"] = AnyCodable(lastFailure.durationMs ?? 0.0)
...
public struct ReplayWorkflow {
    public static func execute(
        ...
        try await database.insertReplayLinkage(linkage, chunkHashes: verifiedChunkHashes)

        forensicsWorkflowLogger.info(
            "Replay prepared for run \(originalRunID, privacy: .public); reconstructed \(verifiedChunkHashes.count, privacy: .public)/\(uniqueChunkHashes.count, privacy: .public) context chunks"
        )
        if uniqueChunkHashes.count != verifiedChunkHashes.count {
            forensicsWorkflowLogger.warning(
                "\(uniqueChunkHashes.count - verifiedChunkHashes.count, privacy: .public) chunks missing from corpus for run \(originalRunID, privacy: .public)"
            )
        }
