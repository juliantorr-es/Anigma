...
ANIGMA_FAST = AgentProfile(
    name="anigma-fast",
    display_name="Anigma Fast",
    description="High-throughput agent for rapid, low-risk edits (renames, trivial fixes).",
    safety=AgentSafety.YOLO,
    overrides={
        "bypass_tool_permissions": True,
        "system_prompt_id": "anigma-implement",
        "tools": {"bash": {"permission": "always"}},
    },
)

ANIGMA_VALIDATE = AgentProfile(
    name="anigma-validate",
    display_name="Anigma Validate",
    description="Strict read-only agent for running tests, validators, and checking build integrity.",
    safety=AgentSafety.SAFE,
    overrides={
        "enabled_tools": ["grep", "read_file", "bash"],
        "system_prompt_id": "anigma-audit",
        "tools": {
            "bash": {
                "permission": "always",
                "allowlist": ["swift test", "swift build", "swiftlint", "td status"]
            }
        },
    },
)

ANIGMA_DOCUMENT = AgentProfile(
    name="anigma-document",
    display_name="Anigma Document",
    description="Dedicated agent for writing documentation, generating reports, and artifact maintenance.",
    safety=AgentSafety.NEUTRAL,
    overrides={
        "system_prompt_id": "anigma-doc",
        "tools": {
            "write_file": {"permission": "always", "allowlist": ["*.md", "Docs/"]},
        },
    },
)
...
BUILTIN_AGENTS: dict[str, AgentProfile] = {
    ...
    "anigma-fast": ANIGMA_FAST,
    "anigma-validate": ANIGMA_VALIDATE,
    "anigma-document": ANIGMA_DOCUMENT,
}
