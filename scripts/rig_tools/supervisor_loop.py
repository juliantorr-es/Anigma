...
from rig_tools import agent_plan, context_compression, events, mlx_local, result, schema_validation
...
def draft_plan(repo_root: Path, *, task: str, mode: str, backend: str, model: str, max_steps: int = DEFAULT_MAX_STEPS, allow_local_patches: bool = False) -> tuple[dict[str, Any], Path | None]:
    ...
    result_payload = mlx_local.generate_summary(prompt=prompt, model=model, max_tokens=512, timeout_seconds=600)
