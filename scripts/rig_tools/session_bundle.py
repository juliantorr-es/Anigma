...
def _read_task_context(repo_root: Path, task: str) -> dict[str, Any]:
    from rig_tools import git_manage, result_index
...
def build_manifest(repo_root: Path, *, task: str, run_id: str | None = None, latest_run: bool = False, include_untracked: bool = False) -> dict[str, Any]:
...
def write_bundle(repo_root: Path, *, task: str, run_id: str | None = None, latest_run: bool = False, out: Path | None = None, dry_run: bool = False, include_untracked: bool = False, max_log_bytes: int = 200000, max_file_bytes: int = 1000000, overwrite: bool = False) -> BundleResult:
    ...
    manifest = build_manifest(repo_root, task=task, run_id=run_id, latest_run=latest_run, include_untracked=include_untracked)
    ctx = _read_task_context(repo_root, task)
